...
Функция ПроверитьВозможностьВыгрузки(ИмяФормы,
	...
	Если ЗаполненРаздел(ИмяФормы, ДанныеМногоуровневыхРазделов["Лист1_1"],
		...
		ИначеЕсли ЗначПок = "643" ИЛИ ЗначПок = "010" Тогда
			СообщитьВТС(ИмяФормы, РезультатВыгрузки,
			"Сведения о нерезиденте: код страны не должен быть равен 643 или 010", "Лист1_1", "П0Л1010000002", НомСтраницы);
			СтатусПроверки = Ложь;
		КонецЕсли;
	...
	Если ЗаполненРаздел(ИмяФормы, ДанныеМногоуровневыхРазделов["Лист1_3"],
		...
		Для каждого СтрокаМнЧ Из ДанныеМнЧ.Строки Цикл
			...
			ИначеЕсли ЗначПок = "643" ИЛИ ЗначПок = "010" Тогда
				СообщитьВТС(ИмяФормы, РезультатВыгрузки, "Сведения об участниках-нерезидентах: в строке " + НомСтроки
				+ " код страны не должен быть равен 643 или 010", "Лист1_3", "П0Л103М100002_" + НомСтроки, НомСтраницы);
				СтатусПроверки = Ложь;
			КонецЕсли;
