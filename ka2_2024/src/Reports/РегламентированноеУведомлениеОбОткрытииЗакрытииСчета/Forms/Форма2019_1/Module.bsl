...
Процедура ЗаполнитьНачальныеДанные()
	...
	Иначе
		...
		ДанныеУведомленияТитульный.Вставить("ПРИЗНАК_НП_ПОДВАЛ", ?(ЭтотОбъект["ЭтоЮЛ"], "1", "3"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ДоступностьОбластиКоррФайл()
	Если ЭтотОбъект["ТекущееИДНаименования"] = "Титульная" Тогда
		...
		ОФ.ЦветФона = ?(ЭтотОбъект["ЭтоЮЛ"], ЭтотОбъект["ЦФЖ"], ЭтотОбъект["ЦФБ"]);
		ОФ.АвтоОтметкаНезаполненного = ЭтотОбъект["ЭтоЮЛ"];
		ОФ.Защита = Не ЭтотОбъект["ЭтоЮЛ"];
	КонецЕсли;
КонецПроцедуры
...
Процедура ПоказатьТекущуюСтраницу(ИмяМакета, МногострочныеЧасти, ПредУИД)
	...
	Если ЭтотОбъект["ТекущееИДНаименования"] = "Лист001" Тогда 
...
Процедура УстановитьДанныеПоРегистрацииВИФНС()
	...
	Иначе
		УведомлениеОСпецрежимахНалогообложения.УстановитьПредставителяПоОрганизации(ЭтотОбъект);
		ПредставлениеУведомления.Области["ПРИЗНАК_НП_ПОДВАЛ"].Значение = ?(ЭтотОбъект["ЭтоЮЛ"], "1", "3");
	КонецЕсли;
...
Функция ЭтоОбластьОКСМ(Область)
	Возврат (Область.Имя = "КодСтр" И ЭтотОбъект["ТекущееИДНаименования"] = "Лист001");
КонецФункции
...
Процедура ЗагрузитьИзXMLНаСервере(ПараметрыЗагрузкиXML)
	...
	Если ТипЗнч(УзелНомКорр) = Тип("СтрокаДереваЗначений") Тогда 
		...
		ЭтотОбъект["ДанныеУведомления"].Титульная.НомКорр = ОТЧ.ПривестиЗначение(УзелНомКорр.ЗначениеЭлемента);
	КонецЕсли;
