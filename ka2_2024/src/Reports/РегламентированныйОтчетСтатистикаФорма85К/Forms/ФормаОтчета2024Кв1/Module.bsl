...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	//Раздел 3
	//Стр. 301 гр. 3 = Сумме стр. 302–303 гр. 3
	Области["П003001030103"].Значение
		= Области["П003001030203"].Значение
		+ Области["П003001030303"].Значение;
	...
	//Раздел 6
	//Стр. 602 = Сумме Стр. 603, 606, 609, 613, 616, 618–620 по графам 3–8
	Для Гр = 4 По 8 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		СуммаСтр602 = 0;
	...
	//Стр.601 = Сумме Стр. 602, 621, 622, 626–629 по графам 3–8
	Для Гр = 4 По 8 Цикл
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		СуммаСтр601 = 0;
		Для Стр = 602 По 629 Цикл
			Если (Стр > 602 И Стр < 621) Или (Стр > 622 И Стр < 626) Тогда
				Продолжить;
			КонецЕсли;
			Если Стр = 627 И (Гр = 6 Или Гр = 7 Или Гр = 8) Тогда
				Продолжить;
			КонецЕсли;
	...
	//Раздел 7
	//Гр. 3 Стр. 701–718 = Сумме Гр. 4-11 по строкам 701–719
	...
	//Гр.13= Гр. 3–Гр. 4
	Для Стр = 801 по 825 Цикл
		Области["П0080010" + Стр + "13"].Значение
			= Области["П0080010" + Стр + "03"].Значение
			- Области["П0080010" + Стр + "04"].Значение;
	КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П003001030103" + ";";
	...
	Для Стр = 801 по 825 Цикл
		Если Стр = 801 Или Стр = 806 Тогда
			Продолжить;
		КонецЕсли;
		Графа = Формат(Гр, "ЧЦ=2; ЧВН=");
		ЯчейкиАвторасчета = ЯчейкиАвторасчета + "П0080010" + Стр + "13" + ";";
	КонецЦикла;
