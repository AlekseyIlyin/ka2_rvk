...
Процедура ОбработкаПрПолучатель()
	...
	Для Каждого Стр Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
		...
		ЭтотОбъект["ДанныеУведомления"].ЗаявВозврДенСр[Стр] = ПредставлениеУведомления.Области[Стр].Значение;
...
Процедура ДеревоСтраницПриАктивизацииСтрокиЗавершение() Экспорт 
	...
	Если ЭтотОбъект["ТекущееИДНаименования"] = "ЗаявВозврДенСр" Тогда
		ОбработкаПрПолучатель();
	КонецЕсли;
КонецПроцедуры
