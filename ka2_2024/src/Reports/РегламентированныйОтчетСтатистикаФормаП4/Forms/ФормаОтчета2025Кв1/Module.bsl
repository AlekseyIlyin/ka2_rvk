...
Процедура ПеренумерацияСтрокТаблицы(ЭтотОбъект)
	...
	Пока Истина Цикл
		Инд = Инд + 1;
		Обл = Области.Найти("П001001000208_" + Инд);
		Если Обл = Неопределено Тогда
			Прервать;
		КонецЕсли;
		Обл2        = Области.Найти("П001002000202_" + Инд);
		НомерСтроки = Инд + 1;
...
Процедура Расшифровать(Команда)
	...
	Если СтрЧислоВхождений(Элементы.ПолеТабличногоДокументаФормаОтчета.ТекущаяОбласть.Имя, "П001") > 0 Тогда
		
		ИмяОбласти = "П001001000207" + Сред(Элементы.ПолеТабличногоДокументаФормаОтчета.ТекущаяОбласть.Имя, 14);
