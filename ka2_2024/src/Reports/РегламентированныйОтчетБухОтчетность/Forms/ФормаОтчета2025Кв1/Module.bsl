...
Процедура УстановитьПервоначальнуюВидимостьЭлементов()
	...
	Элементы.ПодменюПроверка.Видимость = Ложь;
	Элементы.ПодменюОтправка.Видимость = Ложь;
...
Процедура СформироватьСоставПоказателей_ОИК()
	...
	ПараметрыПоказателей.Графы = "01С0, 04Е0, 05Е0, 06Е0, 07Е0, 08Е0, 09Е0, 10Е0";
...
Функция ЕстьНастраиваемыеРазделыОтчета(РазделыСПоддержкойНастройкиЗаполнения)
	...
	ЕстьНастраиваемыеРазделыОтчета = Ложь;
...
Процедура Расчет_ОИК(Показатели, ПараметрыРасчета)
	...
	Графы = Список("04, 05, 06, 07, 08, 09, 10");
...
//
&НаСервере
Функция ВеткаРаздела(ДеревоДанныхРазделов, ИмяРаздела)
...
Функция ЗначениеПоказателяРаздела(ДеревоДанныхРазделов, ИмяРаздела, ИмяПоказателя)
	...
	Возврат РазделОтчета.Строки.НайтиСтроки(ПараметрыПоиска)[0].Данные;
...
Процедура ВывестиНаПечатьЛист(СтруктураПараметровЛиста)
	...
	Ориентация = СтруктураПараметровЛиста.ОриентацияЛиста;
	...
	ПечатнаяФорма.ОриентацияСтраницы  = ?(Ориентация = "Портрет",
