...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(ЭтотОбъект["ДанныеУведомления"].ЛистБ.ОбщСумНалПатУм) Тогда 
		ЭтотОбъект["ДанныеУведомления"].ЛистБ.ОбщСумНалПатУм = 0;
	КонецЕсли;
...
Процедура ЗаполнитьУведомлениеДаннымиПомощника(ДанныеПомощника)
	...
	Для Каждого Элемент Из ДанныеПомощника.ЛистБ Цикл
		ЭтотОбъект["ДанныеУведомления"].ЛистБ[Элемент.Ключ] = Элемент.Значение;
	КонецЦикла;
	...
	Если ЛистыАВДереве <> Неопределено Тогда
		...
		КонецЦикла;
		ЭтотОбъект["ДанныеДопСтрок"].МнгСтр = ПоместитьВоВременноеХранилище(ДанныеЛистаА, УникальныйИдентификатор);
	КонецЕсли;
...
Процедура ПредставлениеУведомленияПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "СумНалПатУм" + ЭтотОбъект["РазделительНомераСтроки"]) Тогда 
		Расчет();
	КонецЕсли;
КонецПроцедуры
...
Процедура Расчет()
	...
	Пока Истина Цикл 
		Инд = Инд + 1;
		Обл = ПредставлениеУведомления.Области.Найти("СумНалПатУм" + ЭтотОбъект["РазделительНомераСтроки"] + Формат(Инд, "ЧГ="));
	...
	ЭтотОбъект["ДанныеУведомления"].ЛистБ.ОбщСумНалПатУм = Сум;
КонецПроцедуры
