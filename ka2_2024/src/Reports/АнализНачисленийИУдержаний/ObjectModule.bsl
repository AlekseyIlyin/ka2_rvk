...
Процедура ПриКомпоновкеОтчета(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Иначе
		...
		ПараметрИсчисленныеНалоги = НастройкиОтчета.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ИсчисленныеНалоги"));
		Если ПараметрИсчисленныеНалоги <> Неопределено Тогда
			ПараметрИсчисленныеНалоги.Значение = Перечисления.ВидыОсобыхНачисленийИУдержаний.СтрокиИсчисленныхНалогов();
		КонецЕсли;
