...
Асинх Процедура Удалить(Команда)
	ВыделенныеСтроки = Элементы.ПриложенныеФайлы.ВыделенныеСтроки;
	Если ВыделенныеСтроки.Количество() = 0 Тогда 
		Возврат;
	КонецЕсли;
	
	Если Ждать ВопросАсинх("Операция удаления приложенных документов не может быть отменена. Продолжить?", РежимДиалогаВопрос.ДаНет)
		<> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;
	
	ФайлыКУдалению = Новый Соответствие;
	ПапкиКУдалению = Новый Соответствие;
	Для Каждого Элт Из Элементы.ПриложенныеФайлы.ВыделенныеСтроки Цикл 
		ЭлементВДереве = ПриложенныеФайлы.НайтиПоИдентификатору(Элт);
		Если ЗначениеЗаполнено(ЭлементВДереве.ПрисоединенныйФайл) Тогда 
			ФайлыКУдалению.Вставить(Элт);
		ИначеЕсли ЭлементВДереве.ПолучитьРодителя() = Неопределено Тогда 
			ПапкиКУдалению.Вставить(ЭлементВДереве.ПолучитьИдентификатор());
			Для Каждого Подч Из ЭлементВДереве.ПолучитьЭлементы() Цикл 
				ФайлыКУдалению.Вставить(Подч.ПолучитьИдентификатор());
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	
	Если ФайлыКУдалению.Количество() > 0 Тогда 
		УдалитьФайлыНаСервере(ФайлыКУдалению, ПапкиКУдалению);
	ИначеЕсли ПапкиКУдалению.Количество() > 0 Тогда 
		Для Каждого Папка Из ПапкиКУдалению Цикл 
			ПриложенныеФайлы.ПолучитьЭлементы().Удалить(ПриложенныеФайлы.НайтиПоИдентификатору(Папка.Ключ));
		КонецЦикла;
		Модифицированность = Истина;
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Процедура УдалитьФайлыНаСервере(ФайлыКУдалению, ПапкиКУдалению)
	Модифицированность = Истина;
	Для Каждого Элт Из ФайлыКУдалению Цикл 
		Стр = ПриложенныеФайлы.НайтиПоИдентификатору(Элт.Ключ);
		УведомлениеОСпецрежимахНалогообложения.УдалениеПрисоединенногоСлужебногоФайла(Стр.ПрисоединенныйФайл);
