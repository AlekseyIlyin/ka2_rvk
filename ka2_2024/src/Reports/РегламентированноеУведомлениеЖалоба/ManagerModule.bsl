...
Функция СформироватьСписокЛистовФорма2024_Отзыв(Объект)
	...
	ДоплонительныеПараметры = Новый Структура;
	ДоплонительныеПараметры.Вставить("НаимНО", "ИФНС " + Титульная.КодНО);
	ДоплонительныеПараметры.Вставить("Флаг" + Отзыв.ОтзывЖалоб, "V");
	ДоплонительныеПараметры.Вставить("ОписаниеОрганизации", СокрЛП(Титульная.Фамилия + " " + Титульная.Имя + " " + Титульная.Отчество 
		+ Титульная.НаимОрг + " " + Титульная.ИНН + " " + Титульная.КПП));
	ЗаполнитьЗначенияСвойств(МакетПФ.Параметры, ДоплонительныеПараметры);
...
Функция СформироватьСписокЛистовФорма2024_ХодПриостРассм(Объект)
	...
	ДоплонительныеПараметры = Новый Структура;
	ДоплонительныеПараметры.Вставить("НаимНО", "ИФНС " + Титульная.КодНО);
	ДоплонительныеПараметры.Вставить("Флаг" + ХодПриостРассм.ПриостЖалоб, "V");
	ДоплонительныеПараметры.Вставить("ОписаниеОрганизации", СокрЛП(Титульная.Фамилия + " " + Титульная.Имя + " " + Титульная.Отчество 
		+ Титульная.НаимОрг + " " + Титульная.ИНН + " " + Титульная.КПП));
	ЗаполнитьЗначенияСвойств(МакетПФ.Параметры, ДоплонительныеПараметры);
	ЗаполнитьЗначенияСвойств(МакетПФ.Параметры, Титульная);
...
Функция СформироватьСписокЛистовФорма2024_ПриостРеш(Объект)
	...
	ДоплонительныеПараметры = Новый Структура;
	ДоплонительныеПараметры.Вставить("НаимНО", "ИФНС " + Титульная.КодНО);
	ДоплонительныеПараметры.Вставить("НаимНОРеш", "ИФНС " + ПриостРеш.КодНОРеш);
	ДоплонительныеПараметры.Вставить("Флаг" + ПриостРеш.ПриостОбжРеш, "V");
	ДоплонительныеПараметры.Вставить("ОписаниеОрганизации", СокрЛП(Титульная.Фамилия + " " + Титульная.Имя + " " + Титульная.Отчество 
		+ Титульная.НаимОрг + " " + Титульная.ИНН + " " + Титульная.КПП));
	...
	Для Инд = 1 По 3 Цикл 
		...
		ДоплонительныеПараметры.Вставить("НомБанкГар" + Инд, Стр["НомБанкГар"]);
		ДоплонительныеПараметры.Вставить("СумБанкГар" + Инд, Стр["СумБанкГар"]);
		ДоплонительныеПараметры.Вставить("СрокДо" + Инд, Стр["СрокДо"]);
		ДоплонительныеПараметры.Вставить("Гарант" + Инд, СокрЛП(Стр["НаимГарант"] + " " + Стр["ИННЮЛ"] + "/" + Стр["КПП"] + "/" + Стр["БИК"]));
	КонецЦикла;
	ЗаполнитьЗначенияСвойств(МакетПФ.Параметры, ДоплонительныеПараметры);
	ЗаполнитьЗначенияСвойств(МакетПФ.Параметры, Титульная);
...
Функция СформироватьСписокЛистовФорма2024_Жалоба(Объект)
	...
	ДоплонительныеПараметры.Вставить("Флаг" + Жалоба.ПредмОбжал, "V");
	ДоплонительныеПараметры.Вставить("ВидДок" + Жалоба.ВидДок, "V");
	ДоплонительныеПараметры.Вставить("НомДок" + Жалоба.ВидДок, Жалоба.НомДок);
	ДоплонительныеПараметры.Вставить("ПорядРассм" + Жалоба.ПорядРассм, "V");
...
Функция ЭлектронноеПредставление_Форма2024_1(Объект, УникальныйИдентификатор)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСФайлами") Тогда
		...
		Для Каждого Элт Из Прилож Цикл 
			СтрокаСведенийЭлектронногоПредставления = СведенияЭлектронногоПредставления.Добавить();
			СтрокаСведенийЭлектронногоПредставления.ИмяФайла = Элт.Значение.ИмяФайлПрил;
