...
Функция ПолучитьТекстЗапросаПоКонтролируемымСделкам_2012() Экспорт
	...
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ИсторияНаименованийКонтрагента.СокращенноеНаименование КАК СТРОКА(1000)), Контрагенты.НаименованиеПолное) КАК Строка040Наименование,
...
Функция ПолучитьТекстЗапросаПоКонтролируемымСделкам_2018() Экспорт
	...
	|	ЕСТЬNULL(ВЫРАЗИТЬ(ИсторияНаименованийКонтрагента.СокращенноеНаименование КАК СТРОКА(1000)), ГоловныеКонтрагенты.НаименованиеПолное) КАК Строка040Наименование,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ДокументСделки = Неопределено ИЛИ
			...
			Если ДокументСделки <> Неопределено Тогда
				...
				Если ПроверитьНеобходимостьЗаписи(ДокументСделки) Тогда
...
Процедура ДобавитьСделкиКорректировкиПоступления(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|		ИНАЧЕ ЕСТЬNULL(ДанныеДокумента.НомерГТД.СтранаПроисхождения, ЗНАЧЕНИЕ(Справочник.СтраныМира.Россия))
	|	КОНЕЦ КАК СтранаПроисхожденияПредметаСделки,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперацииКонтролируемыхСделок.Корректировка) КАК ХозяйственнаяОперация,
	|	ЕСТЬNULL(ДанныеДокумента.Характеристика.Принципал, ЕСТЬNULL(ДанныеНоменклатуры.Принципал, НЕОПРЕДЕЛЕНО)) = НЕОПРЕДЕЛЕНО
	|		ИЛИ ЕСТЬNULL(ДанныеДокумента.Характеристика.Принципал, ЕСТЬNULL(ДанныеНоменклатуры.Принципал, НЕОПРЕДЕЛЕНО))
...
Функция ПроверитьНеобходимостьЗаписи(ДокументСделки)
	
	Записывать = Ложь;
	
	Если ДокументСделки.СуммаДоходов <> 0 ИЛИ ДокументСделки.СуммаДоходовРегулируемых <> 0
					ИЛИ ДокументСделки.СуммаРасходов <> 0 ИЛИ ДокументСделки.СуммаРасходовРегулируемых <> 0
					ИЛИ ДокументСделки.Сделки.Количество()> 0 Тогда
		Записывать = Истина;
	КонецЕсли;
