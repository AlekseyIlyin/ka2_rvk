...
Процедура ПриПолученииДанных(ТекущийОбъект)
	...
	ДоступноОтклонениеЗаявки = (Объект.ПричинаОтсутствия = Перечисления.ПричиныОтсутствийЗаявокКабинетСотрудника.ДниУходаЗаДетьмиИнвалидами
		Или Объект.ПричинаОтсутствия = Перечисления.ПричиныОтсутствийЗаявокКабинетСотрудника.Командировка
		Или Объект.ПричинаОтсутствия = Перечисления.ПричиныОтсутствийЗаявокКабинетСотрудника.Отгул);
...
Процедура ИнициализацияФормы()
	...
	Если Не ДоступноОтклонениеЗаявки Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"ГруппаПояснениеСвязанныеЗаявки", "Видимость", Ложь);
	КонецЕсли;
	
	УстановитьТекстПоясненияПорядкаВыполнения();
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаЗаявкаОтклонена", "Видимость", ДоступноОтклонениеЗаявки);
...
Функция УстановитьТекстПоясненияПорядкаВыполнения()
	...
	ИначеЕсли ДоступноОтклонениеЗаявки Тогда
	...
	Иначе
		Строка1 = НСтр("ru = 'Для выполнения заявки надо создать новый документ или выбрать уже существующий,'");
		Строка2 = НСтр("ru = 'после этого заявку можно отметить как выполненную.'");
		ТекстПояснения = СтрШаблон("%1 %2", Строка1, Строка2);
	КонецЕсли;
...
Функция ОписаниеДокумента(Форма)
	...
	КонецЕсли;
	Описание.ДоступноОтклонениеЗаявки = Форма.ДоступноОтклонениеЗаявки;
