...
Процедура ДанныеФормыВДанныеМенеджераРасчета(МенеджерРасчета, Отбор = Неопределено, ПозицииВставки = Неопределено, ПерезаполнитьНачисления = Ложь)
	...
	Для Каждого СтрокаСотрудника Из Объект.Начисления Цикл 
		...
		ЧастичнаяИндексация.Вставить(СтрокаСотрудника.Сотрудник,
			Новый Структура("СреднийЗаработокИндексируемый, СреднийЗаработокНеиндексируемый", 
			СтрокаСотрудника.СреднийЗаработокИндексируемый, СтрокаСотрудника.СреднийЗаработокНеиндексируемый));
	КонецЦикла;
	...
	Если ПерезаполнитьНачисления Тогда
		Для Каждого Строка Из МенеджерРасчета.Зарплата.Начисления Цикл
			...
			Если ПолучитьФункциональнуюОпцию("ИндексироватьСреднийЗаработокЧастично") Тогда
				ДанныеДокумента = ЧастичнаяИндексация[Строка.Сотрудник];
...
Процедура РассчитатьСреднийЗаработок(ДанныеСотрудника)
	...
	Если ПолучитьФункциональнуюОпцию("ИндексироватьСреднийЗаработокЧастично") Тогда
		ЧастичнаяИндексация = УчетСреднегоЗаработкаКлиентСервер.ДанныеЧастичнойИндексацииСреднегоЗаработка(
			СреднийЗаработокОбщий, ОтработанноеВремяДляСреднегоОбщий, ДополнительныеПараметры, ДанныеСотрудника.СреднийЗаработок);			
		ДанныеСотрудника.СреднийЗаработокИндексируемый = ЧастичнаяИндексация.ИндексируемыйЗаработок;
		ДанныеСотрудника.СреднийЗаработокНеиндексируемый = ЧастичнаяИндексация.НеиндексируемыйЗаработок;
	КонецЕсли;
