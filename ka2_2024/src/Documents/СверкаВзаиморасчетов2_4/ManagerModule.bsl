	...
	Для Каждого СтрокаРасчетов Из ТаблицаВзаиморасчетов Цикл
		...
		Если СтрокаРасчетов.НачальноеСальдо = 0 И ДанныеСумм.ЕстьЗаполненные Тогда
			...
			Если СФДокумента.Количество() > 0 Тогда
				...
				ОписаниеДокумента = ОписаниеРасчетногоДокумента(СтрокаРасчетов.РасчетныйДокумент, СтрокаРасчетов.НомерДокументаИБ, СтрокаРасчетов.ДатаДокументаИБ, "");
...
Функция УстаревшиеПредставленияДокументов()
	...
	//++ НЕ УТ
	ВидыДокументовКонтрагента.Вставить("ВыбытиеДенежныхДокументов",             НСтр("ru='Расходный кассовый ордер'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ Устарело_Переработка24
	ВидыДокументовКонтрагента.Вставить("ЗаказПереработчику",                    НСтр("ru='Заказ'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- Устарело_Переработка24
	ВидыДокументовКонтрагента.Вставить("ЗаказПереработчику2_5",                 НСтр("ru='Заказ'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ Устарело_Переработка24
	ВидыДокументовКонтрагента.Вставить("ОтчетПереработчика",                    НСтр("ru='Отчет переработчика'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- Устарело_Переработка24
	ВидыДокументовКонтрагента.Вставить("ОтчетПереработчика2_5",                 НСтр("ru='Отчет переработчика'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("ПоступлениеДенежныхДокументов",         НСтр("ru='Приходный кассовый ордер'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("ОтчетОператораСистемыПлатон",           НСтр("ru='Акт'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
...
Функция ПредставленияВладельцевДокументов()
	...
	//++ НЕ УТ
	ВидыДокументовКонтрагента.Вставить("ВыбытиеДенежныхДокументов",             НСтр("ru='плательщик'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ Устарело_Переработка24
	ВидыДокументовКонтрагента.Вставить("ЗаказПереработчику",                    НСтр("ru='от'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- Устарело_Переработка24
	ВидыДокументовКонтрагента.Вставить("ЗаказПереработчику2_5",                 НСтр("ru='от'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ Устарело_Переработка24
	ВидыДокументовКонтрагента.Вставить("ОтчетПереработчика",                    НСтр("ru='от'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- Устарело_Переработка24
	ВидыДокументовКонтрагента.Вставить("ОтчетПереработчика2_5",                 НСтр("ru='от'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("ПоступлениеДенежныхДокументов",         НСтр("ru='получатель'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("ОтчетОператораСистемыПлатон",           НСтр("ru='от'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
