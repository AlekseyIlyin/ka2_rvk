...
Процедура РассчитатьСреднийЗаработок()
	...
	Иначе	
		...
		Для Каждого КлючИЗначение Из СпособыРасчетаСреднегоЗаработка Цикл
			...
			ЭтотОбъект[КлючИЗначение.Ключ] = УчетСреднегоЗаработкаКлиентСервер.СреднийЗаработокОбщий(
			...
			Если ПолучитьФункциональнуюОпцию("ИндексироватьСреднийЗаработокЧастично") Тогда
				ЧастичнаяИндексация = УчетСреднегоЗаработкаКлиентСервер.ДанныеЧастичнойИндексацииСреднегоЗаработка(
					ЭтотОбъект.СреднийЗаработокОбщий, ЭтотОбъект.ОтработанноеВремяДляСреднегоОбщий, ДополнительныеПараметры, ЭтотОбъект[КлючИЗначение.Ключ]);			
				УчетСреднегоЗаработка.ЗаполнитьДанныеЧастичнойИндексацииОтпусков(ЭтотОбъект, ЧастичнаяИндексация, КлючИЗначение.Ключ);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
