...
Функция ОписаниеРасчетногоДокумента(ДокументСсылка, Номер, Дата, ПредставлениеДокумента) Экспорт
	...
	ИначеЕсли ДокументСсылка <> Неопределено И НЕ ДокументСсылка.Пустая() Тогда
		...
		ПредставлениеДокументаДляСверки = "";
		ПредставленияДокументов.Свойство(ИмяДокумента, ПредставлениеДокументаДляСверки);
		ОписаниеДокумента.Вставить("ПредставлениеДокументаДляСверки", ПредставлениеДокументаДляСверки);
		Если НЕ ЗначениеЗаполнено(ПредставлениеДокумента) Тогда
			ПредставлениеДокумента = ПредставлениеДокументаДляСверки;
		КонецЕсли;
			...
			Если СФДокумента.Количество() > 0 Тогда
				...
				Если ЗначениеЗаполнено(СтрокаРасчетов.РасчетныйДокумент) Тогда
					...
					НоваяСтрока.СвязанныеДокументы = СтрШаблон(НСтр("ru = '%1 №%2 от %3'"),
									ОписаниеДокумента.ПредставлениеДокументаДляСверки,
