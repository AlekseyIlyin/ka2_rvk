...
Процедура РассчитатьНаСервере()
	...
	КонецЕсли;
	// Рассчет начислений.
	ПерезаполнитьИРассчитатьНачисленияНаСервере(ВыводитьСообщенияПриРасчете, Кэш);
...
Процедура ОчиститьДанныеСреднегоЗаработка(ОчищатьПараметрыПолученияСреднего = Истина)
	...
	Объект.СреднийПоДаннымОрганизации              = Неопределено;
...
Процедура РассчитатьСреднийЗаработок()
	...
	Объект.СреднийПоДаннымОрганизации     = УчетПособийСоциальногоСтрахования.СреднийДневнойЗаработокФСС(Параметры1);
