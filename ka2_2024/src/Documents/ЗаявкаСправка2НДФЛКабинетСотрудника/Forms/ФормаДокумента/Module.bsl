...
Процедура ОбработатьИзменениеДокументыПоЗаявкеНаСервере()
	...
	ОтветПоЗаявке 	= Объект.ОтветПоЗаявке;
	Комментарий 	= Объект.Комментарий;
	ТекущаяМодифицированность = Модифицированность;
	
	ДокументОбъект = Объект.Ссылка.ПолучитьОбъект();
	ДокументОбъект.ДокументыПоЗаявке.Очистить();
	Для каждого СтрокаТЧ Из Объект.ДокументыПоЗаявке Цикл
		ЗаполнитьЗначенияСвойств(ДокументОбъект.ДокументыПоЗаявке.Добавить(), СтрокаТЧ);
	КонецЦикла;
	ДокументОбъект.Записать();
	Прочитать();
	ВывестиДокументыИзТаблицыДокументовЗаявкиНаСервере();
	
	Объект.ОтветПоЗаявке = ОтветПоЗаявке;
