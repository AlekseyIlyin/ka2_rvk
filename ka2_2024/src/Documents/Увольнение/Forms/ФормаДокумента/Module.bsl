...
Процедура ЗаполнитьКомпенсацииЗаОтпуск(ТаблицаНачислений, МенеджерРасчета)
	...
	Для Каждого СтрокаОтпуска Из НайденныеСтроки Цикл
		...
		КонецЕсли;
		МенеджерРасчета.ДобавитьИзвестноеЗначениеПоказателя(НовыйИнтервал, ПоказательУчитыватьМРОТ, ЗначениеПоказателяУчитыватьМРОТ);
		ПоКалендарнымДням = ОстаткиОтпусков.ОписаниеВидаОтпуска(СтрокаОтпуска.ВидОтпуска, ОтпускВРабочихДняхПоДоговору).СпособРасчетаПоКалендарнымДням;
		Если Не Объект.РасчетДенежногоСодержания И ПолучитьФункциональнуюОпцию("ИндексироватьСреднийЗаработокЧастично") Тогда
			УчетСреднегоЗаработка.ДобавитьЗначенияПоказателейЧастичнойИндексацииОтпусков(Объект, МенеджерРасчета, НовыйИнтервал, ПоКалендарнымДням);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого КлючИЗначение Из СпособыРасчетаСреднегоЗаработка Цикл
		...
		Если ПолучитьФункциональнуюОпцию("ИндексироватьСреднийЗаработокЧастично") Тогда
			ЧастичнаяИндексация = УчетСреднегоЗаработкаКлиентСервер.ДанныеЧастичнойИндексацииСреднегоЗаработка(
				Объект.СреднийЗаработокОбщий, Объект.ОтработанноеВремяДляСреднегоОбщий, ДополнительныеПараметры, Объект[КлючИЗначение.Ключ]);			
			УчетСреднегоЗаработка.ЗаполнитьДанныеЧастичнойИндексацииОтпусков(Объект, ЧастичнаяИндексация, КлючИЗначение.Ключ);
		КонецЕсли;
	КонецЦикла;
