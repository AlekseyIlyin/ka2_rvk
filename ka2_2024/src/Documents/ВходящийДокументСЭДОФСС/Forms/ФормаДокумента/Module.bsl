...
Функция ПолучитьФайлПриложения()
	...
	// Поиск печатной формы во вложениях, если есть печатная форма то возвращается ссылка на этот файл.
	Если Объект.ФайлыПриложений.Количество() > 0 Тогда
		ВидФайла = ПрефиксИмениФайлаПечатнойФормы() + Объект.ВидДокументаФСС;
		Для каждого ДанныеФайла из Объект.ФайлыПриложений Цикл
			ВидФайлаПриложения = СтрЗаменить(ДанныеФайла.ВидФайла, ПрефиксИмениФайлаПечатнойФормы(), "");
