...
Процедура ТекстЗапросаТаблицаВтВыделениеАмортизации(ТекстыЗапроса)
	...
	ТекстРаспределенияСуммы = ОбщегоНазначенияУТ.ТекстРаспределенияСуммы(
		...
		|					ТОГДА ЕСТЬNULL(АмортизацияОС.АмортизацияПР, 0) * ВтСуммаКоэффициентов.СуммаКоэффициентовПР / (ЕСТЬNULL(СтоимостьОС.СтоимостьПР, 0) + ЕСТЬNULL(СтоимостьОС.СтоимостьПРЦФ, 0))
		|				ИНАЧЕ 0 
		|		КОНЕЦ
