...
Процедура ЗагрузитьСообщение121(Страхователь, РегистрацияИдентификатор, ТекстXML, Результат, Кэш) Экспорт
	...
	Иначе
		Запись = Набор[0];
		Если ЗначениеЗаполнено(Запись.РегистрацияСтатус) И РегистрацияИдентификатор <> Запись.РегистрацияИдентификатор Тогда
			СуществующийСтатус = ВозможныеСтатусы.Найти(Запись.РегистрацияСтатус, "Статус");
			Если СуществующийСтатус <> Неопределено Тогда
				Если СведенияОСтатусе = Неопределено Тогда
					ОтменитьЗаписьНабора(Набор);
					Возврат; // Неизвестный новый статус.
				ИначеЕсли СведенияОСтатусе.Вес < СуществующийСтатус.Вес Тогда
					ОтменитьЗаписьНабора(Набор);
