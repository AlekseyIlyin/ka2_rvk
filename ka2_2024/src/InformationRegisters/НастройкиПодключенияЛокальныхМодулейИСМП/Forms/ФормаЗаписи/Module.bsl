		...
		Если Результат.Свойство("Результат") И ТипЗнч(Результат.Результат) = Тип("Структура") Тогда
			...
			Иначе
				...
				Если ДополнительныеПараметры.Свойство("ОжидатьПолучениеСтатусаГотовности")
					...
					Если Не ВРег(СтатусСистемы) = СтатусГотово() Тогда
...
Процедура ОбработатьНавигационнуюСсылку(Знач НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ПовторитьНастройкуЛМЧЗ" Тогда
		...
		Если ВРег(СтатусСистемы) = СтатусНеНастроено() Тогда
			ЗапуститьПроцессИнициализацииЛокальногоМодуля();
		ИначеЕсли ВРег(СтатусСистемы) = СтатусИнициализация() Тогда
			ЗапуститьПроцессИнициализацииЛокальногоМодуля();
		ИначеЕсли ВРег(СтатусСистемы) = СтатусОшибкиСинхронизации() Тогда
...
Процедура УправлениеВидимостьюДоступностью(Форма)
	...
	Элементы.НадписьПроверитьСтатус.Видимость = Форма.ЗакончитьПолучениеСтатуса И ВРег(Форма.СтатусСистемы) = СтатусИнициализация();
	
	Если ВРег(Форма.СтатусСистемы) = СтатусНеНастроено() Тогда
		
		Элементы.СтраницыСтатусНастройки.ТекущаяСтраница = Элементы.СтраницаЗапуститьИнициализацию;
		
	ИначеЕсли ВРег(Форма.СтатусСистемы) = СтатусГотово() Тогда
		
		Элементы.СтраницыСтатусНастройки.ТекущаяСтраница = Элементы.СтраницаНастройкаЗавершена;
		
	ИначеЕсли ВРег(Форма.СтатусСистемы) = СтатусОшибкиСинхронизации() Тогда
...
Процедура ОбновитьДанныеПодключенияПоДаннымЛокальногоМодуля()
	...
	Если ВРег(СтатусСистемы) = СтатусГотово() Тогда
	...
	Элементы.ПредставлениеСтатусаСистемы.Заголовок = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
		СтрШаблон(ШаблонСтроки, ПредставлениеСтатусаСистемы));
	
	Элементы.ПредставлениеСтатусаСистемы.Подсказка = ТекстПодсказки;
...
&НаКлиентеНаСервереБезКонтекста
Функция СтатусОшибкиСинхронизации()
	Возврат "SYNC_ERROR";
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция СтатусНеНастроено()
	Возврат "NOT_CONFIGURED";
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция СтатусИнициализация()
	Возврат "INITALIZATION";
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция СтатусГотово()
	Возврат "READY";
КонецФункции
