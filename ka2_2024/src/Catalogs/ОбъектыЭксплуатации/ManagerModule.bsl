...
Функция ОписаниеЗапросаДляВыбора(Параметры, УстановитьВсеОтборы = Ложь, МассивОбъектов = Неопределено) Экспорт
	...
	ИначеЕсли ОтборСписка.Свойство("ТипОС")
		И (ОтборСписка.ТипОС = Перечисления.ТипыОС.ОбъектЭксплуатации
			ИЛИ ОтборСписка.ТипОС = Перечисления.ТипыОС.КапитальноеВложение
			ИЛИ ТипЗнч(ОтборСписка.ТипОС) = Тип("Массив")
				И (ОтборСписка.ТипОС.Найти(Перечисления.ТипыОС.ОбъектЭксплуатации) <> Неопределено
					ИЛИ ОтборСписка.ТипОС.Найти(Перечисления.ТипыОС.КапитальноеВложение) <> Неопределено)) Тогда
		ЭтоВыборОС = Истина;
	КонецЕсли;
	...
	Если ЭтоВыборОС Тогда
		...
		ТекстУсловия = "СправочникОбъектыЭксплуатации.ТипОС В (
