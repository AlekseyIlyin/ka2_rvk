...
Процедура ВыражениеПредставленияНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	ИзменениеВыраженияВРедактореФормул("ВыражениеПредставления");
...
Процедура ВыражениеСортировкиНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	ИзменениеВыраженияВРедактореФормул("ВыражениеСортировки");
...
&НаКлиенте
Процедура ИзменениеВыраженияВРедактореФормул(ИмяВыражения)
	
	ОчиститьСообщения();
	
	ПараметрыРедактированияВыражения = Новый Структура;
	ПараметрыРедактированияВыражения.Вставить("Формула", ЭтотОбъект[ИмяВыражения]);
	ПараметрыРедактированияВыражения.Вставить("ТипРезультата", Новый ОписаниеТипов("Строка"));
	ПараметрыРедактированияВыражения.Вставить("ДеревоОперандов", ПодготовитьДеревоОперандов());
	
	ДополнительныеПараметры = Новый Структура("ИмяВыражения", ИмяВыражения);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ИзменениеВыраженияВРедактореФормулЗавершение", ЭтотОбъект, ДополнительныеПараметры);
...
&НаКлиенте
Процедура ИзменениеВыраженияВРедактореФормулЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		ТекстОшибки = ?(ПустаяСтрока(Результат), "",
			ФинансоваяОтчетностьВызовСервера.ТекстОшибкиВыраженияПредставленияСортировки(Результат, ВидАналитики));
		Если Не ПустаяСтрока(ТекстОшибки) Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстОшибки);
		ИначеЕсли ЭтотОбъект[ИмяВыражения] <> Результат Тогда
			Модифицированность = Истина;
			ЭтотОбъект[ИмяВыражения] = Результат;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Функция ПодготовитьДеревоОперандов()
	
	ДеревоОперандов = РаботаСФормулами.ПолучитьПустоеДеревоОперандов();
	
	ДанныеВидаАналитики = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидАналитики, "Наименование, Идентификатор, ТипЗначения");
	
	КорневаяСтрокаДерева = РаботаСФормулами.НоваяСтрокаДереваОперанда(ДеревоОперандов);
	КорневаяСтрокаДерева.Идентификатор = ДанныеВидаАналитики.Идентификатор;
	КорневаяСтрокаДерева.Представление = ДанныеВидаАналитики.Наименование;
	КорневаяСтрокаДерева.ТипЗначения = ДанныеВидаАналитики.ТипЗначения;
