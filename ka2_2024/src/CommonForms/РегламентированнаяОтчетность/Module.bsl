...
Процедура ПечатьПисьма(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТипЗнч(ПечатаемыйОбъект) = Тип("ДокументСсылка.ОбращениеСФРКСтрахователю") Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Печать обращения не предусмотрена'"));
			Возврат;
		Иначе
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТекущиеДанные.НеОтправляетсяВКонтролирующийОрган
			...
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Снятие с учета иностранного лица в ФНС"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Снятие с учета иностранно") Тогда
...
Процедура ПечатьРегламентированногоОтчетаШтрих(Ссылка, Наименование)
	...
	Если Наименование = "Акцизы на алкогольную продукцию"
		И ОписаниеРегОтчета(Ссылка).ВыбраннаяФорма = "ФормаОтчета2025Кв1" Тогда
		ПоказатьПредупреждение(, НСтр(
	...
	Если Наименование = "Расчет по страховым взносам"
		И ОписаниеРегОтчета(Ссылка).ВыбраннаяФорма = "ФормаОтчета2025Кв1" Тогда
		ПоказатьПредупреждение(, НСтр(
		"ru='Печать со штрихкодом PDF417 формы
		|отчета в редакции приказа ФНС Росссии
		|от 13.09.2024 № ЕД-7-11/739@ будет реализована
		|в будущих версиях программы'"));
		Возврат;
	КонецЕсли;
