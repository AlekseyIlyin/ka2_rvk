...
Процедура РассчитатьСреднийЗаработок(Форма)
	...
	Если Форма.СпособыРасчета = Неопределено Тогда
		...
		Если Форма.ИндексироватьСреднийЗаработокЧастично Тогда 
			ЧастичнаяИндексация = УчетСреднегоЗаработкаКлиентСервер.ДанныеЧастичнойИндексацииСреднегоЗаработка(
				ДанныеНачислений, ДанныеВремени, ДополнительныеПараметры, Форма.СреднийЗаработокИтог);
			Форма.СреднийЗаработокИндексируемыйИтог = ЧастичнаяИндексация.ИндексируемыйЗаработок;
			Форма.СреднийЗаработокНеиндексируемыйИтог = ЧастичнаяИндексация.НеиндексируемыйЗаработок;
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Для Каждого СпособРасчета Из Форма.СпособыРасчета Цикл
		...
		Если Форма.ИндексироватьСреднийЗаработокЧастично Тогда 
			ЧастичнаяИндексация = УчетСреднегоЗаработкаКлиентСервер.ДанныеЧастичнойИндексацииСреднегоЗаработка(
				ДанныеНачислений, ДанныеВремени, ДополнительныеПараметры, Форма[ИмяРеквизита]);
			Форма[ИмяРеквизита + "Индексируемый"] = ЧастичнаяИндексация.ИндексируемыйЗаработок;
			Форма[ИмяРеквизита + "Неиндексируемый"] = ЧастичнаяИндексация.НеиндексируемыйЗаработок;
		КонецЕсли;
	КонецЦикла;
