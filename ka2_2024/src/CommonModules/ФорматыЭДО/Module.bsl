	...
	ИначеЕсли Формат = ПоддерживаемыеФорматы.ФНС.УПД_5_02.ИнформацияПродавца
		...
		Если ЭлектронныеДокументыЭДО.ТребуетсяМашиночитаемаяДоверенность(Организация, СертификатПодписи) Тогда
			...
			Если ЗначениеЗаполнено(ВыбраннаяМЧД) Тогда
				// В соответствии с доверенностью в электронной форме в машиночитаемом виде, если представление доверенности
				// осуществляется посредством включения в каждый пакет электронных документов, подписываемых представителем.
				РаботаСФайламиБЭД.ЗаполнитьСвойствоXDTO(Подписант, "СпосПодтПолном", "2", Истина, Ошибки);
...
Функция СведенияОПодписанте(ОписаниеФайла, ОписаниеОшибки = "") Экспорт
	...
	ИначеЕсли ЕстьСвойствоXDTO(ДанныеФайлаЭД, "ИдФайл")
		...
		// Для формата УПД 5.02 минимальное количество элементов по разделителю "_" равно 12.
		// см. ФорматыЭДО_ФНС.ИдентификаторФайла_УПД_5_02_ИнформацияПродавца.
		Если ЭлементыИдентификатора.Количество() >= 12 Тогда
