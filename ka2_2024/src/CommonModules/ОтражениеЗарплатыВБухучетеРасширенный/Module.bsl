...
Процедура УдержаниеПередЗаписью(ВидРасчетаОбъект) Экспорт
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ВидРасчетаОбъект.СтратегияОтраженияВУчете) Тогда
			...
			КонецЕсли;
		Иначе
			Если ВидРасчетаОбъект.ТребуетсяРасчетБазы 
				И (ВидРасчетаОбъект.СтратегияОтраженияВУчете <> Перечисления.СтратегииОтраженияВУчетеНачисленийУдержаний.ПоБазовымРасчетам
				Или ВидРасчетаОбъект.СтратегияОтраженияВУчете <> Перечисления.СтратегииОтраженияВУчетеНачисленийУдержаний.ПоФактическимНачислениям) Тогда
				ВидРасчетаОбъект.СтратегияОтраженияВУчете = Перечисления.СтратегииОтраженияВУчетеНачисленийУдержаний.ПоБазовымРасчетам;
			КонецЕсли;
		КонецЕсли;
...
Процедура СформироватьДвиженияБухучетНачисленийСотрудников(РегистраторОбъект, Движения, БухучетНачислений, ФормироватьЗаписиТолькоДляИзменений = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(БухучетНачислений) Тогда
		...
		ПроверяемыеСвойства = "СпособОтраженияЗарплатыВБухучете,ОтношениеКЕНВД";
		Если ПолучитьФункциональнуюОпцию("ИспользоватьСтатьиФинансированияЗарплатаРасширенный") Тогда
...
Функция БухучетДействующихНачислений(Форма, СотрудникиДаты, НеУчитыватьРегистраторы, ОписанияТаблиц)
	...
	Для Каждого ОписаниеТаблицы Из ОписанияТаблиц Цикл
		...
		Если ЗарплатаКадрыРасширенныйКлиентСервер.РедактироватьНачисленияВОтдельныхПолях(1, ОписаниеТаблицы) Тогда
			...
			КонецЦикла;
			ТаблицаСотрудников.Свернуть("Сотрудник,Период,Начисление");
		КонецЕсли;
		
	КонецЦикла;
	
	МенеджерВТ = Новый МенеджерВременныхТаблиц;
	ОписаниеФильтра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра(ТаблицаСотрудников, "Сотрудник,Начисление");
...
// Вызвается из документов, для  последующего редактирования плановых начислений в форме РедактированиеСоставаНачисленийИУдержаний.
//
// Параметры:
...
Функция БухучетНачисленийСотрудников(ТаблицаНачисленийСотрудников, ИсключаемыйРегистратор) Экспорт
	...
	МенеджерВТ = Новый МенеджерВременныхТаблиц;
	ОписаниеФильтра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра(ТаблицаНачисленийСотрудников, "Сотрудник,Начисление");
	СоздатьВТБухучетНачисленийСотрудников(МенеджерВТ, ОписаниеФильтра, ПараметрыПостроения);
...
// Вызвается при редактировании плановых начислений в форме РедактированиеСоставаНачисленийИУдержаний.
//
// Параметры:
