...
Процедура СформироватьЗадания(Документ, МенеджерВременныхТаблиц) Экспорт
	...
	ДвиженияФормируютсяПриЗакрытииМесяца = 
		...
		ИЛИ РасчетСтоимостиВНА.ДвиженияЗаписываютсяПриВыполненииОперации(Документ);
...
Процедура СформироватьДвиженияПоДокументам(Период, ПараметрыВыполнения)
	...
	ПодготовитьВременныеТаблицы(ПараметрыВыполнения);
	ВыбратьДокументыИЗаписатьДвижения(ПараметрыВыполнения);
	
КонецПроцедуры

Процедура ПодготовитьВременныеТаблицы(ПараметрыВыполнения)
	...
	Запрос.УстановитьПараметр("НачалоПериода", НачалоМесяца(ПараметрыВыполнения.КонецПериода));
	Запрос.УстановитьПараметр("КонецПериода", ПараметрыВыполнения.КонецПериода);
	Запрос.УстановитьПараметр("НачалоСледующегоПериода", ПараметрыВыполнения.КонецПериода + 1);
...
Процедура ВыбратьДокументыИЗаписатьДвижения(ПараметрыВыполнения)
...
Функция ЗаданияКРасчету(ПараметрыРасчета, НомерДоРасчета)
	...
	|			И ТекущиеДокументы.Дата <= Задания.ОкончаниеПериода
	|			И ТекущиеДокументы.Проведен
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеОС2_4.ОС КАК ТаблицаОС
	|		ПО ТаблицаОС.Ссылка = ТекущиеДокументы.Ссылка
	...
	|					И ТекущиеДокументы.ДатаПереходаПраваСобственности <= Задания.ОкончаниеПериода)
	|			И ТекущиеДокументы.Проведен
	|			И ТекущиеДокументы.ХозяйственнаяОперация В (
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияВнеоборотныхАктивов),
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияОСсОтложеннымПереходомПрав))
