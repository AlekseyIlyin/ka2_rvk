	...
	Если HTTPОтветЭмуляция = Неопределено Тогда
		...
		Попытка
			...
			Если HTTPМетод = "POST" Тогда
				Прогресс = Соединение.ОтправитьДляОбработкиАсинх(HTTPЗапрос);
			Иначе
				Прогресс = Соединение.ВызватьHTTPМетодАсинх(HTTPМетод, HTTPЗапрос);
			КонецЕсли;
			...
			Исключение
				...
				ТекстОшибки = НСтр("ru = 'Ответ от Локального модуля ""Честный знак"" не получен'");
...
Асинх Функция ОбработатьКодМаркировкиНаЛокальномМодулеЧестногоЗнака(ДанныеШтрихкода, ДанныеУстановленныхЛокальныхМодулей, СтруктураДополнительныхПараметров)
		...
		Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
			...
			Иначе
				...
				Если РезультатОтправкиЗапроса.КодСостояния = 401 Тогда
					...
					ДополнениеТекстаОшибки = СтрШаблон(ШаблонПодсказки, ДанныеЛокальногоМодуля.АдресПодключения);
