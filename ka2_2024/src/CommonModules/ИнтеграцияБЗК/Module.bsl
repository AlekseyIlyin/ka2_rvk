...
Процедура ЗаполнитьКадровыеДанныеСотрудников(Объект, ИмяТЧ, ЗаполняемыеПоля, МассивСтрок = Неопределено) Экспорт
	...
	Для каждого ВидРабот Из Объект.ВидыРабот Цикл
		
		Если НачалоПериода = Неопределено Тогда
			НачалоПериода = ВидРабот.ДатаВыполненияРабот;
			КонецПериода = ВидРабот.ДатаВыполненияРабот;
		КонецЕсли;
		
		НачалоПериода = Мин(НачалоПериода, ВидРабот.ДатаВыполненияРабот);
		КонецПериода = Макс(КонецПериода, ВидРабот.ДатаВыполненияРабот);
	...
	Если ЗаполняемыеПоля.Свойство("ОтработаноЧасов")
		...
		Для Каждого СтрокаТаблицы Из ДанныеСотрудников Цикл
			...
			СтрокаСотрудники.Период = НачалоПериода;
			СтрокаСотрудники.ДатаНачала = НачалоПериода;
			СтрокаСотрудники.ДатаОкончания = КонецДня(КонецПериода);
		КонецЦикла;
		...
		ПараметрыВТ.ДатаНачала = НачалоПериода;
		ПараметрыВТ.ДатаОкончания = КонецДня(КонецПериода);
		ПараметрыВТ.МесяцДатаНачала = НачалоМесяца(Объект.Месяц);
		ПараметрыВТ.МесяцДатаОкончания = КонецМесяца(Объект.Месяц);
...
//
// Параметры:
//   ТипНалога  - СправочникСсылка.ВидыНалоговВзносов - "Типы налогов".
...
Процедура ЗаполнитьТипНалогаОперацииПоЗарплаты(ТипНалога, ВидОперации) Экспорт
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоЗарплате.НДФЛДоначисленныйПоРезультатамПроверки Тогда
		ТипНалога = Справочники.ВидыНалоговВзносов.НДФЛДоначисленныйПоРезультатамПроверки;
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоЗарплате.НДФЛДоходыКонтрагентов Тогда
		ТипНалога = Справочники.ВидыНалоговВзносов.НДФЛДоходыКонтрагентов;
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоЗарплате.НДФЛПередачаЗадолженностиВНалоговыйОрган Тогда
		ТипНалога = Справочники.ВидыНалоговВзносов.НДФЛПередачаЗадолженностиВНалоговыйОрган;
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоЗарплате.НДФЛПрочиеРасчетыСПерсоналом Тогда
		ТипНалога = Справочники.ВидыНалоговВзносов.НДФЛПрочиеРасчетыСПерсоналом;
	...
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоЗарплате.НФДЛДивиденды Тогда
		ТипНалога = Справочники.ВидыНалоговВзносов.НДФЛДивиденды;
	ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПоЗарплате.НФДЛДивидендыСотрудникам Тогда
		ТипНалога = Справочники.ВидыНалоговВзносов.НДФЛДивидендыСотрудникам;
...
// Получает сумму НДФЛ, удержанного (начисленного) за период
// 
// Параметры:
//	Организация			- СправочникСсылка.Организации - ссылка на организацию.
//	Период				- Дата - месяц получения данных.
//	ТринадцатыйПериод	- Булево - признак получения данных за конец декабря.
//	ПереходныйПериод	- Булево - признак перехода на ЕНП.
//
// Возвращаемое значение:
//  ТаблицаЗначений - таблица начисленного НДФЛ.
//
Функция УдержанныйНДФЛЗаПериод(Организация, Период, ТринадцатыйПериод = Ложь, ПереходныйПериод = Ложь) Экспорт
	
	ТаблицаНДФЛ = НоваяТаблицаУдержанногоНДФЛ();
	КБКсПревышения = Справочники.ВидыНалоговВзносов.КБКПоВидуНалоговогоОбязательства(Справочники.ВидыНалоговВзносов.НДФЛСПревышения);
	
	Если Период >= РеглУчетСервер.ДатаНачалаОбязательногоПримененияЕНП() Тогда
		Если ТринадцатыйПериод Тогда // 23-31 декабря
			ДатаНачалаОтбора    = Дата(Год(Период), Месяц(Период), 23);
			ДатаОкончанияОтбора = КонецГода(Период);
	...
	Если ПереходныйПериод Тогда
		// ++todo - последний рабочий день. Требует уточнения в ФНС
		ДатаНачалаОтбора = Дата(Год(НачалоМесяца(Период)-1), 12, 28);
	КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплатыУТ") Тогда
		...
		КодТерритории = Справочники.РегистрацииВНалоговомОргане.КодТерритории(РегистрацияВНалоговомОргане, "ОКТМО");
		...
		Для Каждого СтрокаТаблицы Из УдержаноНалогаЗаПериод Цикл
			...
			Если СтрокаТаблицы.Сумма <> 0 Тогда
				...
				Если СокрЛП(СтрокаТаблицы.КБК) = КБКсПревышения Тогда
					НоваяСтрока.ТипНалога = Справочники.ВидыНалоговВзносов.НДФЛСПревышения;
				Иначе
...
Функция НоваяТаблицаУдержанногоНДФЛ()
	...
	ТаблицаНДФЛ.Колонки.Добавить("Налог",                       Новый ОписаниеТипов("СправочникСсылка.ВидыНалоговВзносов"));
