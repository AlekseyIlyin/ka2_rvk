...
Процедура ЗаполнитьНомераГТДПоУчетнымДанным(ТаблицаТоваров, ВариантПолученияДанных, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеДанные.Вставить("ПересчетСуммы", ПересчетСуммы);
	ДополнительныеДанные.Вставить("ПересчетСуммыНДС", ПересчетСуммыНДС);
	ДополнительныеДанные.Вставить("ПересчетСуммыСНДС", ПересчетСуммыСНДС);
	...
	Если ЕстьЦены Тогда
		...
		ЗаполнитьНомераГТДПоУчетнымДаннымВнутренний(ТоварыКЗаполнению, ТоварыСуммаКЗаполнению,
...
Процедура ЗаполнитьНомераГТДПоУчетнымДаннымВнутренний(ТоварыКЗаполнению, ТоварыСуммаКЗаполнению, НомераГТДКЗаполнению,
	ДополнительныеДанные)
	
	ДействияСоСтрокой = ДополнительныеДанные.ДействияСоСтрокой;
	ПересчетСуммы = ДополнительныеДанные.ПересчетСуммы;
	ПересчетСуммыНДС = ДополнительныеДанные.ПересчетСуммыНДС;
	ПересчетСуммыСНДС = ДополнительныеДанные.ПересчетСуммыСНДС;
	...
	Для Каждого СтрокаТоваровДо Из ТоварыСуммаКЗаполнению Цикл
		...
		Если СтрокаТоваровДо.Количество <= 0 Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Каждого СтрокаТоваров Из СтрокиТоваров Цикл
			...
			СтрокаТоваровДо.Количество = СтрокаТоваровДо.Количество - СтрокаТоваров.Количество;
			ЕстьСумма = ТоварыКЗаполнению.Колонки.Найти("Сумма") <> Неопределено
				И ТоварыСуммаКЗаполнению.Колонки.Найти("Сумма") <> Неопределено;
			Если ЕстьСумма Тогда
				СтрокаТоваровДо.Сумма = СтрокаТоваровДо.Сумма - СтрокаТоваров.Сумма;
			КонецЕсли;
