...
Процедура ПроверитьАктуальностьИнформацииОбОтпуске() Экспорт 
	...
	|		ПО АктуальностьИнформацииОбОтпуске.Сотрудник = ВыгружаемыеОбъекты.Ссылка
	|ГДЕ
...
Процедура ОбновитьАктуальностьИнформацииОбОтпуске(ОбработанныеСотрудники, АктуальностьИнформацииОбОтпуске) Экспорт
	...
	ТаблицаАктуальности = АктуальностьИнформацииОбОтпуске.СкопироватьКолонки();
	Для каждого СтрокаТЗ Из АктуальностьИнформацииОбОтпуске Цикл
		Если ОбработанныеСотрудники[СтрокаТЗ.Сотрудник] = Неопределено Тогда
			Продолжить;
		КонецЕсли;
	...
	Попытка
		...
		Для каждого СтрокаТЗ Из ТаблицаАктуальности Цикл
			МенеджерЗаписи = РегистрыСведений.АктуальностьИнформацииОбОтпуске.СоздатьМенеджерЗаписи();
			ЗаполнитьЗначенияСвойств(МенеджерЗаписи, СтрокаТЗ);
			МенеджерЗаписи.Записать();
		КонецЦикла;
