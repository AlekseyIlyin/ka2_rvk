...
Функция НастройкаПолученияФактаПоДокументуНаДату(ВидДокумента, ПравилаПолученияФакта)
	...
	КолонкиТипыДанныхУчетаПравил = "ТипДанныхУчетаПриход, ТипДанныхУчетаРасход";
	...
	Для Каждого ПриходРасход Из ТипыДанныхУчетаПравил Цикл
		...
		СКД = ИсточникиДанныхСервер.СхемаКомпоновкиДанныхПоТаблицеКорреспонденцийРегистров(ШаблонИсточникаФакта);
		ПриходРасход.НастройкиСКД = БюджетированиеСервер.ИнициализированныеНастройкиПоУмолчанию(СКД);
	КонецЦикла;
	...
	ОписанияПравил = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(ПравилаПолученияФакта.ВыгрузитьКолонку("Правило"),
		СтрСоединить(РеквизитыПравил, ", ") + ", СтатьяБюджетов, ИсточникСуммыОперации, КомпоновщикНастроек");
...
Функция ПараметрыПолученияФактаДляКонтроляЛимитов(РеквизитыДокумента, ДействующиеЛимиты, ОтборПоЗаявке, ДополнительныйОтборРасшифровкиОтчета = Неопределено)
	...
	КонецЕсли;
	Если ОтборПоЗаявке Тогда
		ПараметрыПолученияФакта.Вставить("ОтборПоРегистраторуДляВсегоФакта", РеквизитыДокумента.Документ);
	КонецЕсли;
	КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(
		ОбщиеПараметрыИсточникаДанных.ДополнительныйОтбор,
		"Сценарий",
...
Функция НастройкиЛимитовНаДату(Параметры) Экспорт
	...
	Если Не ПоРегиструЛимитов Тогда
		...
		ТекстЗапроса = ТекстыЗапросов[ТекстыЗапросов.Количество()-1];
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ДействующиеПравилаПолученияФакта",
			"ДействующиеПравилаПолученияФактаОперативныйУчет");
		ТекстыЗапросов[ТекстыЗапросов.Количество()-1] = ТекстЗапроса;
		...
		Если Не Результат.Пустой() Тогда
			...
			Для Каждого СтрокаПравила Из ИсходнаяТаблицаПравилСПроизвольнымиДанными Цикл
				ТипыДанныхУчетаПравила = ИсточникиДанныхПовтИсп.ТипыДанныхУчетаПравилаСПроизвольнымиДанными(
