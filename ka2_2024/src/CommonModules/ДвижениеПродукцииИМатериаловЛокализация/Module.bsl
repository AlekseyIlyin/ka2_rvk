...
Функция ПолучитьЗапросПолученияДанныхДляПечатнойФормыМ11()
	...
	Запрос.Текст =
		...
		|	Товары.Назначение КАК НазначениеПолучателя
		|ПОМЕСТИТЬ Товары
		|ИЗ
		|	Документ.ДвижениеПродукцииИМатериалов.Товары КАК Товары
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыНоменклатуры КАК ВидыНоменклатуры
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Назначения КАК ПолучателиНазначения
		|		ПО Товары.НазначениеПолучателя = ПолучателиНазначения.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НаправленияДеятельности КАК НДПолучатель
		|		ПО ПолучателиНазначения.НаправлениеДеятельности = НДПолучатель.Ссылка
		|УПОРЯДОЧИТЬ ПО
		|	Ссылка,
		|	НомерСтроки
		|ИТОГИ
