...
Процедура УвеличитьСтоимостьИАмортизациюПриРазукомплектацииИОбъединении(ДанныеСтроки, СтрокаИсходноеОС) Экспорт
	
	//++ Локализация
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДанныеСтроки, "ОстаточнаяСтоимостьПР") Тогда
		ДанныеСтроки.ОстаточнаяСтоимостьПР = СтрокаИсходноеОС.СтоимостьСписанияПР;
		ДанныеСтроки.СтоимостьДляВычисленияПР = СтрокаИсходноеОС.СтоимостьСписанияПР;
	КонецЕсли;
	//-- Локализация
...
Процедура ТекстЗапросаДоначислениеАмортизацииБезИзмененийБУ(СписокЗапросов, ПараметрыРасчета)
	...
	|	&СуммаАмортизацииПР КАК СуммаПР
	|
	...
	ТекстСуммаАмортизацииПР = ТекстЗапросаСуммаАмортизацииПР(Ложь);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&СуммаАмортизацииПР", ТекстСуммаАмортизацииПР);
...
Процедура ТекстЗапросаРасчетАмортизацииБезИзмененийБУ(СписокЗапросов, ПараметрыРасчета)
	...
	|
	|	ВЫБОР 
	|		КОГДА (СписокОС.ОстаточнаяСтоимостьПР + СписокОС.ОстаточнаяСтоимостьПРЦФ - СписокОС.СуммаПР) < &МинимальнаяОстаточнаяСтоимостьРегл
	|				И (СписокОС.ОстаточнаяСтоимостьПР + СписокОС.ОстаточнаяСтоимостьПРЦФ) > 0
	|				И СписокОС.СуммаПР > 0
	|
	...
	ТекстСуммаАмортизацииПР = ТекстЗапросаСуммаАмортизацииПР(Истина);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&СуммаАмортизацииПР", ТекстСуммаАмортизацииПР);
...
Процедура ОбъединитьРасчетАмортизацииБУНУ(ПараметрыРасчета)
	...
	|				ТОГДА 0
	|			ИНАЧЕ СУММА(СписокОС.СуммаПР - СписокОС.СуммаПРЦФ) 
	|		КОНЕЦ КАК СуммаПР,
	|
	...
	|				ТОГДА 0
	|			ИНАЧЕ СУММА(СписокОС.СуммаПРЦФ) 
	|		КОНЕЦ КАК СуммаПРЦФ,
	|
	|		СписокОС.ПередаватьРасходыВДругуюОрганизацию  КАК ПередаватьРасходыВДругуюОрганизацию,
	|		СписокОС.ОрганизацияПолучательРасходов        КАК ОрганизацияПолучательРасходов,
	...
	|				КОГДА РасчетАмортизации.АмортизацияНачисляетсяПоСтандартамМУ
	|
	|					ТОГДА ВЫРАЗИТЬ(РасчетАмортизации.СуммаПР * РасчетАмортизации.КоэффициентЦФПР КАК ЧИСЛО(31,2)) 
	|
	|				КОГДА РасчетАмортизации.СтоимостьДляВычисленияПР <> 0
	|
	|					ТОГДА ВЫРАЗИТЬ(ВЫБОР
	|							КОГДА &СуммаПР_1 > РасчетАмортизации.ОстаточнаяСтоимостьПРЦФ
	|										И РасчетАмортизации.ОстаточнаяСтоимостьПРЦФ >= 0
	|									ИЛИ &СуммаПР_1 < РасчетАмортизации.ОстаточнаяСтоимостьПРЦФ
	|										И РасчетАмортизации.ОстаточнаяСтоимостьПРЦФ <= 0
	|								ТОГДА РасчетАмортизации.ОстаточнаяСтоимостьПРЦФ
	|
	|							ИНАЧЕ &СуммаПР_1
	|						КОНЕЦ * РасчетАмортизации.КоэффициентЦФПР КАК ЧИСЛО(31,2))
	|
	|				ИНАЧЕ 0
	|
	|			КОНЕЦ КАК СуммаПРЦФ,
	...
	|			ВЫРАЗИТЬ(РасчетАмортизации.СуммаПР * РасчетАмортизации.КоэффициентЦФПР КАК ЧИСЛО(31,2)) КАК СуммаПРЦФ,
	|
	|			0 КАК СуммаНУ,
	|			
	|			РасчетАмортизации.КоэффициентЦФБУ КАК КоэффициентЦФБУ,
...
Функция ТекстЗапросаСуммаАмортизацииБУ(ПараметрыТекстаЗапроса) Экспорт
	...
	|
	|		ТОГДА СписокОС.ОстаточнаяСтоимостьРегл - &СуммаДоначисления - СписокОС.ЛиквидационнаяСтоимость - &ОстатокОбесценения
	|
	|	КОГДА НЕ СписокОС.АмортизацияНачисляетсяПоСтандартамМУ
...
Функция ТекстЗапросаСуммаАмортизацииПР(ЕстьСуммаДоначисления)
	...
	| 		ТОГДА 0
	|
	|	КОГДА ((СписокОС.ОстаточнаяСтоимостьПР + СписокОС.ОстаточнаяСтоимостьПРЦФ - &СуммаДоначисления) = 0 
	|				ИЛИ СписокОС.РесурсВыработанныйЗаТекущийПериод <= 0)
	|
	|		ТОГДА 0
	|
	...
	|
	|		ТОГДА СписокОС.ОстаточнаяСтоимостьПР + СписокОС.ОстаточнаяСтоимостьПРЦФ - &СуммаДоначисления
	|
		// Линейный, ПропорциональноОбъемуПродукции
	...
	|					ЗНАЧЕНИЕ(Перечисление.СпособыНачисленияАмортизацииОС.ПропорциональноОбъемуПродукции)) 
	|
	|		ТОГДА ВЫРАЗИТЬ(ВЫРАЗИТЬ(СписокОС.СтоимостьДляВычисленияАмортизацииПР - &СуммаДоначисления КАК ЧИСЛО(20,8))
	|								* ВЫРАЗИТЬ(СписокОС.РесурсВыработанныйЗаТекущийПериод КАК ЧИСЛО(20,8)) 
	|								/ ВЫРАЗИТЬ(СписокОС.РесурсНазначенный - СписокОС.РесурсВыработанныйЗаПрошлыйПериод КАК ЧИСЛО(20,8)) КАК ЧИСЛО(31,2))
	|
		// УменьшаемогоОстатка (последний год)
	...
	|			И (СписокОС.РесурсНазначенный - СписокОС.РесурсВыработанныйЗаПрошлыйПериод - СписокОС.РесурсВыработанныйЗаТекущийПериод) <= 12 
	|
	|		ТОГДА ВЫРАЗИТЬ((СписокОС.СтоимостьДляВычисленияАмортизацииПР - &СуммаДоначисления)
	|								* ВЫРАЗИТЬ(СписокОС.РесурсВыработанныйЗаТекущийПериод КАК ЧИСЛО(20,8)) 
	|								/ ВЫРАЗИТЬ(СписокОС.РесурсНазначенный - СписокОС.РесурсВыработанныйЗаПрошлыйПериод КАК ЧИСЛО(20,8)) КАК ЧИСЛО(31,2))
	|
	|
	...
	ТекстСуммаДоначисления = ?(ЕстьСуммаДоначисления, "ЕСТЬNULL(Доначисление.СуммаПР, 0)", "0");
...
Процедура ЗаменитьПараметрыТекстаЗапросаДляРасчетаАмортизацииБУ(ТекстЗапроса)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&СуммаАмортизацииПР", ТекстЗапросаСуммаАмортизацииПР(Ложь));
