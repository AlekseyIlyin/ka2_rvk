...
//								   по дате начичисления мат. выгоды (вместо даты выдачи или даты изменения условий займа)
...
//		                               исчисления налога.
//
Процедура СформироватьСоциальныеВычетыПоУдержаниям(Регистратор, Движения, Отказ, Организация, ДатаОперации, МесяцНалоговогоПериола, Удержания, Записывать = Ложь, ОкончательныйРасчет = Истина) Экспорт
	...
	УчетНДФЛБазовый.СформироватьСоциальныеВычетыПоУдержаниям(Регистратор, Движения, Отказ, Организация, ДатаОперации, МесяцНалоговогоПериола, Запрос.Выполнить().Выгрузить(), Записывать, ОкончательныйРасчет);
...
Функция ИсчисленныйНалогСДоходами(ИсчисленныйНалог, НачисленныеДоходы, ДатаПолученияДохода) Экспорт
	...
	КонецЦикла;
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "0 > 0", "(" + ТекстНалогов + ")");
	ТекстНалогов = "";
	Для каждого ИмяРесурса Из ИменаРесурсовНалога Цикл
