...
Процедура РасчетПоказателей_6НДФЛ_2024Кв2(НачалоПериода, ОкончаниеПериода, ДатаПодписи, Организация, РегистрацияВНалоговомОргане, ОписаниеТаблиц, ПодробныеДанные, НомерКорректировки)
	...
	Если ОписаниеТаблиц.Свойство("Раздел1НалогиВозвраты") Тогда
		...
		|			И НалогиПоДатам.КБК = ОтборНеотрицательныхУдержаний.КБК
		|			И НалогиПоДатам.КрайнийСрокУплаты = ОтборНеотрицательныхУдержаний.КрайнийСрокУплаты
		|ГДЕ
		|	НалогиПоДатам.ВариантУдержания = ЗНАЧЕНИЕ(Перечисление.ВариантыУдержанияНДФЛ.Удержано)
		|
	...
	Если ОписаниеТаблиц.Свойство("Раздел2Доходы") Или ОписаниеТаблиц.Свойство("Раздел2Налоги2024") Или ОписаниеТаблиц.Свойство("КоличествоПолучателейДохода") Или ОписаниеТаблиц.Свойство("Раздел2Вычеты") Или ОписаниеТаблиц.Свойство("Приложение1Налогоплательщики") Тогда
		...
		Запрос.УстановитьПараметр("ТаблицаПределовДляНалоговыхБаз", ТаблицаПределовДляНалоговыхБаз()); 
		...
		Если Запрос.Выполнить().Выгрузить()[0].Количество > 0 Тогда
			...
			ПараметрыСтрокиПревышения = Новый Структура("Предел,СтавкаДо,СтавкаНалогаДо,СтавкаПосле,СтавкаНалогаПосле", 0, 0);
			Для каждого СтрокаТаблицы Из ТаблицаРасчетаНарастающимИтогом Цикл
				Если ТекущееФизлицо <> СтрокаТаблицы.ФизическоеЛицо Или ТекущаяНалоговаяБаза <> СтрокаТаблицы.НалоговаяБаза Или ТекущийПериод <> СтрокаТаблицы.Период Тогда
					Если ОблагаемыйДоходДоПредела <> 0 Тогда
						ВыделитьСуммыПревышенияПредела(ОблагаемыйДоходДоПредела, ПараметрыСтрокиПревышения, СтрокиРаспределения, ДоходыСУточненнымиСтавками, Истина, Истина)
					КонецЕсли;
					...
					Если СоответствиеПараметровНалоговыхБаз[СтрокаТаблицы.НалоговаяБаза].КоличествоПределов = 1 Тогда
						...
						ПараметрыСтрокиПревышения.Предел = СоответствиеПараметровНалоговыхБаз[ТекущаяНалоговаяБаза].Предел1
					Иначе
						...
						ИначеЕсли СтрокаТаблицы.ЭтоДеньПревышенияПредела2 Тогда
							...
							ПараметрыСтрокиПревышения.Предел = СоответствиеПараметровНалоговыхБаз[ТекущаяНалоговаяБаза].Предел2
						ИначеЕсли СтрокаТаблицы.ЭтоДеньПревышенияПредела3 Тогда
							...
							ПараметрыСтрокиПревышения.Предел = СоответствиеПараметровНалоговыхБаз[ТекущаяНалоговаяБаза].Предел3
						Иначе
							ПараметрыСтрокиПревышения.СтавкаНалогаДо = 20; 
							ПараметрыСтрокиПревышения.СтавкаДо = Перечисления.НДФЛСтавки.Ставка22;
							ПараметрыСтрокиПревышения.СтавкаНалогаПосле = 22; 
							ПараметрыСтрокиПревышения.СтавкаПосле = Перечисления.НДФЛСтавки.Ставка22;
							ПараметрыСтрокиПревышения.Предел = СоответствиеПараметровНалоговыхБаз[ТекущаяНалоговаяБаза].Предел4
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
				Если ОблагаемыйДоходДоПредела = 0 Тогда
		...
		Если ОписаниеТаблиц.Свойство("Раздел2Вычеты") Или ОписаниеТаблиц.Свойство("Приложение1Налогоплательщики") Тогда
				...
				|	МИНИМУМ(ДоходыПоСтавкам.Ставка) КАК Ставка,
...
Функция ДанныеОДоходахНалогахВычетах(Сотрудники, НалоговыйПериод, Организация, РегистрацияВНалоговомОргане, ДатаАктуальности, КППОтправки, ВРазрезеРегистраторов, ДополнительныйОтбор) Экспорт
	...
	Иначе
		...
		Если Запрос.Выполнить().Выгрузить()[0].Количество > 0 Тогда
			...
			СтрокиРаспределения = Новый Массив;
			ОблагаемыйДоходДоПредела = 0; 
			ПараметрыСтрокиПревышения = Новый Структура("Предел,СтавкаДо,СтавкаПосле", 0, 0);
			Для каждого СтрокаТаблицы Из ТаблицаРасчетаНарастающимИтогом Цикл
				Если ТекущееФизлицо <> СтрокаТаблицы.ФизическоеЛицо Или ТекущаяНалоговаяБаза <> СтрокаТаблицы.НалоговаяБаза Или ТекущийПериод <> СтрокаТаблицы.ДатаПолученияДохода Тогда
					Если ОблагаемыйДоходДоПредела <> 0 Тогда
						ВыделитьСуммыПревышенияПредела(ОблагаемыйДоходДоПредела, ПараметрыСтрокиПревышения, СтрокиРаспределения, ДоходыСУточненнымиСтавками, Ложь, Ложь)				
					КонецЕсли;
					СтрокиРаспределения.Очистить();
					ОблагаемыйДоходДоПредела = 0;
					ТекущееФизлицо = СтрокаТаблицы.ФизическоеЛицо;
					ТекущаяНалоговаяБаза = СтрокаТаблицы.НалоговаяБаза;
					ТекущийПериод = СтрокаТаблицы.ДатаПолученияДохода;
					Если СоответствиеПараметровНалоговыхБаз[СтрокаТаблицы.НалоговаяБаза].КоличествоПределов = 1 Тогда
						ПараметрыСтрокиПревышения.СтавкаДо = Перечисления.НДФЛСтавки.Ставка13;
						ПараметрыСтрокиПревышения.СтавкаПосле = Перечисления.НДФЛСтавки.Ставка15;
						ПараметрыСтрокиПревышения.Предел = СоответствиеПараметровНалоговыхБаз[ТекущаяНалоговаяБаза].Предел1
		...
		|	ДоходыПоСтавкам.НалоговаяБаза КАК НалоговаяБаза,
		|	ДоходыПоСтавкам.МесяцНалоговогоПериода КАК МесяцНалоговогоПериода,
		|	МИНИМУМ(ДоходыПоСтавкам.СтавкаНДФЛ) КАК Ставка
...
Процедура ВыделитьСуммыПревышенияПредела(Знач ОблагаемыйДоходДоПредела, ПараметрыДляЗаполнения, СтрокиРаспределения, СтрокиДоходовПревышающихПредел, ЗаполнятьСтавкуЧислом, ЗаполнятьКБК) Экспорт 
...
Функция ТаблицаПределовДляНалоговыхБаз()
	
	ПараметрыРасчетаНДФЛПоПрогрессивнойШкале = УчетНДФЛ.ПараметрыРасчетаНДФЛПоПрогрессивнойШкалеС2025Года();
	СоответствиеПараметровНалоговыхБаз = ПараметрыРасчетаНДФЛПоПрогрессивнойШкале.СоответствиеПараметровНалоговыхБаз;
	ТаблицаПределовДляНалоговыхБаз = Новый ТаблицаЗначений;
	ТаблицаПределовДляНалоговыхБаз.Колонки.Добавить("НалоговаяБаза", Новый ОписаниеТипов("ПеречислениеСсылка.КатегорииДоходовНДФЛ"));
	ТаблицаПределовДляНалоговыхБаз.Колонки.Добавить("Предел1", Новый ОписаниеТипов("Число"));
	ТаблицаПределовДляНалоговыхБаз.Колонки.Добавить("Предел2", Новый ОписаниеТипов("Число"));
	ТаблицаПределовДляНалоговыхБаз.Колонки.Добавить("Предел3", Новый ОписаниеТипов("Число"));
	ТаблицаПределовДляНалоговыхБаз.Колонки.Добавить("Предел4", Новый ОписаниеТипов("Число"));
	Для каждого КлючЗначение Из СоответствиеПараметровНалоговыхБаз Цикл
		НоваяСтрока = ТаблицаПределовДляНалоговыхБаз.Добавить();
		НоваяСтрока.НалоговаяБаза = КлючЗначение.Ключ;
		НоваяСтрока.Предел1 = КлючЗначение.Значение.Предел1;
		НоваяСтрока.Предел2 = КлючЗначение.Значение.Предел2;
		НоваяСтрока.Предел3 = КлючЗначение.Значение.Предел3;
		НоваяСтрока.Предел4 = КлючЗначение.Значение.Предел4;
	КонецЦикла;
	
	Возврат ТаблицаПределовДляНалоговыхБаз
