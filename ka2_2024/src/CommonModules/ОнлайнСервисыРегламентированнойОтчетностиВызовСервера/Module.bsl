...
Функция СформироватьПрокси(НастройкиПрокси, Протокол)
	...
	Прокси.Установить(Протокол,
		...
		СтроковыеФункцииКлиентСервер.СтрокаВЧисло(НастройкиПрокси["Порт"]),
...
Функция СобытиеНаступило(Знач ИмяСобытия, Знач НенайденноеСчитатьНаступившим = Ложь) Экспорт
	...
	ИначеЕсли ТипЗнч(НенайденноеСчитатьНаступившим) = Тип("Строка") Тогда
		Если ХранилищеОбщихНастроек.Загрузить(НенайденноеСчитатьНаступившим) = Истина Тогда
			Возврат Истина;
		Иначе
