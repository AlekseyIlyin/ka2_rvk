...
Функция ЗаполнитьПоФактическойОтгрузке(ОбъектДокумента, СтруктураДокументаОтгрузки) Экспорт
	...
	Если СтруктураДокументаДляЗаполнения.ЗаказЯвляетсяРаспоряжением Тогда
		...
		//++ НЕ УТ
		МассивТиповЗаказов.Добавить(Тип("ДокументСсылка.ЗаказПереработчику2_5"));
...
Функция ТекстЗапросаПоФактическойОтгрузке(СтруктураДокументаДляЗаполнения, СтруктураРезультатаЗапроса = Неопределено)
	...
	|	ЗаказТовары.Упаковка                           КАК Упаковка,
	|	ЗаказТовары.Серия                              КАК Серия,
	|	ЗаказТовары.Ссылка.ВидЦены                     КАК ВидЦены,
	|	ЗаказТовары.Количество                         КАК КоличествоВЗаказе,
	|	ЗаказТовары.Цена                               КАК Цена,
	|	ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)    КАК СтавкаНДС,
	|	ЗаказТовары.Сумма                              КАК Сумма,
	|	0                                              КАК СуммаНДС,
	|	ЗаказТовары.Сумма                              КАК СуммаСНДС,
	...
	|	ВтТоварыКОформлению КАК ТаблицаОстатки
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказПереработчику2_5.ОбеспечениеМатериаламиИРаботами КАК ЗаказТовары
	|	ПО ТаблицаОстатки.Номенклатура = ЗаказТовары.Номенклатура
	|	И ТаблицаОстатки.Характеристика = ЗаказТовары.Характеристика
	|	И ТаблицаОстатки.Серия = ЗаказТовары.Серия
	|	И ТаблицаОстатки.КодСтроки = ЗаказТовары.КодСтроки
...
Функция ДополнительныеТаблицыЗаказов(МассивЗаказовКлиентов, Валюта)
	...
	|	КОНЕЦ КАК ПересчитатьВВалютуДокумента
	|ИЗ
	|	Документ.ЗаказПереработчику2_5 КАК ТаблицаЗаказов
	|ГДЕ
	|	ТаблицаЗаказов.Ссылка В(&МассивЗаказовКлиентов)
	//-- НЕ УТ
	|;
...
Процедура ДополнитьСтруктуруРаспоряжений(СоответствиеРаспоряжений, ПараметрыФормирования)
	...
	//++ НЕ УТ
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказПереработчику2_5) Тогда
		ТекстыЗапроса.Добавить(
			"ВЫБРАТЬ
			|	ЗаказКлиента.Ссылка КАК Ссылка,
			|	ЗаказКлиента.Проведен КАК Проведен,
			|	ЗаказКлиента.Касса КАК Касса,
			|	ЗаказКлиента.Организация КАК Организация,
			|	ЗаказКлиента.СкладМатериалов КАК Склад
			|ИЗ
			|	Документ.ЗаказПереработчику2_5 КАК ЗаказКлиента
			|ГДЕ
			|	ЗаказКлиента.Ссылка В(&СписокДокументов)
			|");
	КонецЕсли;
...
Функция ТекстЗапросаПоЗначащимРеквизитамРаспоряжений()
	...
	ТекстыРаспоряжениеНаОформление.Добавить(
		...
		|			КОГДА ТИПЗНАЧЕНИЯ(РаспоряженияНаОтгрузкуОбороты.Распоряжение) = ТИП(Документ.ЗаказПереработчику2_5)
		|				ТОГДА ВЫРАЗИТЬ(РаспоряженияНаОтгрузкуОбороты.Распоряжение КАК Документ.ЗаказПереработчику2_5).СкладМатериалов
		//-- НЕ УТ
		|			КОГДА ИСТИНА
		...
		|			КОГДА ТИПЗНАЧЕНИЯ(РаспоряженияНаОтгрузкуОбороты.Распоряжение) = ТИП(Документ.ЗаказПереработчику2_5)
		|				И ВЫРАЗИТЬ(РаспоряженияНаОтгрузкуОбороты.Распоряжение КАК Документ.ЗаказПереработчику2_5).СкладМатериалов.ЭтоГруппа
		|				И ВЫРАЗИТЬ(РаспоряженияНаОтгрузкуОбороты.Распоряжение КАК Документ.ЗаказПереработчику2_5).СкладМатериалов.ВыборГруппы =
		...
		//++ НЕ УТ
		|			КОГДА РаспоряженияНаОтгрузкуОбороты.Распоряжение.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПроизводствоУПереработчика2_5)
...
Функция МетаданныеНакладнойПоОперации(ПараметрыОперации)
	...
	//++ НЕ УТ
	ИначеЕсли ОперацияРаспоряжения = Перечисления.ХозяйственныеОперации.ПередачаПереработчику2_5 Тогда
		МетаданныеНакладной = Метаданные.Документы.ПередачаТоваровХранителю;
	//-- НЕ УТ
	КонецЕсли;
