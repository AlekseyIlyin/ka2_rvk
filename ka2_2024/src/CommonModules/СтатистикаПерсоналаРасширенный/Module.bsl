...
Процедура ПоместитьКатегориюПерсоналаВОписаниеПозиции(Форма, ДанныеПозиции) Экспорт
	...
	Для Каждого Запись Из Форма.СтрокиОтчетностиМониторингаРаботниковСоциальнойСферы Цикл
		ОписаниеКаретории = Новый Структура();
		ОписаниеКаретории.Вставить("ВидФормыМониторинга", Запись.ВидФормыМониторинга);
		ОписаниеКаретории.Вставить("СтрокаОтчетностиМониторингаРаботниковСоциальнойСферы", Запись.СтрокаОтчетностиМониторингаРаботниковСоциальнойСферы);
		КатегорииПерсонала.Добавить(ОписаниеКаретории);
	КонецЦикла;
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка настройки статичтики порсонала для организации'");
	Обработчик.НачальноеЗаполнение = Истина;
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка вид формы мониторингана в строках отчетности мониторинга работников социальной сферы'");
	Обработчик.НачальноеЗаполнение = Истина;
...
Функция КритерииОтбораПоРегистрацииРеспондента(ПараметрыОтчета)
	...
	//		   * Сотрудников на территориях использующих регистрацию организации;
	//		   * Исключаем сотрудников на территориях с другой регистрацией;
	// - ТОСП, то берем:
	...
	//		* Сотрудников на территориях с этой же регистрацией;
	//		* Исключаем сотрудников на территориях с другой регистрацией;
	...
	Если СвойстваРеспондента.ЭтоОрганизация Тогда
		...
		Запрос = Новый Запрос(
			"ВЫБРАТЬ
			|	ДОБАВИТЬКДАТЕ(&ДатаАктуальности, ЧАС, 1) КАК Период
			|ПОМЕСТИТЬ ВТДатыДляСрезаТерриторий");
		Запрос.УстановитьПараметр("ДатаАктуальности", ПараметрыОтчета.ДатаНачалаПериодаОтчета);

		ОписаниеФильтра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра("ВТДатыДляСрезаТерриторий", "Период");
		ЗарплатаКадрыОбщиеНаборыДанных.ОбъединитьЗапросы(Запрос,
			ЗарплатаКадрыПериодическиеРегистры.ЗапросВТИмяРегистраСрез("ТерриторииСотрудников", Истина, ОписаниеФильтра));
		...
		ЗапросДанных = Новый Запрос(
			...
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТТерриторииСотрудниковСрезПоследних КАК ТерриторииСотрудников
		...
		ЗапросДанных.УстановитьПараметр("ДатаАктуальности", ПараметрыОтчета.ДатаНачалаПериодаОтчета);
	...
	Если КритерииОтбора.ПоИсключаемым Тогда
		...
		Иначе
			ГруппаИ = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
				Отбор, НСтр("ru = 'Исключаемые подразделения и сотрудники'"), ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
		...
		Иначе
			Если КритерииОтбора.ВключаемыеПодразделения <> Неопределено И КритерииОтбора.ВключаемыеСотрудники <> Неопределено Тогда
				МестоОтбора = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
					Отбор, НСтр("ru = 'Подразделения или сотрудники'"), ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли);
			Иначе
				МестоОтбора = Отбор;
			КонецЕсли;
			...
			Иначе
				ГруппаИ = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
					МестоОтбора, НСтр("ru = 'Подразделение'"), ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
