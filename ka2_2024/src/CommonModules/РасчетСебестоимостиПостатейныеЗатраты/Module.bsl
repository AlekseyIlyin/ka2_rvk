...
Процедура ПодготовитьДанныеПоНаправлениямРаспределенияПоПодразделениям(Запрос, ПараметрыРасчета)
	...
	|	ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) КАК АналитикаРасходов,
	|	ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка) КАК Подразделение,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РПП.АналитикаРасходов КАК АналитикаРасходов,
	|	РПП.НаправлениеРаспределения КАК НаправлениеРаспределения,
	|	РПП.АналитикаРасходов КАК Подразделение
	|ИЗ
	...
	Пока ПервичныеДанныеПоПодразделениям.Следующий() Цикл
		...
		ПодходящиеПодразделения = Документы.РаспределениеПрочихЗатрат.ПодразделенияПоНаправлению(
			ПервичныеДанныеПоПодразделениям.АналитикаРасходов,
		...
		Для Каждого ПодходящиееПодразделение Из ПодходящиеПодразделения Цикл
			НоваяСтрока = ПодразделенияСНаправлениемРаспределения.Добавить();
			НоваяСтрока.АналитикаРасходов = ПервичныеДанныеПоПодразделениям.АналитикаРасходов;
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПоместитьТаблицуЗначенийВоВременнуюТаблицу(
		...
		"АналитикаРасходов, Подразделение, НаправлениеРаспределения",
		"АналитикаРасходов, Подразделение, НаправлениеРаспределения"
	);
...
//
Функция ТекстБазаРаспределенияДолейПоЭтапам(ПараметрыРасчета) Экспорт
...
Функция ТекстБазаРаспределенияДолейПоПодразделениям(ПараметрыРасчета)
	...
	ТекстПоПоказателям = 
		...
		|			И ДД.АналитикаРасходов = ПНР.АналитикаРасходов
		|
		|ГДЕ
...
Функция ТекстПартииДляОтнесенияРасходов(ПараметрыРасчета = Неопределено) Экспорт
...
Функция ТекстВыпущеннаяПродукция()
	...
	ТекстЗапроса =
		...
		|		КОГДА ДД.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Работа)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
...
Функция ТекстРасходыПрочихРасходовНЗППоМатериалам() // использует ПартииДляОтнесенияРасходов, МатериальныеЗатратыПроизводства, ОстаткиПриходыРасходы, РаботыДляДавальца
	...
	|	ВЫБОР 
	|		КОГДА ДД.ВариантРаспределенияРасходовНУ В (ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты), ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаСебестоимостьПроизводства))
	|			ТОГДА ВЫРАЗИТЬ(Материальные.СтоимостьРегл / ЕСТЬNULL(Коэффициенты.КоэффициентРегл, 1) КАК ЧИСЛО (23, 10))
	|		ИНАЧЕ
	|			0
	...
	|			ТОГДА 0
	|		КОГДА ДД.ВариантРаспределенияРасходовНДД В (ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты), ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаСебестоимостьПроизводства))
	|			ТОГДА ВЫРАЗИТЬ(Материальные.СтоимостьРегл / ЕСТЬNULL(Коэффициенты.КоэффициентРегл, 1) КАК ЧИСЛО (23, 10))
	|		ИНАЧЕ
	|			0
