...
Процедура ОтправитьОтчетИзФормыСписка(Форма, ИмяКоманды, КонтекстЭДОКлиент, ТекущиеДанные) Экспорт
	...
	Если ТекущиеДанныеНаименованиеОтчета = "Движение средств нерезидента за пределами РФ"
		И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.дв.ден.нерез.за п.РФ") Тогда
		ПоказатьИВернутьОшибку(Форма,
			НСтр("ru = 'Отправка в электронном виде формы отчета не предусмотрена
					   |законодательством. Форма сдается в бумажном виде'"));
		Возврат;
	...
	Если ТекущиеДанныеНаименованиеОтчета = "Акцизы на алкогольную продукцию"
		И ТипЗнч(ТекущиеДанные.РегламентированныйОтчет) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
		ОписаниеРегОтчета = РегламентированнаяОтчетностьВызовСервера.ОписаниеРегОтчета(
			ТекущиеДанные.РегламентированныйОтчет);
		Если ОписаниеРегОтчета.ВыбраннаяФорма = "ФормаОтчета2025Кв1"
			И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.акцизы алкоголь 25к1",
			'20250101') Тогда
			ПоказатьИВернутьОшибку(Форма,
				НСтр("ru = 'Отправка в электронном виде формы отчета не предусмотрена
						   |законодательством. Форма сдается в бумажном виде'"));
