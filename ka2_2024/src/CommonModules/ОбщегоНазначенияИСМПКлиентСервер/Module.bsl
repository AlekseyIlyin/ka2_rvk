...
Функция СтатусЛокальногоМодуляЧестныйЗнак(СтатусСистемы) Экспорт
	...
	Если ВРег(СтатусСистемы) = "NOT_CONFIGURED" Тогда
		ПредставлениеСтатуса = "не настроено";
	ИначеЕсли ВРег(СтатусСистемы) = "INITIALIZATION" Тогда
		ПредставлениеСтатуса = "инициализация";
	ИначеЕсли ВРег(СтатусСистемы) = "READY" Тогда
		ПредставлениеСтатуса = "готово к использованию";
	ИначеЕсли ВРег(СтатусСистемы) = "SYNC_ERROR" Тогда
	...
	Иначе
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Внутренняя ошибка. Не определено представление статуса для статуса системы %1'"), СтатусСистемы);
	КонецЕсли;
