...
Процедура НачалоВыбораАналитикиРасходов(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(АналитикаРасходов) = Тип("СправочникСсылка.ПартииПроизводства") Тогда
		...
		ОрганизацияЗначение = Неопределено;
		ДатаЗначение = Неопределено;
		
		Если Не ТипЗнч(Данные) = Тип("ФормаКлиентскогоПриложения") Тогда
			Данные.Свойство("Организация", ОрганизацияЗначение);
			ПутьКДаннымСодержитОбъект = Лев(ПараметрыВыбораСтатьи.ПутьКДанным, 6) = "Объект";
			Если Не ЗначениеЗаполнено(ОрганизацияЗначение) И ПутьКДаннымСодержитОбъект Тогда
				Форма.Объект.Свойство("Организация", ОрганизацияЗначение);
			КонецЕсли;
			
			Данные.Свойство("Дата", ДатаЗначение);
			Если Не ЗначениеЗаполнено(ДатаЗначение) И ПутьКДаннымСодержитОбъект Тогда
				Форма.Объект.Свойство("Дата", ДатаЗначение);
			КонецЕсли;
		КонецЕсли;
		
		СтруктураОтбора = Новый Структура;
		СтруктураОтбора.Вставить("Организация", ОрганизацияЗначение);
		ПараметрыФормы = Новый Структура();
		
		Если ЗначениеЗаполнено(ДатаЗначение) Тогда
			ПараметрыФормы.Вставить("НачалоПериода",	НачалоМесяца(ДатаЗначение));
			ПараметрыФормы.Вставить("ОкончаниеПериода",	КонецМесяца(ДатаЗначение));
		КонецЕсли;
