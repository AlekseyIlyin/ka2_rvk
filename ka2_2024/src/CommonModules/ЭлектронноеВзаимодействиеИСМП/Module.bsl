...
Процедура ДобавитьСведенияОУчастникахОбмена_5_02(ДанныеШтрихкодовУпаковок, Источник, СтруктураДанных, ДеревоДанных, ДатаСведений, Отказ) Экспорт
	...
	ЭлектронноеВзаимодействиеИСМППереопределяемый.ЗаполнитьСведенияОбАдресахУчастниковОбмена_5_02(СтруктураДанных, Источник, СтруктураАдресов);
	...
	Для Каждого ТипУчастника Из Участники Цикл
		...
		Для Каждого Участник Из СписокУчастниковСтороны Цикл
			...
			Если ТипУчастника.Ключ = "Продавец"
				...
				НоваяСтрокаГрузополучателя[Участник.Значение] = ЭлектронноеВзаимодействие.ДанныеЭлементаДереваЭлектронногоДокумента(
...
Процедура ДополнитьРезультатКодамиОСУ(ТаблицаТоваров, ПараметрыСканирования, ЧастичноеСодержимоеРезультат) Экспорт
	...
	ТекстЗапроса = 
		...
		|	И НастройкиУчетаМаркируемойПродукцииИСМП.ДатаОптКонтрольПоштучно <> ДАТАВРЕМЯ(1, 1, 1)
		|	И НастройкиУчетаМаркируемойПродукцииИСМП.ДатаОптКонтрольПоштучно > &КонтролируемаяДата
		|	И НастройкиУчетаМаркируемойПродукцииИСМП.ДатаЗапретПродажиНемаркированныхОстатков < &КонтролируемаяДата
		|	И НЕ &НеКонтролироватьДатыДействияОСУ
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	СвойстваМаркируемойПродукции.ВидПродукции В (&ВидыПродукцииПоддерживаютОСУ)
		|	И &НеКонтролироватьДатыДействияОСУ
		|";
	...
	Если ДокументВнутреннегоПеремещения Тогда
		ВидыПродукцииПоддерживаютОСУ = ОбщегоНазначенияИСКлиентСервер.ВидыПродукцииОбъемноСортовогоУчетаДляВнутреннегоПеремещения();
	Иначе
		ВидыПродукцииПоддерживаютОСУ = ОбщегоНазначенияИСКлиентСервер.ВидыПродукцииОбъемноСортовогоУчета();
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("НеКонтролироватьДатыДействияОСУ", ДокументВнутреннегоПеремещения);
	...
	Для Каждого СтрокаТЧ Из МаркируемаяПродукцияДляОСУ Цикл
		...
		ИначеЕсли ОбщегоНазначенияИСМППовтИсп.ДействуетКонтрольПриПродажеОптомПоштучно(СтрокаТЧ.ВидПродукцииИС, ПроверятьНаДату)
			И Не ОбщегоНазначенияИСВызовСервера.ЭтоДокументВнутреннегоПеремещения(ПараметрыСканирования.СсылкаНаОбъект) Тогда
			ЕстьОшибки = Истина;
			НоваяСтрока.ТекстОшибкиИС = СтрШаблон(
				НСтр("ru = 'Требуется указание кода маркировки, по %1 ОСУ на дату %2 не поддерживается.'"),
				ОбщегоНазначенияИС.ПредставлениеНоменклатуры(СтрокаТЧ.Номенклатура, СтрокаТЧ.Характеристика), Формат(ПроверятьНаДату, "ДФ=dd.MM.yyyy;"));
