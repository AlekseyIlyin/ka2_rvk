...
Функция ФактическиеДанныеИзИсточников(ИмяСправочникаИсточника, ПараметрыПолученияФакта)
	...
	Для Каждого ПрименяемоеПравилоФакта Из ТаблицаПравилСтатей Цикл
		...
		Если ИдентификаторИсточникаДанных = "ДвиженияДенежныхСредствКВыплате" Тогда
			ИдентификаторИсточникаДанных = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"ДвиженияДенежныхСредствКВыплате%1", МониторингЦелевыхПоказателей.ПолучитьИмяЗначенияПеречисления(
				ПрименяемоеПравилоФакта.ТипДанныхУчетаПриход));
		КонецЕсли;
