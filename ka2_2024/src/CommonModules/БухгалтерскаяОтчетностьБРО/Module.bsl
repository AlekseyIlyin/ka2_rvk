...
Функция НовыйКонтекстОтчета() Экспорт
	...
	Контекст.Вставить("Период",              '0001-01-01'); // конец периода, за который готовится отчетность (отчетная дата)
...
Функция ГрафыОтчета() Экспорт
	...
	ГрафыРаздела.Вставить(0, "04");
	ГрафыРаздела.Вставить(1, "05");
	ГрафыОтчета.Вставить("ОФР", ГрафыРаздела);
...
//
Функция СоздатьВписываемыеСтроки(Родитель, Знач Количество, КодСтрокиРаздела = Неопределено)
...
// В общем случае заполняется отчет в целом - все доступные для заполнения разделы и оба периода (отчетный и сравнительный).
//
// Параметры:
//  Контейнер                  - ДеревоЗначений - заполняемые данные разделов в терминах формы отчета; в Контейнер помещается результат заполнения
//  ЗаполняемыеРазделы         - Структура - перечень разделов отчета; Ключ - имя раздела из РазделыОтчета, Значение - Булево, Истина, если этот раздел нужно заполнить
//  ЗаполняемыеПоказатели      - НовыйЗаполняемыеПоказатели - описание структуры отчета и правил его заполнения
//                             - Неопределено - использовать структуру отчета и правила его заполнения по умолчанию
//  ОтчетныйПериодНачало       - Дата - начало отчетного периода (данных, относящихся к отчетному периоду).
//                               Обычно это 1 января отчетного года, но может быть и другая дата, если речь идет о первом отчетном периоде организации.
//  ОтчетныйПериодКонец        - Дата, МоментВремениСУточнениемПериода - 
//  СравнительныйПериодНачало  - Дата - начало сравнительного периода.
//                               Если заполнение сведений за сравнительный период не требуется, следует передать пустую дату
//  СравнительныйПериодКонец   - Дата, МоментВремениСУточнениемПериода - конец сравнительного периода.
//                               Если заполнение сведений за сравнительный период не требуется, следует передать пустую дату
...
//
Процедура ЗаполнитьОтчет(Контейнер, ЗаполняемыеРазделы, ЗаполняемыеПоказатели, ОтчетныйПериодНачало, ОтчетныйПериодКонец, СравнительныйПериодНачало, СравнительныйПериодКонец, Организации) Экспорт
	...
	КонтекстОтчета = НовыйКонтекстОтчета();
	ЗаполнитьПериодОтчета(КонтекстОтчета.ОтчетныйПериод,      ОтчетныйПериодНачало,      ОтчетныйПериодКонец);
	ЗаполнитьПериодОтчета(КонтекстОтчета.СравнительныйПериод, СравнительныйПериодНачало, СравнительныйПериодКонец);
	...
	Для Каждого Раздел Из РазделыОтчета() Цикл
		...
		Если Не ЗаполняемыеРазделы.Свойство(Раздел) Или ЗаполняемыеРазделы[Раздел] <> Истина Тогда
			Продолжить;
		КонецЕсли;
		
		ЗаполняемыеПоказателиРаздела = ЗаполняемыеПоказатели.Разделы[Раздел];
		Если ЗаполняемыеПоказателиРаздела = Неопределено Тогда
			ЗаполняемыеПоказателиРаздела = НовыйЗаполняемыеПоказателиРаздела();
			БухгалтерскаяОтчетностьБРОПереопределяемый.ПриОпределенииЗаполняемыхПоказателей(ЗаполняемыеПоказателиРаздела, Раздел, КонтекстОтчета);
		КонецЕсли;
...
Процедура ЗаполнитьКонтейнер(ОписаниеРаздела, РезультатЗаполнения, ЗаполняемыеПоказатели, НомерГрафыЗначений) Экспорт
	...
	Для Каждого ПланГруппы Из ПланРаздела.Строки Цикл
		...
		Для Каждого ЗаписьЗначенияЗаполненияПоказателя Из ЗаписьЗначенияЗаполненияГруппы.Строки Цикл
			...
			ДополнитьРасшифровкуКонтейнера(
				ЯчейкаГруппы,
...
Функция УстановитьЗначениеКонтейнера(ЗаписьОтчета, ЗаписьЗначенияЗаполнения, НомерГрафыЗначений, ДанныеРасшифровки, ШаблонРасшифровкиМеханизм2011)
	...
	Если ЗаписьЗначенияЗаполнения <> Неопределено Тогда
		ДополнитьРасшифровкуКонтейнера(ЯчейкаЗначения, ЗаписьЗначенияЗаполнения, ДанныеРасшифровки, ШаблонРасшифровкиМеханизм2011);
	КонецЕсли;
...
Процедура ДополнитьРасшифровкуКонтейнера(ЯчейкаЗначения, ЗаписьЗначенияЗаполнения, ДанныеРасшифровки, ШаблонРасшифровкиМеханизм2011)
	
	ЗаписиРасшифровки = ЗаписьЗначенияЗаполнения.Расшифровка;
	
	Для Каждого ЗаписьРасшифровки Из ЗаписиРасшифровки Цикл
		ОберткаРасшифровки = ОбщегоНазначенияКлиентСервер.СкопироватьРекурсивно(ШаблонРасшифровкиМеханизм2011);
