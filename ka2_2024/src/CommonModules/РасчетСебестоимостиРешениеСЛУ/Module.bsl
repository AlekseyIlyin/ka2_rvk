...
Функция ТекстВтПеремещенияСписания() // вт ВтПеремещенияСписания, использует ВтРаботыДляДавальца
	...
	Возврат "
		...
		|			И УзлыКорректировкиИсточник.АналитикаФинансовогоУчета = Себестоимость.КорАналитикаФинансовогоУчета
...
Функция ТекстСвязиУзловРезервовПодОбесценениеЗапасов()
	...
	Возврат "
		...
		|		Себестоимость.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОтчетДавальцу2_5)
		|		И (АналитикаНоменклатурыИсточник.Номенклатура = АналитикаНоменклатурыПриемник.Номенклатура
		...
		|		ПО ЕСТЬNULL(УзлыКорректировкиПриемникПередачи.АналитикаУчетаНоменклатуры, УзлыКорректировкиПриемник.АналитикаУчетаНоменклатуры) = АналитикаНоменклатурыПриемник.КлючАналитики
		|
		|	ГДЕ
...
Функция ТекстСвязиСЛУ_ПостатейныеРасходыПредприятия()
	Возврат "
		...
		|	НЕ ПеремещенияСписания.НомерУзлаПриемник ЕСТЬ NULL
		|	И НЕ ПеремещенияСписания.НомерУзлаИсточник ЕСТЬ NULL
		|	И (ПеремещенияСписания.РеализацияВДругуюОрганизацию ИЛИ НЕ ПеремещенияСписания.ИсточникНаСреднескользящей)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
