...
//  Измерение - Строка, ПланВидовХарактеристикСсылка.АналитикиСтатейБюджетов - аналитика бюджетного отчета
//
// Возвращаемое значение:
//  Строка - текст ошибки (если проверка пройдена удачно, то пустая строка)
//
Функция ТекстОшибкиВыраженияПредставленияСортировки(ТекстВыражения, Измерение) Экспорт
	
	Если ТипЗнч(Измерение) = Тип("ПланВидовХарактеристикСсылка.АналитикиСтатейБюджетов") Тогда
		ИмяИзмерения = ФинансоваяОтчетностьПовтИсп.ИмяПоляБюджетногоОтчета(Измерение);
	Иначе
		ИмяИзмерения = Измерение;
	КонецЕсли;
	ОписаниеТиповИзмерения = ФинансоваяОтчетностьПовтИсп.ТипЗначенияВидаАналитики(ИмяИзмерения, Измерение);
	
	ТекстВыраженияСПсевдонимом = ФинансоваяОтчетностьПовтИсп.ТекстВыраженияПредставленияСортировкиСИменемИзмерения(
		ТекстВыражения, ИмяИзмерения);
	...
	Для Каждого Тип Из ОписаниеТиповИзмерения.Типы() Цикл
		ИмяТаблицыДанных = РаботаСФормуламиПовтИсп.ПолноеИмяТаблицыДанных(Метаданные.НайтиПоТипу(Тип).ПолноеИмя());
		ТекстыЗапросов.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонЗапроса,
			ТекстВыраженияСПсевдонимом, ИмяТаблицыДанных, ИмяИзмерения));
	КонецЦикла;
