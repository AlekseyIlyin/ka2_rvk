...
Функция ТекстЗапросаТаблицаОперацииСПрослеживаемымиТоварамиРасширенный(Запрос, ТекстыЗапроса, Регистры)
	...
	ВсегдаОтражатьВРасширеном = УчетнаяПолитика.ПрименяетсяУСН(Запрос.Параметры.Организация,Запрос.Параметры.Период);
	Если ВсегдаОтражатьВРасширеном Тогда
...
Функция ТекстПереносПретензииНаРасчетыНовыеВзаиморасчеты()
	...
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|
	|	Расчеты.СуммаРегл - ЕСТЬNULL(РасчетыПоТаре.СуммаРегл, 0) - ВЫБОР
	|		КОГДА ЕСТЬNULL(РасчетыПоТаре.СуммаРегл, 0) - РасчетыПоДолгу.СуммаРегл > 0
	|		ТОГДА ЕСТЬNULL(РасчетыПоТаре.СуммаРегл, 0) - РасчетыПоДолгу.СуммаРегл
	|		ИНАЧЕ 0 КОНЕЦ КАК Сумма,
	|	Расчеты.СуммаУпр - ЕСТЬNULL(РасчетыПоТаре.СуммаУпр, 0) - ВЫБОР
	|		КОГДА ЕСТЬNULL(РасчетыПоТаре.СуммаУпр, 0) - РасчетыПоДолгу.СуммаУпр > 0
	|		ТОГДА ЕСТЬNULL(РасчетыПоТаре.СуммаУпр, 0) - РасчетыПоДолгу.СуммаУпр
	|		ИНАЧЕ 0 КОНЕЦ КАК СуммаУУ,
	|
	...
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|
	|	Расчеты.Сумма - ЕСТЬNULL(РасчетыПоТаре.Сумма, 0) - ВЫБОР
	|		КОГДА ЕСТЬNULL(РасчетыПоТаре.Сумма, 0) - РасчетыПоДолгу.Сумма > 0
	|		ТОГДА ЕСТЬNULL(РасчетыПоТаре.Сумма, 0) - РасчетыПоДолгу.Сумма
	|		ИНАЧЕ 0 КОНЕЦ КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	0 КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		РасчетыСПоставщикамиНоваяАрхитектураПоОбъектамРасчетов КАК РасчетыПоПретензиямСводно
	|	ПО
	|		РасчетыПоПретензиям.ОбъектРасчетов = РасчетыПоПретензиямСводно.ОбъектРасчетов
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		РасчетыСПоставщикамиНоваяАрхитектура КАК Расчеты
	|	ПО
	...
	|	ПО
	|		РасчетыПоПретензиям.Ссылка = РасчетыПоДолгу.Ссылка
	|		И Расчеты.Предоплата <> 0 И РасчетыПоДолгу.Долг <> 0
	|		И (РасчетыПоДолгу.ОбъектРасчетов <> РасчетыПоПретензиям.ОбъектРасчетов
	|		ИЛИ РасчетыПоДолгу.ВидСчета <> ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РасчетыСПоставщиками))
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РасчетыПоТареСводно КАК РасчетыПоТаре
	|	ПО
