...
// Выполняет обновление данных регистра сведений ВерсииПоставяемыхНастроек.
//
Процедура ОбновитьСлужебныеДанныеНастроек() Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ВерсииПоставяемыхНастроек.Идентификатор КАК Идентификатор,
		|	ВерсииПоставяемыхНастроек.Версия КАК Версия,
		|	ВерсииПоставяемыхНастроек.ОбновлятьАвтоматически КАК ОбновлятьАвтоматически,
		|	ВерсииПоставяемыхНастроек.ОбщиеДанные КАК ОбщиеДанные,
		|	ВерсииПоставяемыхНастроек.СохранятьФайлВКэш КАК СохранятьФайлВКэш
		|ИЗ
		|	РегистрСведений.ВерсииПоставяемыхНастроек КАК ВерсииПоставяемыхНастроек";
	...
	Для Каждого ОписаниеНастройки Из Настройки Цикл
		...
		Если ВыборкаДетальныеЗаписи.НайтиСледующий(Отбор) Тогда
			...
			Если ВыборкаДетальныеЗаписи.ОбновлятьАвтоматически <> ОписаниеНастройки.ОбновлятьАвтоматически
				...
				Запись = РегистрыСведений.ВерсииПоставяемыхНастроек.СоздатьМенеджерЗаписи();
				ЗаполнитьЗначенияСвойств(
					Запись,
	...
	Запрос.Текст =
		...
		|	РегистрСведений.ВерсииПоставяемыхНастроек КАК ВерсииПоставяемыхНастроек
		|ГДЕ
		|	НЕ ВерсииПоставяемыхНастроек.Идентификатор В (&ИспользуемыеНастройки)";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Набор = РегистрыСведений.ВерсииПоставяемыхНастроек.СоздатьНаборЗаписей();
...
// Выполняет обновление данных регистра сведений ВерсииПоставяемыхНастроекОбластейДанных.
//
Процедура ОбновитьСлужебныеДанныеНастроекОбластейДанных(Параметры) Экспорт
	...
	Попытка
		...
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ВерсииПоставяемыхНастроекОбластейДанных");
		...
		Запрос.Текст =
			"ВЫБРАТЬ
			|	ВерсииПоставяемыхНастроекОбластейДанных.Идентификатор КАК Идентификатор,
			|	ВерсииПоставяемыхНастроекОбластейДанных.Версия КАК Версия
			|ИЗ
			|	РегистрСведений.ВерсииПоставяемыхНастроекОбластейДанных КАК ВерсииПоставяемыхНастроекОбластейДанных";
		...
		Для Каждого ОписаниеНастройки Из Настройки Цикл
			...
			Если Не ВыборкаДетальныеЗаписи.НайтиСледующий(Отбор) Тогда
				Запись = РегистрыСведений.ВерсииПоставяемыхНастроекОбластейДанных.СоздатьМенеджерЗаписи();
		...
		Запрос.Текст = 
			...
			|	РегистрСведений.ВерсииПоставяемыхНастроекОбластейДанных КАК ВерсииПоставяемыхНастроекОбластейДанных
			|ГДЕ
			|	НЕ ВерсииПоставяемыхНастроекОбластейДанных.Идентификатор В (&ИспользуемыеНастройки)";
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Набор = РегистрыСведений.ВерсииПоставяемыхНастроекОбластейДанных.СоздатьНаборЗаписей();
			Набор.Отбор.Идентификатор.Установить(ВыборкаДетальныеЗаписи.Идентификатор);
			Набор.Записать();
		КонецЦикла;
	...
	Исключение
		...
		ЗаписатьИнформациюВЖурналРегистрации(
			ИнформацияОбОшибке,
			Истина,
			Метаданные.РегистрыСведений.ВерсииПоставяемыхНастроекОбластейДанных);
...
Функция ПараметрыНастройки(Идентификатор) Экспорт
	...
	Запрос.Текст =
		...
		|	РегистрСведений.ВерсииПоставяемыхНастроек КАК ВерсииПоставяемыхНастроек
		|ГДЕ
		|	ВерсииПоставяемыхНастроек.Идентификатор = &Идентификатор";
...
Функция ВерсияПоставляемойНастройки(Идентификатор) Экспорт
	...
	ИмяРегистра = ?(ОбщиеДанные,
		"ВерсииПоставяемыхНастроек",
		"ВерсииПоставяемыхНастроекОбластейДанных");
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	РегВерсииПоставяемыхНастроек.Версия КАК Версия
		|ИЗ
		|	РегистрСведений.%1 КАК РегВерсииПоставяемыхНастроек
		|ГДЕ
		|	РегВерсииПоставяемыхНастроек.Идентификатор = &Идентификатор";
...
Процедура ОбновитьСлужебныеДанныеНастройки(ДанныеНастройки, ДанныеФайла, ОбновитьКэш = Истина) Экспорт
		...
		Иначе
			...
			Попытка
				...
				Блокировка = Новый БлокировкаДанных;
				ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ВерсииПоставяемыхНастроек");
...
Процедура ОбновлениеПоставляемыхНастроек() Экспорт
	...
	Запрос.Текст = 
		...
		|	РегистрСведений.ВерсииПоставяемыхНастроек КАК ВерсииПоставяемыхНастроек
		|ГДЕ
		|	ВерсииПоставяемыхНастроек.ОбновлятьАвтоматически";
...
Функция ДанныеПоставляемыхНастроекИзИзКэша(Идентификаторы)
	...
	Запрос.Текст =
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ВерсииПоставяемыхНастроек КАК ВерсииПоставяемыхНастроек
		|		ПО КэшДанныхПоставляемыхНастроек.Идентификатор = ВерсииПоставяемыхНастроек.Идентификатор
		|ГДЕ
...
//   регистр сведений ВерсииПоставяемыхНастроекОбластейДанных.
//
...
Процедура ПроверитьДоступностьОбновления()
	...
	ОбъектМетаданных = ?(
		ИспользоватьДанныеОбласти(),
		Метаданные.РегистрыСведений.ВерсииПоставяемыхНастроекОбластейДанных,
		Метаданные.РегистрыСведений.ВерсииПоставяемыхНастроек);
