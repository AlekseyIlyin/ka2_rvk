...
Функция ЗапрещенаРаботаРазрешительногоРежимаСФФД_1_05(ВидПродукции, ВидОперации, ТребуетсяПроверкаСредствамиККТ, ПодключенноеККТИСМП) Экспорт
	...
	Если ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ПродукцияПодлежитОбязательнойОнлайнПроверкеПередРозничнойПродажей(
		...
		И ЗначениеЗаполнено(ПодключенноеККТИСМП) Тогда
