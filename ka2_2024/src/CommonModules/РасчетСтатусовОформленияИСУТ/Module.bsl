...
Процедура ПриОпределенииЗапросаДляИСМП(
	...
	ВидыПродукцийРазрешенияНаОтгрузку = ИнтеграцияИСМПКлиентСервер.ВидыПродукцииТребующиеРазрешенияНаОтгрузку();
	
	ЭлементыОтборНоменклатуры.Очистить();
	ЭлементыОтборНоменклатуры.Добавить("
		|ЛОЖЬ");
	Для Каждого ВидПродукцииИСМП Из УчитываемыеВидыПродукции Цикл
		Если ВидыПродукцийРазрешенияНаОтгрузку.Найти(ВидПродукцииИСМП) <> Неопределено Тогда
			ОсобенностьУчета = ИнтеграцияИСУТКлиентСервер.ОсобенностьУчетаПоВидуПродукции(ВидПродукцииИСМП);
			ЭлементыОтборНоменклатуры.Добавить(СтрШаблон("
			|	ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.%1)", XMLСтрока(ОсобенностьУчета)));
		КонецЕсли;
	...
	ОтборНоменклатурыДляРазрешенияНаОтгрузку = СтрШаблон("СправочникНоменклатура.ОсобенностьУчета В(%1)", СтрСоединить(ЭлементыОтборНоменклатуры, ","));
	...
	ИначеЕсли МетаданныеДокументаОснования = Метаданные.Документы.РеализацияТоваровУслуг Тогда
		...
		ИначеЕсли МетаданныеДокументаИСМП = Метаданные.Документы.РазрешениеНаОтгрузкуИСМП Тогда
			...
			ТекстЗапроса = ТекстЗапросаДокументаОснованияПоЭлектронномуДокументу(
				МетаданныеДокументаОснования, ОтборНоменклатурыДляРазрешенияНаОтгрузку);
...
Функция ТекстЗапросаДокументаОснованияПоЭлектронномуДокументу(МетаданныеДокументаОснования, ОтборНоменклатуры)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ОтборНоменклатуры",   ОтборНоменклатуры);
	...
	Для Каждого ВидПродукцииИСМП Из ВидыПродукцийРазрешенияНаОтгрузку Цикл
		ОсобенностьУчета = ИнтеграцияИСУТКлиентСервер.ОсобенностьУчетаПоВидуПродукции(ВидПродукцииИСМП);
		УсловияКонвертации.Добавить(
			СтрШаблон(
				"КОГДА СправочникНоменклатура.ОсобенностьУчета = ЗНАЧЕНИЕ(Перечисление.ОсобенностиУчетаНоменклатуры.%1)
				|	ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыПродукцииИС.%2)",
