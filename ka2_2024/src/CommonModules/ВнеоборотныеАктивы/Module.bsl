...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Ввод остатков узлов и компонентов амортизации'", КодыЯзыков); // @НСтр-1
	...
	Набор.Используется = Истина;
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Перемещение НМА'", КодыЯзыков); // @НСтр-1
	...
	Набор.Используется = Истина;
	МультиязычностьСервер.ЗаполнитьМультиязычныйРеквизит(Набор, "Наименование",
		"ru = 'Наработки объектов эксплуатации'", КодыЯзыков); // @НСтр-1
...
Функция СформироватьТаблицуВтПорцияДанныхКРасчету(НачалоПериода, МенеджерВременныхТаблиц, РазмерПорцииДанных = 100, ХозяйственныеОперации = Неопределено) Экспорт
	...
	|			ИСТИНА
	|		ИЗ
	|			ВтНеобработанныеДанныеКРасчету КАК ПредыдущиеДанныеКРасчету
	|		ГДЕ
	|			ПредыдущиеДанныеКРасчету.ОбъектУчета = ТекущиеДанныеКРасчету.ОбъектУчета
	|			И ПредыдущиеДанныеКРасчету.Ссылка <> ТекущиеДанныеКРасчету.Ссылка
	|
	|			И (ПредыдущиеДанныеКРасчету.Дата < ТекущиеДанныеКРасчету.Дата
	|				ИЛИ ПредыдущиеДанныеКРасчету.Дата = ТекущиеДанныеКРасчету.Дата
	|					И ПредыдущиеДанныеКРасчету.Приоритет < ТекущиеДанныеКРасчету.Приоритет
	|				ИЛИ ПредыдущиеДанныеКРасчету.Дата = ТекущиеДанныеКРасчету.Дата
	|					И ПредыдущиеДанныеКРасчету.Приоритет = ТекущиеДанныеКРасчету.Приоритет
	|					И ПредыдущиеДанныеКРасчету.Ссылка < ТекущиеДанныеКРасчету.Ссылка))
	|
...
Функция ОчередьОбъектов(НачалоПериода, КонецПериода, СписокОрганизаций, МаксимальныйНомерОчереди, МенеджерВременныхТаблиц) Экспорт
	...
	|
	|ИЗ
		// Связь ОС в одном документе
	|	(ВЫБРАТЬ
	|		ТекущиеОбъекты.ОсновноеСредство КАК ОбъектУчета,
	|		ЕСТЬNULL(ЗависимыеОбъекты.ОсновноеСредство, НЕОПРЕДЕЛЕНО) КАК ЗависимыйОбъект
	|
	|	ИЗ
	|		РегистрСведений.ДокументыПоОС КАК ТекущиеОбъекты
	|
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДокументыПоОС КАК ЗависимыеОбъекты
	|			ПО ЗависимыеОбъекты.Ссылка = ТекущиеОбъекты.Ссылка
	|				И ЗависимыеОбъекты.ОсновноеСредство <> ТекущиеОбъекты.ОсновноеСредство
	|
	|	ГДЕ
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|
		// Связь НМА в одном документе
	|	ВЫБРАТЬ
	|		ТекущиеОбъекты.НематериальныйАктив КАК ОбъектУчета,
	|		ЕСТЬNULL(ЗависимыеОбъекты.НематериальныйАктив, НЕОПРЕДЕЛЕНО) КАК ЗависимыйОбъект
	|
	|	ИЗ
	|		РегистрСведений.ДокументыПоНМА КАК ТекущиеОбъекты
	|
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДокументыПоНМА КАК ЗависимыеОбъекты
	|			ПО ЗависимыеОбъекты.Ссылка = ТекущиеОбъекты.Ссылка
	|				И ЗависимыеОбъекты.НематериальныйАктив <> ТекущиеОбъекты.НематериальныйАктив
	|
	|	ГДЕ
	|		ТекущиеОбъекты.Ссылка В (
	|			ВЫБРАТЬ
	...
	|	ВЫБРАТЬ
	|		ТекущиеОбъекты.ОсновноеСредство КАК ОбъектУчета,
	|		ЗависимыеОбъекты.ОсновноеСредство КАК ЗависимыйОбъект
	|
	|ИЗ
	|	РегистрНакопления.СтоимостьОС КАК ТекущиеОбъекты
	|
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьОС КАК ЗависимыеОбъекты
	|			ПО ЗависимыеОбъекты.ВидАналитикиКапитализацииРасходов = ТекущиеОбъекты.ВидАналитикиКапитализацииРасходов
	|				И ЗависимыеОбъекты.АналитикаКапитализацииРасходов = ТекущиеОбъекты.АналитикаКапитализацииРасходов
	|				И ЗависимыеОбъекты.Регистратор <> ТекущиеОбъекты.Регистратор
	|				И ЗависимыеОбъекты.ОсновноеСредство <> ТекущиеОбъекты.ОсновноеСредство
	|				И ЗависимыеОбъекты.Активность
	|
	|ГДЕ
	...
	|				ВтСписокДокументов КАК ВтСписокДокументов)
	|
	|	И ЗависимыеОбъекты.Регистратор В (
	|			ВЫБРАТЬ
	|				ВтСписокДокументов.Ссылка
	|			ИЗ
	|				ВтСписокДокументов КАК ВтСписокДокументов)
	...
	|	ЗаданияКРасчету КАК ЗаданияКРасчету
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтЗависимостьОбъектов КАК ВтЗависимостьОбъектов
	...
	ТаблицаОбъектов.Индексы.Добавить("ОбъектУчета,Обработано");
	...
	ОчередьОбъектов.Колонки.Добавить("НомерОчереди", ОбщегоНазначения.ОписаниеТипаЧисло(3));
	
	ТекущийНомерОчереди = 1;
	Для Каждого Выборка Из ТаблицаОбъектов Цикл
		
		Если Выборка.Обработано Тогда
			Продолжить;
		КонецЕсли;
		
		Выборка.Обработано = Истина;
		
		ЗависимыеОбъекты = Новый Массив;
		ЗависимыеОбъекты.Добавить(Выборка.ОбъектУчета);
		Если ЗначениеЗаполнено(Выборка.ЗависимыйОбъект) Тогда
			ЗависимыеОбъекты.Добавить(Выборка.ЗависимыйОбъект);
		КонецЕсли;
		
		// Поиск и добавление всех зависимых объектов
		Пока ЗависимыеОбъекты.Количество() <> 0 Цикл
			
			ЗависимыйОбъект = ЗависимыеОбъекты[0];
			
			ТекущийОбъект = ОчередьОбъектов.Добавить();
			ЗаполнитьЗначенияСвойств(ТекущийОбъект, Выборка);
			ТекущийОбъект.ОбъектУчета = ЗависимыйОбъект;
			ТекущийОбъект.НомерОчереди = ТекущийНомерОчереди;
			
			СтруктураПоиска = Новый Структура("ОбъектУчета, Обработано", ЗависимыйОбъект, Ложь);
			СписокОбъектов = ТаблицаОбъектов.НайтиСтроки(СтруктураПоиска);
			Для Каждого Элемент Из СписокОбъектов Цикл

				ТекущийОбъект = ОчередьОбъектов.Добавить();
				ЗаполнитьЗначенияСвойств(ТекущийОбъект, Элемент);
				ТекущийОбъект.НомерОчереди = ТекущийНомерОчереди;
				Элемент.Обработано = Истина;
				
				Если ЗначениеЗаполнено(Элемент.ЗависимыйОбъект) Тогда
					ЗависимыеОбъекты.Добавить(Элемент.ЗависимыйОбъект);
				КонецЕсли;
