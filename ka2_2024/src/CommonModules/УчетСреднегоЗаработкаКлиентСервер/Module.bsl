...
Функция ДанныеЧастичнойИндексацииСреднегоЗаработка(ДанныеНачислений, ДанныеВремени, ДополнительныеПараметры, СреднийЗаработок) Экспорт 
	
	ТолькоИндексируемыйЗначение = ДополнительныеПараметры.ТолькоИндексируемый;
	ДополнительныеПараметры.ТолькоИндексируемый = Истина;
	
	ИндексируемыйЗаработок = УчетСреднегоЗаработкаКлиентСервер.СреднийЗаработокОбщий(ДанныеНачислений, ДанныеВремени, ДополнительныеПараметры);
	НеиндексируемыйЗаработок = СреднийЗаработок - ИндексируемыйЗаработок;
	ДополнительныеПараметры.ТолькоИндексируемый = ТолькоИндексируемыйЗначение;
	
	ДанныеСреднегоЗаработка = Новый Структура;
	ДанныеСреднегоЗаработка.Вставить("ИндексируемыйЗаработок", ИндексируемыйЗаработок);
	ДанныеСреднегоЗаработка.Вставить("НеиндексируемыйЗаработок", НеиндексируемыйЗаработок);
