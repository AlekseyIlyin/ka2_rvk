...
Функция ПравилаЗагрузкиДляФормы(ПутьКФорме, НастройкиЗагрузки = Неопределено) Экспорт
	...
	МакетНайден = Ложь;
	КоллекцияМакетов = ?(СтрНачинаетсяС(ПутьКФорме, "ВнешнийОтчет."), 
		Метаданные.Отчеты[СтрРазделить(ПутьКФорме, ".")[1]].Макеты, 
		ОбъектОтчет.Метаданные().Макеты);
	
	Для Каждого МакетТД Из КоллекцияМакетов Цикл
		Если СтрНачинаетсяС(МакетТД.Имя, "EF_") И СтрЗаканчиваетсяНа(МакетТД.Имя, ИмяМакета) Тогда 
			Если СтрНачинаетсяС(ПутьКФорме, "ВнешнийОтчет.") Тогда 
				ПравилаЗагрузки = Отчеты[СтрРазделить(ПутьКФорме, ".")[1]].ПолучитьМакет(МакетТД.Имя);
			Иначе
				ПравилаЗагрузки = ОбъектОтчет.ПолучитьМакет(МакетТД.Имя);
			КонецЕсли;
			МакетНайден = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	Если Не МакетНайден И ОбъектОтчет.Метаданные().Макеты.Найти(ИмяМакета) <> Неопределено Тогда
...
Процедура УведомлениеОСпецрежимахНалогообложения_ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка) Экспорт 
	...
	ИначеЕсли Параметры.Свойство("ЗначениеКопирования") И ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда
		...
		УведомлениеОСпецрежимахНалогообложения.ЗаменитьИмяОтчетаПередОткрытием(ВидСообщения, ИмяФормы, ИмяОтчета);
		ОтобранныеФормыКоличество = Отчеты[ИмяОтчета].ПолучитьТаблицуФорм().НайтиСтроки(Новый Структура("ИмяФормы", ИмяФормы)).Количество();
		Если ОтобранныеФормыКоличество = 0 Тогда 
