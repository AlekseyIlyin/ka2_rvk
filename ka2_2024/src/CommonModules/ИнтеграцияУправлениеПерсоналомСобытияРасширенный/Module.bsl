...
Процедура ФотографииФизическихЛицПриЗаписи(НаборЗаписей) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	РезультатПроверки = ИнтеграцияУправлениеПерсоналом.РезультатПроверкиФизическогоЛица(ФизическиеЛица);
	ИнтеграцияУправлениеПерсоналомСобытия.ЗарегистрироватьИзмененияОбъектов(РезультатПроверки, Неопределено, Неопределено, Неопределено, Неопределено);
...
Процедура ПлановыеЕжегодныеОтпускаПриЗаписи(НаборЗаписей) Экспорт
	...
	|ГДЕ
	|	ВыгружаемыеОбъекты.Ссылка = &Сотрудник";
