	...
	Для Сч = 0 По Количество Цикл
		...
		Если ПереходящаяЧастьПредыдущейСмены Тогда 
			...
			Иначе
				...
				ИмяУстанавливаемогоПараметра = "УстановленДень" + Строка(НомерДня);
			КонецЕсли;
		ИначеЕсли НомерДня = День(КонецМесяца(СтрокаНабора.Период)) И Сч <> Количество
			И СтрокаНабора.Период + 86400 = НаборЗаписейУчетаВремени[Сч + 1].Период
			И НаборЗаписейУчетаВремени[Сч + 1].ПереходящаяЧастьПредыдущейСмены Тогда
