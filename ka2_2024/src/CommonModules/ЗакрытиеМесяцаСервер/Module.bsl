...
Процедура Использование_ПереходНаНовыеРежимыРаботы(ПараметрыОбработчика) Экспорт
	...
	Если ПроизводствоСервер.НастройкиПодсистемыПроизводство().ИспользуетсяПроизводство21 Тогда
		...
		Если ПроизводствоСервер.НастройкиПодсистемыПроизводство().ИспользуетсяПроизводство22 Тогда
			ТекстПояснения = НСтр("ru='Завершить переход на управление производством версии 2.2 и выключить функциональную опцию ""Производство (версия 2.1)""'", ОбщегоНазначения.КодОсновногоЯзыка());
		Иначе
			ТекстПояснения = НСтр("ru='Производство (версия 2.2)'", ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоНаСтороне")
	 И НЕ ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоНаСтороне2_5") Тогда
		...
		УстановитьСостояниеВыполненСОшибками(
			ПараметрыОбработчика,
			НСтр("ru='Передача в переработку (версия 2.5)'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ТекстГиперссылкиПоУмолчанию,
			ОписаниеДействия_ОткрытьФорму(Метаданные.Обработки.ПанельАдминистрированияКА.Формы.Производство.ПолноеИмя()),
			СтатусПоУмолчанию);
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_2")
	 И НЕ ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_4") Тогда
		...
		УстановитьСостояниеВыполненСОшибками(
			ПараметрыОбработчика,
			НСтр("ru='Учет внеоборотных активов (версия 2.4)'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ТекстГиперссылкиПоУмолчанию,
...
Процедура ОписаниеПроверок_ИсправлениеОшибокВИсходныхДанных(ТаблицаПроверок)
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Цены возвратных отходов в документе ""Отчет переработчика""'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Цена должна быть заполнена.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- Устарело_Переработка24
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Наличие отчетов переработчика с выпуском по фиксированной стоимости без выпуска по расчетной'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Без выпуска по расчетной стоимости в НЗП могут остаться незакрытые суммы затрат.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Корректность указания документов производства без заказов в документах распределения расходов.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Указаны документы производства без заказов для которых введено Сторно.'", ОбщегоНазначения.КодОсновногоЯзыка())); 	
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Корректность указания партий производства в документах распределения расходов.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Указаны партии производства, не соответствуюшие документам ""Производство без заказа""..'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Корректность указания статей калькуляций для побочных изделий.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Указаны одинаковые статьи кулькуляции для материалов и побочного выхода по фиксированной стоимости.'", ОбщегоНазначения.КодОсновногоЯзыка())); 	
	...
	Если НЕ ПолучитьФункциональнуюОпцию("АналитическийУчетПоГруппамПродукции") Тогда
		...
		ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
			НСтр("ru='Корректность указания групп продукции в партиях производства.'", ОбщегоНазначения.КодОсновногоЯзыка()),
			НСтр("ru='В партиях производства используются группы продукции, несмотря на отключенную опцию учета по группам продукции.'", ОбщегоНазначения.КодОсновногоЯзыка())); 	
	КонецЕсли;
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Соответствие отчета переработчика и корректировки отчета переработчика.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='Заполнение корректировки отчета переработчика должно соответствовать отчету переработчика.'", ОбщегоНазначения.КодОсновногоЯзыка())); 	
	//-- НЕ УТ
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru='Соответствие организации и подразделения в первичных документах и настройках объектов возникновения затрат.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru='В документе регистрации расхода и указанном объекте возникновения затрат должны совпадать организация и подразделение.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
КонецПроцедуры
...
Процедура ПроверкаКорректностьЗаполненияНомеровГТДВВозвратах(ПараметрыПроверки) Экспорт
	...
	|	И Товары.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	//++ НЕ УТ
	|	И НЕ Товары.Ссылка.ХозяйственнаяОперация В
