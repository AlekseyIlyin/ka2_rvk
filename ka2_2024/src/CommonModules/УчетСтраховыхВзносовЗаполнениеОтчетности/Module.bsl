	...
	Если ВсеФизлица.Количество() > 0 Тогда
		...
		Для каждого СтрокаРезультата Из ВсеФизлица Цикл
			...
			Если СтрокаРезультата.ФизическоеЛицо <> Неопределено И СтрокаРезультата.П000030001001 <> "1" Тогда
				КодКатегорииНулевыхДоходов = КодКатегорииНулевыхДоходовПоКатегорииЗЛ(СтрокаРезультата.КатегорияЗастрахованныхЛиц, ОкончаниеПериода, НачисленоВПоследниеТриМесяца);
...
Процедура РасчетПоказателей_РСВ_2025(НачалоПериода, ОкончаниеПериода, Организация, ОписаниеТаблиц, ПодробныеДанные, ДатаАктуальности, МассивФизическихЛиц)
	...
	Если ОписаниеТаблиц.Свойство("ЧисленностьЗастрахованныхВРазрезеТарифов2023") Или ОписаниеТаблиц.Свойство("ВсеТарифы2023") 
		...
		|	СУММА(СведенияОДоходах.ОбъектОбложенияВзносамиСтатья7ПФР - СведенияОДоходах.ДенежноеДовольствиеВоеннослужащих - СведенияОДоходах.ВыплатыУказанныеВПункте1Части3Статьи9 - СведенияОДоходах.СуммаПоТрудовомуДоговору) КАК НачисленоПоГПХ,
	...
	Если ОписаниеТаблиц.Свойство("ВзносыВРазрезеТарифов2023") Тогда
		...
		|	ЕСТЬNULL(СУММА(ВЫБОР
		|				КОГДА МЕСЯЦ(ВзносыФизлиц.Период) В (&Месяц1, &Месяц2, &Месяц3)
...
Функция КодКатегорииНулевыхДоходовПоКатегорииЗЛ(КатегорияЗастрахованныхЛиц, ОкончаниеПериода, НачисленоВПоследниеТриМесяца)
	
	Если ОкончаниеПериода > '20250101' И Не НачисленоВПоследниеТриМесяца Тогда
		Возврат "НВ"	
	Иначе
		Возврат КатегорияЗастрахованныхЛиц
	КонецЕсли;
