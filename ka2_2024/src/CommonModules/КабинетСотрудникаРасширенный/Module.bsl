...
Процедура ОпубликоватьПраваНаОтпуск(ПараметрыОбмена, БылиОшибки)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 500
	|	ПраваНаОтпуск.Ссылка КАК Сотрудник,
	...
	|		ПО ПраваНаОтпуск.Ссылка = Сотрудники.Ссылка
...
Процедура ОтменитьРегистрациюСотрудникиДляОбновленияПубликацииПравНаОтпуск(ТаблицаИзменений, Результат)
	...
	Если ТаблицаИзменений.Количество() > 0 Тогда
		...
		Попытка
			...
			|ИЗ
			|	ВТОтменитьИзменения КАК ОтменитьИзменения
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИзмененияДляОбменаКабинетСотрудника КАК Сотрудники
			|		ПО ОтменитьИзменения.Сотрудник = Сотрудники.Ссылка
			|			И ОтменитьИзменения.ТипОбъекта = Сотрудники.ТипОбъекта";
			
			Выборка = Запрос.Выполнить().Выбрать();
			
			Пока Выборка.Следующий() Цикл
				МенеджерЗаписи = РегистрыСведений.ИзмененияДляОбменаКабинетСотрудника.СоздатьМенеджерЗаписи();
				МенеджерЗаписи.Ссылка = Выборка.Сотрудник;
