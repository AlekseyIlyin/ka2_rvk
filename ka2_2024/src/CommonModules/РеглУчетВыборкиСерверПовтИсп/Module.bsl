...
// 
// Возвращаемое значение:
//  СписокЗначений - ПланСчетовСылка.Хозрасчетный - 90 и 91 счета.
...
Функция ЗапросДанных(ДокументИмя, ЭтоОбъектРасчетов = Истина, ВременныеТаблицыДокументаПодготовлены = Ложь) Экспорт
	...
	ОсобенностиДокумента = ОсобенностиДокумента(ДокументИмя);
	...
	Иначе
		...
		Если ОсобенностиДокумента.ЭтоДокументУчетаСебестоимости Тогда
			ТекстыСозданияВременныхТаблиц.Добавить(ТекстСебестоимость());
		КонецЕсли;
		
		Если ОсобенностиДокумента.ЭтоДокументУчетаНДС Тогда
	...
	Если ОсобенностиДокумента.ЭтоРасчетныйДокумент Или ДокументИмя = "РасчетКурсовыхРазниц" Тогда
	...
	Если Не ОсобенностиДокумента.ДокументИспользуетВыручкуИСебестоимостьПродаж Тогда
	...
	Если ОсобенностиДокумента.ЭтоРасчетныйДокумент Тогда
	...
	Если ОсобенностиДокумента.ЭтоОбъектРасчетов Тогда
	...
	ТекстыСозданияВременныхТаблиц.Добавить(ТекстДолгосрочнаяЗадолженность(ДокументИмя, ОсобенностиДокумента));
	ТекстыСозданияВременныхТаблиц.Добавить(ТекстНастройкиСтатейРасходов(ОсобенностиДокумента.ЭтоДокументУчетаРасходов));
	ТекстыСозданияВременныхТаблиц.Добавить(ТекстНастройкиСтатейДоходов());
	ТекстыСозданияВременныхТаблиц.Добавить(ТекстВтУточнениеПериода());
	...
	Если ДокументИмя <> "РегистраторРасчетов" Тогда
		...
		КонецЕсли;
		Если ПолучитьФункциональнуюОпцию("ИспользоватьОбесценениеВНА") И ОсобенностиДокумента.ЭтоДокументОбесцененияВНА Тогда
...
Функция ЗапросОбработкиДанных(ЕстьДолгосрочнаяЗадолженность, ЕстьПроводкиПоРасходам = Истина) Экспорт
	...
	|	ВЫБОР
	|		КОГДА РегламентнаяОперация.ТипОперации = ЗНАЧЕНИЕ(Перечисление.ТипыРегламентныхОпераций.РеклассификацияДолгосрочныхАктивовОбязательств)
	...
	|	ПО
	|		РегламентнаяОперация.Ссылка = Данные.Ссылка
	|
	|ГДЕ
	...
	Если ЕстьПроводкиПоРасходам Тогда
		...
		|	Данные.ВидСчетаДт КАК ВидСчетаДт,
		|	Данные.АналитикаУчетаДт КАК ОбъектУчетаДт,
		|	ЕСТЬNULL(НастройкиСтатейРасходовДт.ГФУ_НУ, Данные.АналитикаУчетаДт) КАК АналитикаУчетаДт,
		|	Данные.МестоУчетаДт КАК МестоУчетаДт,
		|	
		|	Данные.ВалютаДт                     КАК ВалютаДт,
		|	Данные.ПодразделениеДт              КАК ПодразделениеДт,
		...
		|	Данные.ВидСчетаКт КАК ВидСчетаКт,
		|	Данные.АналитикаУчетаКт КАК ОбъектУчетаКт,
		|	ЕСТЬNULL(НастройкиСтатейРасходовКт.ГФУ_НУ, Данные.АналитикаУчетаКт) КАК АналитикаУчетаКт,
		|	Данные.МестоУчетаКт КАК МестоУчетаКт,
		|	
		|	Данные.ВалютаКт                  КАК ВалютаКт,
		|	Данные.ПодразделениеКт           КАК ПодразделениеКт,
		...
		|	Данные.Сторно                   КАК Сторно
		|	
		|ПОМЕСТИТЬ ДанныеДляСопоставления2
		|
		|ИЗ
		|	Данные КАК Данные
		|	
		...
		|	Данные.ВидСчетаДт КАК ВидСчетаДт,
		|	Данные.АналитикаУчетаДт КАК ОбъектУчетаДт,
		|	ЕСТЬNULL(НастройкиСтатейРасходовДт.ГФУ_УУ, Данные.АналитикаУчетаДт) КАК АналитикаУчетаДт,
		|	Данные.МестоУчетаДт КАК МестоУчетаДт,
		|	
		|	Данные.ВалютаДт                     КАК ВалютаДт,
		|	Данные.ПодразделениеДт              КАК ПодразделениеДт,
		...
		|	Данные.ВидСчетаКт КАК ВидСчетаКт,
		|	Данные.АналитикаУчетаКт КАК ОбъектУчетаКт,
		|	ЕСТЬNULL(НастройкиСтатейРасходовКт.ГФУ_УУ, Данные.АналитикаУчетаКт) КАК АналитикаУчетаКт,
		|	Данные.МестоУчетаКт КАК МестоУчетаКт,
		|	
		|	Данные.ВалютаКт                  КАК ВалютаКт,
		|	Данные.ПодразделениеКт           КАК ПодразделениеКт,
		...
		|	Данные.Сторно                   КАК Сторно
		|	
		|ПОМЕСТИТЬ ДанныеДляСопоставления3
		|
		|ИЗ
		|	Данные КАК Данные
		|	
	...
	Если ЕстьДолгосрочнаяЗадолженность Тогда
		...
		|	Данные.Сторно                   КАК Сторно
		|
		|ПОМЕСТИТЬ ДанныеДляСопоставления4
		|
		|ИЗ
		|	Данные КАК Данные
		|	
		...
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ДолгосрочнаяЗадолженностьКт.ВариантПереноса, ""Основной"") = ""ЧерезТранзитныйСчет""
		|		ТОГДА Данные.МестоУчетаКт
		|		ИНАЧЕ Данные.МестоУчетаДт
		|	КОНЕЦ КАК МестоУчетаДт,
		|	
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ДолгосрочнаяЗадолженностьКт.ВариантПереноса, ""Основной"") = ""ЧерезТранзитныйСчет""
		...
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ДолгосрочнаяЗадолженностьДт.ВариантПереноса, ""Основной"") = ""ЧерезТранзитныйСчет""
		|		ТОГДА Данные.МестоУчетаДт
		|		ИНАЧЕ Данные.МестоУчетаКт
		|	КОНЕЦ КАК МестоУчетаКт,
		|	
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ДолгосрочнаяЗадолженностьДт.ВариантПереноса, ""Основной"") = ""ЧерезТранзитныйСчет""
		...
		|	ЛОЖЬ КАК Сторно
		|
		|ПОМЕСТИТЬ ДанныеДляСопоставления5
		|
		|ИЗ
		|	Данные КАК Данные
		|	
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ДанныеДляСопоставления5", "ДанныеДляСопоставления6");
		МассивТекстовЗапроса.Добавить(ТекстЗапроса);
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПереносСуммыЗадолженности", "ИСТИНА");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПереносЗадолженностиРазделением", "ЛОЖЬ");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ДанныеДляСопоставления5", "ДанныеДляСопоставления7");
		МассивТекстовЗапроса.Добавить(ТекстЗапроса);
		...
		|	ЛОЖЬ КАК Сторно
		|
		|ПОМЕСТИТЬ ДанныеДляСопоставления8
		|
		|ИЗ
		|	Данные КАК Данные
		|	
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПереносСуммыЗадолженности", "ИСТИНА");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПереносЗадолженностиРазделением", "ЛОЖЬ");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ДанныеДляСопоставления5", "ДанныеДляСопоставления9");
		#КонецОбласти
		МассивТекстовЗапроса.Добавить(ТекстЗапроса);
		...
		|	ЛОЖЬ                            КАК Сторно
		|
		|ПОМЕСТИТЬ ДанныеДляСопоставления10
		|
		|ИЗ
		|	Данные КАК Данные
		|	
		...
		|	ЛОЖЬ                            КАК Сторно
		|
		|ПОМЕСТИТЬ ДанныеДляСопоставления11
		|
		|ИЗ
		|	Данные КАК Данные
		|	
		...
		|	ЕСТЬNULL(ДолгосрочнаяЗадолженностьКт.АналитикаУчета, Данные.АналитикаУчетаДт) КАК АналитикаУчетаДт,
		|	ЕСТЬNULL(ДолгосрочнаяЗадолженностьКт.МестоУчета, Данные.МестоУчетаДт) КАК МестоУчетаДт,
		|	
		...
		|	ЕСТЬNULL(ДолгосрочнаяЗадолженностьКт.НаправлениеДеятельности, Данные.НаправлениеДеятельностиДт) КАК НаправлениеДеятельностиДт,
		|	ЕСТЬNULL(ДолгосрочнаяЗадолженностьДт.Долгосрочная, ЛОЖЬ) КАК ДолгосрочныйДт,
		|	
		|	Данные.СчетДт КАК СчетДт,
		...
		|	ЕСТЬNULL(ДолгосрочнаяЗадолженностьДт.АналитикаУчета, Данные.АналитикаУчетаКт) КАК АналитикаУчетаКт,
		|	ЕСТЬNULL(ДолгосрочнаяЗадолженностьДт.МестоУчета, Данные.МестоУчетаКт) КАК МестоУчетаКт,
		|	
		...
		|	ЕСТЬNULL(ДолгосрочнаяЗадолженностьДт.НаправлениеДеятельности, Данные.НаправлениеДеятельностиКт) КАК НаправлениеДеятельностиКт,
		|	ЕСТЬNULL(ДолгосрочнаяЗадолженностьКт.Долгосрочная, ЛОЖЬ) КАК ДолгосрочныйКт,
		|	
		|	Данные.СчетКт КАК СчетКт,
		...
		|	ЛОЖЬ КАК Сторно
		|
		|ПОМЕСТИТЬ ДанныеДляСопоставления12
		|
		|ИЗ
		|	Данные КАК Данные
		|	
		...
		Если ЕстьПроводкиПоРасходам Тогда
			ТекстЗапроса = "
			|ВЫБРАТЬ * ПОМЕСТИТЬ ДанныеДляСопоставления_Предварительная ИЗ ДанныеДляСопоставления1
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ * ИЗ ДанныеДляСопоставления2
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ * ИЗ ДанныеДляСопоставления3
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ * ИЗ ДанныеДляСопоставления4
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ * ИЗ ДанныеДляСопоставления5
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ * ИЗ ДанныеДляСопоставления6
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ * ИЗ ДанныеДляСопоставления7
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ * ИЗ ДанныеДляСопоставления8
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ * ИЗ ДанныеДляСопоставления9
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ * ИЗ ДанныеДляСопоставления10
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ * ИЗ ДанныеДляСопоставления11
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ * ИЗ ДанныеДляСопоставления12
			|";
			МассивТекстовЗапроса.Добавить(ТекстЗапроса);
		Иначе
			ТекстЗапроса = "
	...
	|		ТОГДА Данные.НаправлениеДеятельностиДт
	|		КОГДА (Данные.ВидСчетаДт В (&УчетЗатрат) И НЕ ЕСТЬNULL(НаправленияДеятельностиДт.УчетЗатрат, ЛОЖЬ))
	|			ИЛИ (Данные.ВидСчетаДт В (&УчетДоходов) И НЕ ЕСТЬNULL(НаправленияДеятельностиДт.УчетДоходов, ЛОЖЬ))
	|			ИЛИ (Данные.ВидСчетаДт В (&УчетДенежныхСредств) И НЕ ЕСТЬNULL(НаправленияДеятельностиДт.УчетДенежныхСредствРаздельно, ЛОЖЬ))
	|			ИЛИ (Данные.ВидСчетаДт В (&УчетВнеоборотныхАктивов) И НЕ ЕСТЬNULL(НаправленияДеятельностиДт.УчетВнеоборотныхАктивов, ЛОЖЬ))
	|			ИЛИ (Данные.ВидСчетаДт В (&УчетРасчетовСПоставщиками) И НЕ ЕСТЬNULL(НаправленияДеятельностиДт.УчетРасчетовСПоставщиками, ЛОЖЬ))
	|			ИЛИ (Данные.ВидСчетаДт В (&УчетРезервов) И ЕСТЬNULL(НаправленияДеятельностиДт.ГосударственныйКонтракт,
	|				ЗНАЧЕНИЕ(Справочник.ГосударственныеКонтракты.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.ГосударственныеКонтракты.ПустаяСсылка))
	|			ИЛИ (Данные.ВидСчетаДт В (&УчетРезервов) И НЕ ЕСТЬNULL(НаправленияДеятельностиДт.УчетЗатрат, ЛОЖЬ))
	...
	|		ТОГДА Данные.НаправлениеДеятельностиКт 
	|		КОГДА (Данные.ВидСчетаКт В (&УчетЗатрат) И НЕ ЕСТЬNULL(НаправленияДеятельностиКт.УчетЗатрат, ЛОЖЬ))
	|			ИЛИ (Данные.ВидСчетаКт В (&УчетДоходов) И НЕ ЕСТЬNULL(НаправленияДеятельностиКт.УчетДоходов, ЛОЖЬ))
	|			ИЛИ (Данные.ВидСчетаКт В (&УчетДенежныхСредств) И НЕ ЕСТЬNULL(НаправленияДеятельностиКт.УчетДенежныхСредствРаздельно, ЛОЖЬ))
	|			ИЛИ (Данные.ВидСчетаКт В (&УчетВнеоборотныхАктивов) И НЕ ЕСТЬNULL(НаправленияДеятельностиКт.УчетВнеоборотныхАктивов, ЛОЖЬ))
	|			ИЛИ (Данные.ВидСчетаКт В (&УчетРасчетовСПоставщиками) И НЕ ЕСТЬNULL(НаправленияДеятельностиКт.УчетРасчетовСПоставщиками, ЛОЖЬ))
	|			ИЛИ (Данные.ВидСчетаКт В (&УчетРезервов) И ЕСТЬNULL(НаправленияДеятельностиКт.ГосударственныйКонтракт,
	|				ЗНАЧЕНИЕ(Справочник.ГосударственныеКонтракты.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.ГосударственныеКонтракты.ПустаяСсылка))
	|			ИЛИ (Данные.ВидСчетаКт В (&УчетРезервов) И НЕ ЕСТЬNULL(НаправленияДеятельностиКт.УчетЗатрат, ЛОЖЬ))
	...
	|
	|ИЗ
	|	ДанныеДляСопоставления_Предварительная КАК Данные
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		НастройкиСтатейРасходов КАК НастройкиСтатейРасходовДт
	|	ПО
	...
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		НастройкиСтатейДоходов КАК НастройкиСтатейДоходовДт
	|	ПО
	|		НастройкиСтатейДоходовДт.СтатьяДоходов = Данные.АналитикаУчетаДт
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		НастройкиСтатейДоходов КАК НастройкиСтатейДоходовКт
	|	ПО
	|		НастройкиСтатейДоходовКт.СтатьяДоходов = Данные.АналитикаУчетаКт
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		Справочник.НаправленияДеятельности КАК НаправленияДеятельностиДт
	|	ПО
	|		НаправленияДеятельностиДт.Ссылка = Данные.НаправлениеДеятельностиДт
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		Справочник.НаправленияДеятельности КАК НаправленияДеятельностиКт
	|	ПО
	|		НаправленияДеятельностиКт.Ссылка = Данные.НаправлениеДеятельностиКт
	|";
	...
	Для каждого РазделУчета Из НастройкаСчетовУчетаКлиентСервер.СписокРазделовСчетовУчета(Истина).ВыгрузитьЗначения() Цикл
		...
		Если РазделУчета = "Доходы" Тогда
			...
			|		ТОГДА ЕСТЬNULL(НастройкиСтатейДоходов%1.ГФУ, &ПустоеЗначениеАналитикаУчета_Доходы)";
...
Функция ЗапросСопоставлений(ПодготовкаДанныхДляСопоставления = Истина, ЕстьДолгосрочнаяЗадолженность = Истина, ЕстьПроводкиПоРасходам = Истина) Экспорт
	
	МассивТекстовЗапроса = Новый Массив;
	
	Если ПодготовкаДанныхДляСопоставления Тогда
		Запрос = ЗапросОбработкиДанных(ЕстьДолгосрочнаяЗадолженность, ЕстьПроводкиПоРасходам);
	...
	|		ТОГДА НЕОПРЕДЕЛЕНО
	|		КОГДА ЕСТЬNULL(СчетаДтДанных.ВидСубконто1, НЕОПРЕДЕЛЕНО) = ВидыДт1.Ссылка ТОГДА Данные.СубконтоДт1
	|		КОГДА ЕСТЬNULL(СчетаДтДанных.ВидСубконто1, НЕОПРЕДЕЛЕНО) = ВидыДт2.Ссылка ТОГДА Данные.СубконтоДт2
	|		КОГДА ЕСТЬNULL(СчетаДтДанных.ВидСубконто1, НЕОПРЕДЕЛЕНО) = ВидыДт3.Ссылка ТОГДА Данные.СубконтоДт3
	...
	|		ТОГДА НЕОПРЕДЕЛЕНО
	|		КОГДА ЕСТЬNULL(СчетаДтДанных.ВидСубконто2, НЕОПРЕДЕЛЕНО) = ВидыДт1.Ссылка ТОГДА Данные.СубконтоДт1
	|		КОГДА ЕСТЬNULL(СчетаДтДанных.ВидСубконто2, НЕОПРЕДЕЛЕНО) = ВидыДт2.Ссылка ТОГДА Данные.СубконтоДт2
	|		КОГДА ЕСТЬNULL(СчетаДтДанных.ВидСубконто2, НЕОПРЕДЕЛЕНО) = ВидыДт3.Ссылка ТОГДА Данные.СубконтоДт3
	...
	|		ТОГДА НЕОПРЕДЕЛЕНО
	|		КОГДА ЕСТЬNULL(СчетаДтДанных.ВидСубконто3, НЕОПРЕДЕЛЕНО) = ВидыДт1.Ссылка ТОГДА Данные.СубконтоДт1
	|		КОГДА ЕСТЬNULL(СчетаДтДанных.ВидСубконто3, НЕОПРЕДЕЛЕНО) = ВидыДт2.Ссылка ТОГДА Данные.СубконтоДт2
	|		КОГДА ЕСТЬNULL(СчетаДтДанных.ВидСубконто3, НЕОПРЕДЕЛЕНО) = ВидыДт3.Ссылка ТОГДА Данные.СубконтоДт3
...
Функция ЗапросХозрасчетный() Экспорт
	...
	Запрос = Новый Запрос("
		...
		|	ХЗ.Организация КАК Организация,
		|	ЕСТЬNULL(ВтУточнениеПериода.УточнениеПериода, 0) КАК УточнениеПериода,
		|
		...
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ 
		|		ВтУточнениеПериода КАК ВтУточнениеПериода
		|	ПО
		|		ХЗ.Ссылка = ВтУточнениеПериода.Ссылка
		|
		|ГДЕ
		...
		|	ХЗ.Период,
		|	ХЗ.Организация,
		|	ЕСТЬNULL(ВтУточнениеПериода.УточнениеПериода, 0),
		|
...
Функция ЗапросЗаданияКФормированиюФинРезультата() Экспорт
	...
	Возврат Новый Запрос(
		...
		|	И НЕ ТИПЗНАЧЕНИЯ(РазрезыКОтражению.Ссылка) В (&ТипыДокументовВводаОстатков)
...
#Область ВтСтроки
	...
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.ГруппыАналитическогоУчетаНоменклатуры.ПустаяСсылка)
	|		КОГДА УчетнаяПолитикаОрганизаций.УчетПродукцииПереработчикаНа20Счете
 	|			И ЕСТЬNULL(КорВидыЗапасов.ТипЗапасов, ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПродукцияПереработчика)
	|			ТОГДА ЕСТЬNULL(КорАналитика.Номенклатура.ГруппаАналитическогоУчета, ЗНАЧЕНИЕ(Справочник.ГруппыАналитическогоУчетаНоменклатуры.ПустаяСсылка))
	|		КОГДА НЕ Движения.ГруппаПродукции = ЗНАЧЕНИЕ(Справочник.ГруппыАналитическогоУчетаНоменклатуры.ПустаяСсылка)
	|		 И НЕ ТИПЗНАЧЕНИЯ(Движения.Регистратор) В (&ТипыДокументовПроизводства22)
	|		 И НЕ Движения.ХозяйственнаяОперация В (
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПродукцииИзКладовой),
	...
	|
	|	ЕСТЬNULL(АналитикаПоПартнерам.НаправлениеДеятельности, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)) КАК НаправлениеДеятельностиКонтрагента, 
	|
	|
	|	ВЫБОР
	...
	|	ПО
	|		Движения.АналитикаУчетаПоПартнерам = АналитикаПоПартнерам.Ссылка
	|
	|	
	...
	|	КОНЕЦ,
	|
	|	ЕСТЬNULL(АналитикаПоПартнерам.НаправлениеДеятельности, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)), 
	|
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(Движения.Партия) = ТИП(Справочник.ПартииПроизводства)
	|			ТОГДА ВЫРАЗИТЬ(Движения.Партия КАК Справочник.ПартииПроизводства).НаправлениеДеятельности
...
Функция ТекстРасчетовПоНовойАрхитектуре()
	...
	|	Расчеты.КорДокументОбъектаРасчетов КАК КорДокументОбъектаРасчетов,
	|	Расчеты.КорПодразделение КАК КорПодразделение,
	|	Расчеты.КорГруппаФинансовогоУчета КАК КорГруппаФинансовогоУчета,
	|	Расчеты.КорНаправлениеДеятельности КАК КорНаправлениеДеятельности,
	|
	...
	|	Расчеты.КорДоговор,
	|	Расчеты.КорДокументОбъектаРасчетов,
...
Функция ТекстДолгосрочнаяЗадолженность(ДокументИмя, ОсобенностиДокумента)
...
Функция ТекстНастройкиСтатейРасходов(ЭтоДокументУчетаРасходов)
	ТекстЗапроса = 
	...
	|ИНДЕКСИРОВАТЬ ПО
	|	СтатьяРасходов,
	|	РаздельноеОтражение_УУ,
	|	РаздельноеОтражение_НУ
	|;
	|";
	
	Если ЭтоДокументУчетаРасходов Тогда
...
Функция ТекстНастройкиСтатейДоходов()
	Возврат
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СтатьиДоходов.Ссылка КАК СтатьяДоходов,
	|	СтатьиДоходов.ГруппаФинансовогоУчета КАК ГФУ
	|ПОМЕСТИТЬ НастройкиСтатейДоходов
	|ИЗ
...
Функция ТекстВтУточнениеПериода()
	
	Если Не БухгалтерскийУчетКлиентСервер.ИспользуетсяМежотчетныйПериод() Тогда
		Возврат 
			"ВЫБРАТЬ
			|	NULL КАК Ссылка,
			|	0 КАК УточнениеПериода
			|
			|ПОМЕСТИТЬ ВтУточнениеПериода
			|
			|;";
	КонецЕсли;
	...
	|	Регистратор.УточнениеПериода КАК УточнениеПериода
	|ПОМЕСТИТЬ ВтУточнениеПериода
	|ИЗ
	|	ДокументыКОтражению  КАК ДокументыКОтражению
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ИзменениеУсловийДоговораАренды КАК Регистратор
	|		ПО Регистратор.Ссылка = ДокументыКОтражению.Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ИЗ 
	|	ДокументыКОтражению  КАК ДокументыКОтражению
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КорректировкаСтоимостиИАмортизацииОС КАК Регистратор
	|		ПО Регистратор.Ссылка = ДокументыКОтражению.Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|ИЗ
	|	ДокументыКОтражению  КАК ДокументыКОтражению
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КорректировкаСтоимостиИАмортизацииНМА КАК Регистратор
	|		ПО Регистратор.Ссылка = ДокументыКОтражению.Ссылка
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка;";
...
Функция ТекстЗачетАвансов(ДокументИмя) Экспорт
	...
	|	И НЕ (Расчеты.Ссылка ССЫЛКА Документ.Бронирование И Расчеты.ДокументОбъектаРасчетов = Расчеты.Ссылка)
...
#Область РасходыНаСебестоимостьПродаж // Расходы на себестоимость продаж (Дт 90.02 :: Кт 20, 26, 44 )
	...
	|	КорСтроки.НаправлениеДеятельностиКонтрагента КАК НаправлениеДеятельностиДт,
	|	НЕОПРЕДЕЛЕНО КАК СчетДт,
	|
	|	Аналитика.Номенклатура КАК СубконтоДт1,
...
Функция СтрокаМестоУчета(МестоУчета) Экспорт
	...
	Если ЗначениеЗаполнено(МестоУчета) Тогда
		...
		СоответствиеТипаМестаПредставлению.Вставить(Тип("СправочникСсылка.Склады"),
			НСтр("ru = 'на складе ""%МестоУчета%""'", ОбщегоНазначения.КодОсновногоЯзыка()));
		СоответствиеТипаМестаПредставлению.Вставить(Тип("СправочникСсылка.СтруктураПредприятия"),
			НСтр("ru = 'в подразделении ""%МестоУчета%""'", ОбщегоНазначения.КодОсновногоЯзыка()));
		СоответствиеТипаМестаПредставлению.Вставить(Тип("СправочникСсылка.Партнеры"),
			НСтр("ru = 'по партнеру ""%МестоУчета%""'", ОбщегоНазначения.КодОсновногоЯзыка()));
		СоответствиеТипаМестаПредставлению.Вставить(Тип("ПеречислениеСсылка.ВидыЦенностей"),
			НСтр("ru = 'по виду ценности ""%МестоУчета%""'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		Если ПредставлениеАналитики = Неопределено Тогда
			ПредставлениеАналитики = НСтр("ru = 'по месту учета ""%МестоУчета%""'", ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
...
Функция СтрокаАналитикаУчета(АналитикаУчета) Экспорт
	...
	Если ЗначениеЗаполнено(АналитикаУчета) Тогда
		...
		СоответствиеТипаАналитикиПредставлению.Вставить(Тип("СправочникСсылка.ГруппыФинансовогоУчетаДоходовРасходов"),
			НСтр("ru = 'по группе фин. учета ""%АналитикаУчета%""'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		СоответствиеТипаАналитикиПредставлению.Вставить(Тип("ПланВидовХарактеристикСсылка.СтатьиДоходов"),
			НСтр("ru = 'по статье ""%АналитикаУчета%""'", ОбщегоНазначения.КодОсновногоЯзыка()));
		...
		СоответствиеТипаАналитикиПредставлению.Вставить(Тип("СправочникСсылка.КатегорииЭксплуатации"),
			НСтр("ru = 'для категории эксплуатации ""%АналитикаУчета%""'", ОбщегоНазначения.КодОсновногоЯзыка()));
		СоответствиеТипаАналитикиПредставлению.Вставить(Тип("СправочникСсылка.ВидыПодарочныхСертификатов"),
			НСтр("ru = 'для вида подарочного сертификата ""%АналитикаУчета%""'", ОбщегоНазначения.КодОсновногоЯзыка()));
		СоответствиеТипаАналитикиПредставлению.Вставить(Тип("СправочникСсылка.БанковскиеСчетаОрганизаций"),
			НСтр("ru = 'для банковского счета ""%АналитикаУчета%""'", ОбщегоНазначения.КодОсновногоЯзыка()));
		СоответствиеТипаАналитикиПредставлению.Вставить(Тип("СправочникСсылка.Кассы"),
			НСтр("ru = 'для кассы ""%АналитикаУчета%""'", ОбщегоНазначения.КодОсновногоЯзыка()));
		СоответствиеТипаАналитикиПредставлению.Вставить(Тип("ПеречислениеСсылка.ТипыНалогообложенияНДС"),
			НСтр("ru = 'для налогообложения ""%АналитикаУчета%""'", ОбщегоНазначения.КодОсновногоЯзыка()));
		
		ПредставлениеАналитики = СоответствиеТипаАналитикиПредставлению.Получить(ТипЗнч(АналитикаУчета));
		Если ПредставлениеАналитики = Неопределено Тогда
			ПредставлениеАналитики = НСтр("ru = 'для аналитики учета ""%АналитикаУчета%""'", ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
...
Функция СтрокаВидСчета(ИмяВидаСчета, РазделУчета) Экспорт
	...
	ИначеЕсли РазделУчета = "РасчетыСПартнерами"
		...
		Результат = НСтр("ru = 'переноса долгосрочной задолженности'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	ИначеЕсли РазделУчета = "РасчетыСКомитентамиПоЗакупке" И ИмяВидаСчета = "РасчетыСКлиентами" Тогда
		Результат = НСтр("ru = 'Расчеты с комитентами по закупке товаров'", ОбщегоНазначения.КодОсновногоЯзыка());
	ИначеЕсли РазделУчета = "РасчетыСКомитентамиПоЗакупке" И ИмяВидаСчета = "АвансыПолученные" Тогда
		Результат = НСтр("ru = 'Авансы с комитентами по закупке товаров'", ОбщегоНазначения.КодОсновногоЯзыка());
	ИначеЕсли РазделУчета = "РасчетыСПереработчиками" И ИмяВидаСчета = "РасчетыСКлиентами" Тогда
		Результат = НСтр("ru = 'Расчеты с переработчиками по выкупу'", ОбщегоНазначения.КодОсновногоЯзыка());
	ИначеЕсли РазделУчета = "РасчетыСПереработчиками" И ИмяВидаСчета = "РасчетыСПоставщиками" Тогда
		Результат = НСтр("ru = 'Расчеты с переработчиками по оказанию услуг'", ОбщегоНазначения.КодОсновногоЯзыка());
	ИначеЕсли РазделУчета = "РасчетыСПереработчиками" И ИмяВидаСчета = "АвансыПолученные" Тогда
		Результат = НСтр("ru = 'Авансы с переработчиками по выкупу'", ОбщегоНазначения.КодОсновногоЯзыка());
	ИначеЕсли РазделУчета = "РасчетыСПереработчиками" И ИмяВидаСчета = "АвансыВыданные" Тогда
		Результат = НСтр("ru = 'Авансы с переработчиками по оказанию услуг'", ОбщегоНазначения.КодОсновногоЯзыка());
	ИначеЕсли РазделУчета = "РасчетыСДавальцами" И ИмяВидаСчета = "РасчетыСКлиентами" Тогда
		Результат = НСтр("ru = 'Расчеты с давальцами по оказанию услуг'", ОбщегоНазначения.КодОсновногоЯзыка());
	ИначеЕсли РазделУчета = "РасчетыСДавальцами" И ИмяВидаСчета = "РасчетыСПоставщиками" Тогда
		Результат = НСтр("ru = 'Расчеты с давальцами по выкупу'", ОбщегоНазначения.КодОсновногоЯзыка());
	ИначеЕсли РазделУчета = "РасчетыСДавальцами" И ИмяВидаСчета = "АвансыПолученные" Тогда
		Результат = НСтр("ru = 'Авансы с давальцами по оказанию услуг'", ОбщегоНазначения.КодОсновногоЯзыка());
	ИначеЕсли РазделУчета = "РасчетыСДавальцами" И ИмяВидаСчета = "АвансыВыданные" Тогда
		Результат = НСтр("ru = 'Авансы с давальцами по выкупу'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Иначе
		СписокРазделов = РегистрыСведений.ПорядокОтраженияНаСчетахУчета.СписокРазделовСчетовУчета();
		ПредставлениеРазделаУчета = СписокРазделов.НайтиПоЗначению(РазделУчета).Представление;
		Результат = СтрШаблон(НСтр("ru = '%1 для вида счета %2'", ОбщегоНазначения.КодОсновногоЯзыка()), ПредставлениеРазделаУчета, ПредставлениеВидаСчета);
	КонецЕсли;
		...
		Если РазделУчета.Пометка Тогда
			...
			Если СписокСчетов.НайтиПоЗначению(ИмяВидаСчета) <> Неопределено Тогда
				...
				Иначе
					...
					Иначе
						Возврат СтрШаблон(НСтр("ru = '%1 для вида счета ""%2""'", ОбщегоНазначения.КодОсновногоЯзыка()), РазделУчета.Представление, Строка(ВидСчета))
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Функция ОсобенностиДокумента(ДокументИмя)
	
	Особенности = Новый Структура();
	Особенности.Вставить("ЭтоОбъектРасчетов", ЭтоОбъектРасчетов(ДокументИмя));
	Особенности.Вставить("ЭтоРасчетныйДокумент", ЭтоРасчетныйДокумент(ДокументИмя));
	Особенности.Вставить("ЭтоДокументУчетаНДС", ЭтоДокументУчетаНДС(ДокументИмя));
	Особенности.Вставить("ЭтоДокументУчетаСебестоимости", ЭтоДокументУчетаСебестоимости(ДокументИмя));
	Особенности.Вставить("ЭтоДокументУчетаРасходов", ЭтоДокументУчетаРасходов(ДокументИмя));
	Особенности.Вставить("ЭтоДокументОбесцененияВНА", ЭтоДокументОбесцененияВНА(ДокументИмя));
	Особенности.Вставить("ДокументИспользуетВыручкуИСебестоимостьПродаж", 
		ДокументыИспользующиеВыручкуИСебестоимостьПродаж().Найти(ДокументИмя) <> Неопределено);
...
Функция ЭтоДокументУчетаНДС(ДокументИмя) Экспорт
	...
	|ИЗ
	|	РегистрНакопления.НДСПредъявленный КАК НДСПредъявленный");
	ТипыРегистратора= Запрос.Выполнить().Выгрузить().Колонки.Регистратор.ТипЗначения;
	ДокументТип = Тип("ДокументСсылка." + ДокументИмя);
	
	Возврат ТипыРегистратора.СодержитТип(ДокументТип);
	
КонецФункции

Функция ЭтоДокументУчетаСебестоимости(ДокументИмя) Экспорт
...
Функция ЭтоДокументУчетаРасходов(ДокументИмя) Экспорт
...
Функция ЭтоДокументОбесцененияВНА(ДокументИмя) Экспорт
...
Функция ИспользуемыеТаблицыДляОтраженияТипа(ИмяТипаДокумента) Экспорт
	...
	ТаблицыТиповДокументов.Вставить("АктПремииКлиенту", ТаблицыДокумента);
	...
	ТаблицыДокумента = Новый Массив;
	ТаблицыДокумента.Добавить("Документ.ВводОстатковУзловКомпонентовАмортизации");
	ТаблицыДокумента.Добавить("РегистрНакопления.СтоимостьОС");
	ТаблицыДокумента.Добавить("РегистрНакопления.АмортизацияОС");
	ТаблицыТиповДокументов.Вставить("ВводОстатковУзловКомпонентовАмортизации", ТаблицыДокумента);
	...
	Если МассивТипов = Неопределено Тогда
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Отражение документов ""%1""
				|в регламентированном учете остановлено из-за отсутствия описания входящих данных.'"),
			ИмяТипаДокумента);
		ВызватьИсключение ТекстОшибки;	
	КонецЕсли;
...
Функция ПараметрыОтраженияРеглУчетаПоУмолчанию(ДетализацияПроводок = Ложь) Экспорт
	...
	СтруктураПараметров.Вставить("ИспользоватьУчетПрослеживаемыхИмпортныхТоваров",
								УчетПрослеживаемыхТоваровЛокализация.ИспользоватьУчетПрослеживаемыхИмпортныхТоваров(Дата(1, 1, 1)));
	СтруктураПараметров.Вставить("ДатаНачалаУчетаПрослеживаемыхИмпортныхТоваров",
