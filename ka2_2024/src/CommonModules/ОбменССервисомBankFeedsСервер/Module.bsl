...
Функция СписокБанковскиеСчетовИПериодыЗагрузкиПриСинхронизации(БанковскийСчет)
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ТаблицаЗагрузкиДат.ДатаДокумента, 0) = 0
	...
	Если ЗначениеЗаполнено(БанковскийСчет) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеОтбора", "СписокБанковскихСчетов.БанковскийСчет = &БанковскийСчет");
	Иначе
...
Функция ЗаполнитьБанковскиеСчетаПриВыгрузке(БанковскийСчет)
	...
	Если ЗначениеЗаполнено(БанковскийСчет) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеОтбора", "ТаблицаБанковскихСчетов.БанковскийСчет = &БанковскийСчет");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеОтбора", "ИСТИНА");
	КонецЕсли;	
...
//++ Локализация
	...
	//++ НЕ УТ
	|	ГражданствоФизическихЛицСрезПоследних.Страна КАК СтранаГражданстваРегистратора,
	//-- НЕ УТ
...
//++ Локализация
	...
	|		ПО ФизическиеЛица.Ссылка = ГражданствоФизическихЛицСрезПоследних.ФизическоеЛицо
	//-- НЕ УТ
