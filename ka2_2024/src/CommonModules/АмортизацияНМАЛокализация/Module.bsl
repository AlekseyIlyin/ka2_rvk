...
Процедура ДополнитьТекстЗапросаПараметрыРасчетаАмортизации(СписокЗапросов, ПараметрыРасчета) Экспорт
	...
	|
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(Изменения_БУ.ПодразделениеРасходов, СписокНМА.ПодразделениеБУ) <> ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|			ТОГДА ЕСТЬNULL(Изменения_БУ.ПодразделениеРасходов, СписокНМА.ПодразделениеБУ) 
	|		ИНАЧЕ ЕСТЬNULL(Изменения_БУ.Подразделение, МестоУчетаНМА.Подразделение) 
	|	КОНЕЦ КАК ПодразделениеРасходов,
	|
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(Изменения_БУ.ПодразделениеРасходов, СписокНМА.ПодразделениеБУ) <> ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|			ТОГДА ЕСТЬNULL(Изменения_БУ.НаправлениеДеятельностиРасходов, СписокНМА.НаправлениеДеятельностиБУ) 
	|		ИНАЧЕ ЕСТЬNULL(Изменения_БУ.НаправлениеДеятельностиРасходов, СписокНМА.НаправлениеДеятельностиБУ) 
	|	КОНЕЦ КАК НаправлениеДеятельностиРасходов,
	|
...
Процедура ОбъединитьРасчетАмортизацииБУНУ(ПараметрыРасчета)
	...
	|
	|		СУММА(СписокНМА.СуммаПР - СписокНМА.СуммаПРЦФ) КАК СуммаПР,
	|
	...
	|
	|		СУММА(СписокНМА.СуммаПРЦФ) КАК СуммаПРЦФ,
	|
	|		СписокНМА.ПередаватьРасходыВДругуюОрганизацию  КАК ПередаватьРасходыВДругуюОрганизацию,
	|		СписокНМА.ОрганизацияПолучательРасходов        КАК ОрганизацияПолучательРасходов,
	|
	...
	|
	|			ВЫБОР
	|				КОГДА (&СуммаПР_1 * РасчетАмортизации.КоэффициентЦФПР) > РасчетАмортизации.ОстаточнаяСтоимостьПРЦФ
	|							И РасчетАмортизации.ОстаточнаяСтоимостьПРЦФ >= 0
	|						ИЛИ (&СуммаПР_1 * РасчетАмортизации.КоэффициентЦФПР) < РасчетАмортизации.ОстаточнаяСтоимостьПРЦФ
	|							И РасчетАмортизации.ОстаточнаяСтоимостьПРЦФ <= 0
	|					ТОГДА РасчетАмортизации.ОстаточнаяСтоимостьПРЦФ
	|				ИНАЧЕ ВЫРАЗИТЬ(&СуммаПР_1 * РасчетАмортизации.КоэффициентЦФПР КАК ЧИСЛО(31,2))
	|			КОНЕЦ КАК СуммаПРЦФ,
	|
	|			0 КАК СуммаНУ,
	|			
	...
	|
	|			ВЫБОР
	|				КОГДА &СуммаПР_2 > РасчетАмортизации.ОстаточнаяСтоимостьПР
	|							И РасчетАмортизации.ОстаточнаяСтоимостьПР >= 0
	|						ИЛИ &СуммаПР_2 < РасчетАмортизации.ОстаточнаяСтоимостьПР
	|							И РасчетАмортизации.ОстаточнаяСтоимостьПР <= 0
	|					ТОГДА РасчетАмортизации.ОстаточнаяСтоимостьПР
	|				ИНАЧЕ &СуммаПР_2
	|			КОНЕЦ КАК СуммаПР,
	|
	|			ВЫБОР
	|				КОГДА (&СуммаПР_2 * РасчетАмортизации.КоэффициентЦФПР) > РасчетАмортизации.ОстаточнаяСтоимостьПРЦФ
	|							И РасчетАмортизации.ОстаточнаяСтоимостьПРЦФ >= 0
	|						ИЛИ (&СуммаПР_2 * РасчетАмортизации.КоэффициентЦФПР) < РасчетАмортизации.ОстаточнаяСтоимостьПРЦФ
	|							И РасчетАмортизации.ОстаточнаяСтоимостьПРЦФ <= 0
	|					ТОГДА РасчетАмортизации.ОстаточнаяСтоимостьПРЦФ
	|				ИНАЧЕ ВЫРАЗИТЬ(&СуммаПР_2 * РасчетАмортизации.КоэффициентЦФПР КАК ЧИСЛО(31,2))
	|			КОНЕЦ КАК СуммаПРЦФ,
	|
	|			0 КАК СуммаНУ,
	|			
...
Функция ТекстЗапросаСуммаАмортизацииБУ(ПараметрыТекстаЗапроса)
	...
	|
	|	КОГДА СписокНМА.АмортизацияНачисляетсяПоСтандартамМУ
	|			И (СписокНМА.ОстаточнаяСтоимостьРегл - &СуммаДоначисления - СписокНМА.ЛиквидационнаяСтоимость - &ОстатокОбесценения) <= 0 
	|
	|		ТОГДА 0
	|
	|	КОГДА СписокНМА.АмортизацияНачисляетсяПоСтандартамМУ
	...
	|
	|		ТОГДА СписокНМА.ОстаточнаяСтоимостьРегл - &СуммаДоначисления - СписокНМА.ЛиквидационнаяСтоимость - &ОстатокОбесценения
	|
	|	КОГДА НЕ СписокНМА.АмортизацияНачисляетсяПоСтандартамМУ
	|			И СписокНМА.ОстаточнаяСтоимостьРегл <= 0 
	|
	...
	|								/ ВЫРАЗИТЬ(СписокНМА.РесурсНазначенный - СписокНМА.РесурсВыработанныйЗаПрошлыйПериод КАК ЧИСЛО(20,8)) КАК ЧИСЛО(31,2))
	|
	|			ИНАЧЕ ВЫРАЗИТЬ(ВЫРАЗИТЬ(СписокНМА.СтоимостьДляВычисленияАмортизации - &СуммаДоначисления - СписокНМА.ЛиквидационнаяСтоимость КАК ЧИСЛО(20,8)) 
	|								* ВЫРАЗИТЬ(СписокНМА.РесурсВыработанныйЗаТекущийПериод КАК ЧИСЛО(20,8))
	|								/ ВЫРАЗИТЬ(СписокНМА.РесурсНазначенный - СписокНМА.РесурсВыработанныйЗаПрошлыйПериод КАК ЧИСЛО(20,8)) КАК ЧИСЛО(31,2))
	|		КОНЕЦ
	|
