...
Функция ПодготовитьСообщение(
	...
	ПодготовитьТехническуюИнформациюДляИнформационногоЦентра(
		ДанныеСообщения,
		ДанныеВложений);
	...
	СообщениеВИнформационныйЦентр.ДополнительныеДанные          = ДанныеСообщения;
...
//   *АдресВоВременномХранилище - Строка - адрес файла - вложения во временном хранилище.
//
Процедура ПодготовитьТехническуюИнформациюДляИнформационногоЦентра(ДанныеСообщения, ДанныеВложений)
	...
	// Т.к. данные передаются на клиент актуализируем данные.
	ОписаниеРасширений = Новый Массив;
	...
	Для Каждого ДанныеРасширения Из ДанныеРасширений Цикл
		...
		ОписаниеРасширения.Вставить("Имя", ДанныеРасширения.Имя);
		ОписаниеРасширения.Вставить("УникальныйИдентификатор", Строка(ДанныеРасширения.УникальныйИдентификатор));
		ОписаниеРасширения.Вставить("Активно", ДанныеРасширения.Активно);
		ОписаниеРасширения.Вставить("БезопасныйРежим", ДанныеРасширения.БезопасныйРежим);
		...
		Иначе
			ВызватьИсключение НСтр("ru = 'Неизвестный вариант назначения расширения'");
		КонецЕсли;
		ОписаниеРасширения.Вставить("ЗначениеНазначение", ЗначениеНазначение);
		ЭтоПатч = ДанныеРасширения.Назначение = НазначенияРасширенияКонфигурации.Исправление
			И СтрНачинаетсяС(ДанныеРасширения.Имя, "EF");
		ОписаниеРасширения.Вставить("ЭтоПатч", ЭтоПатч);
		ОписаниеРасширений.Добавить(ОписаниеРасширения);
		
	КонецЦикла;
	
	ДанныеТехническойИнформации.ДанныеРасширений = ОписаниеРасширений;
