...
Процедура СоздатьСальдоВременныхРазниц(МенеджерВременныхТаблиц, ОтчетнаяДата, КонтекстРасчета)
	...
	ПланГруппировки = ПланГруппировки();
...
Процедура РассчитатьТекущийНалог(ТаблицаРасчета, КонтекстРасчета)
	...
	ЭффектИзмененияВариантаНалогообложения = КонтекстРасчета.ДанныеПроводокИзмененияВарианта.Итог("Сумма");
	Если ЭффектИзмененияВариантаНалогообложения <> 0 Тогда
		Протокол = ТаблицаРасчета[0].РасчетРасходаПоНалогу;
		Протокол.ВлияниеОшибокУчетаОтложенногоНалога = Протокол.ВлияниеОшибокУчетаОтложенногоНалога + ЭффектИзмененияВариантаНалогообложения;
		Протокол.ЭффектИзмененияСтавкиТекущегоНалога = Протокол.ЭффектИзмененияСтавкиТекущегоНалога + ЭффектИзмененияВариантаНалогообложения;
		ДобавитьЗаписьПротоколаРасчета(Протокол, "ВлияниеОшибокУчетаОтложенногоНалога");
		ДобавитьЗаписьПротоколаРасчета(Протокол, "ЭффектИзмененияСтавкиТекущегоНалога");
	КонецЕсли;
...
Процедура ОбработатьИзменениеВариантаНалогообложенияПрибыли(ТаблицаРасчета, КонтекстРасчета)
	...
	ДанныеПроводокИзмененияВарианта = ДанныеПроводокИзмененияВарианта(МенеджерВременныхТаблиц, 
		ТаблицаРасчета, КонтекстРасчета, ЕстьНастройкиПоОбъектамУчета);
		
	КонтекстРасчета.Вставить("ДанныеПроводокИзмененияВарианта", ДанныеПроводокИзмененияВарианта);
...
Функция ДанныеПроводокИзмененияВарианта(МенеджерВременныхТаблиц, 
	...
	ДанныеПроводокИзмененияВарианта = ТаблицаДанныхПроводокИзмененияВарианта();
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока = ДанныеПроводокИзмененияВарианта.Добавить();		
		НоваяСтрока.РазницаСтавок = РазницаСтавок;
		НоваяСтрока.Сумма = Выборка.СуммаВР * (СтавкаНалогаКт - СтавкаНалогаДт); 
		
	КонецЦикла;
	
	ДанныеПроводокИзмененияВарианта.Свернуть("РазницаСтавок", "Сумма"); 
	
	Возврат ДанныеПроводокИзмененияВарианта;
	
КонецФункции

Процедура СформироватьПроводкиПриИзмененииВариантаНалогообложенияПрибыли(КонтекстРасчета, Проводки)
	
	ДанныеПроводокИзмененияВарианта = КонтекстРасчета.ДанныеПроводокИзмененияВарианта;
	
	Если ДанныеПроводокИзмененияВарианта.Количество() > 0
	...
	ВидыСубконтоВариантыНалогообложения =
 	...
 	Для каждого Строка Из ДанныеПроводокИзмененияВарианта Цикл
		...
		Проводка.Содержание  = СтрШаблон(ШаблонПроводки, Окр(Строка.РазницаСтавок * 100, 2)); 
...
Функция ТекстЗапроса_ДанныеПроводокПриИзмененииВариантаНалогообложенияПрибыли(КонтекстРасчета, 
	...
	Текст = "ВЫБРАТЬ
	        |	ВЫБОР
	        |		КОГДА ПравилаДт.ПозицияСубконтоВариантНалогообложения = 0
	        ...
	        |	ВЫБОР
	        |		КОГДА ПравилаКт.ПозицияСубконтоВариантНалогообложения = 0
	        ...
	        |	ПравилаДт.ВидАктивовОбязательств КАК ВидАктивовОбязательствДт,
	        |	ПравилаДт.СчетГруппировки КАК СчетДт,
	        |	ПравилаКт.ВидАктивовОбязательств КАК ВидАктивовОбязательствКт,
	        |	ПравилаКт.СчетГруппировки КАК СчетКт,
	        ...
	        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ПравилаГруппировки КАК ПравилаДт
	        |		ПО ДанныеПроводок.СчетДт = ПравилаДт.Счет
	        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ПравилаГруппировки КАК ПравилаКт
	        ...
	        |	ВТ_ДанныеПроводокПоВариантамНалогообложения.ВидАктивовОбязательствДт КАК ВидАктивовОбязательствДт,
	        |	ВТ_ДанныеПроводокПоВариантамНалогообложения.СчетДт КАК СчетДт,
	        |	ВТ_ДанныеПроводокПоВариантамНалогообложения.ВидАктивовОбязательствКт КАК ВидАктивовОбязательствКт,
	        |	ВТ_ДанныеПроводокПоВариантамНалогообложения.СчетКт КАК СчетКт,
	        |	ВТ_ДанныеПроводокПоВариантамНалогообложения.Организация КАК Организация,
	        |	ВТ_ДанныеПроводокПоВариантамНалогообложения.ПодразделениеДт КАК ПодразделениеДт,
	        |	ВТ_ДанныеПроводокПоВариантамНалогообложения.НаправлениеДеятельностиДт КАК НаправлениеДеятельностиДт,
	        |	ВТ_ДанныеПроводокПоВариантамНалогообложения.ПодразделениеКт КАК ПодразделениеКт,
	        |	ВТ_ДанныеПроводокПоВариантамНалогообложения.НаправлениеДеятельностиКт КАК НаправлениеДеятельностиКт,
	        |	ВТ_ДанныеПроводокПоВариантамНалогообложения.СуммаВР КАК СуммаВР
	        |ИЗ
