...
Процедура ЗарегистрироватьОшибки(ПараметрыВыполнения)
	...
	|
	|ИМЕЮЩИЕ
	|	(СУММА(ЕСТЬNULL(РасчетСтоимости.ФактическаяСтоимостьУУ, 0)) < СУММА(ВтТаблицаОбъектов.СтоимостьУУ)
	|			И ВтТаблицаОбъектов.ОтражатьВУпрУчете
	|		ИЛИ СУММА(ЕСТЬNULL(РасчетСтоимости.ФактическаяСтоимостьБУ, 0)) < СУММА(ВтТаблицаОбъектов.СтоимостьБУ)
	...
	|			И НЕ ВтТаблицаОбъектов.ОтражатьВБУ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Пока Выборка.Следующий() Цикл
		...
		ФактическаяСтоимостьБУ = Выборка.ФактическаяСтоимостьБУ;
		
		Если ТипЗнч(Выборка.Ссылка) = Тип("ДокументСсылка.МодернизацияОС2_4")
			И Выборка.ОтражатьВБУ
			И Выборка.ОтражатьВНУ
			И Выборка.ФактическаяСтоимостьБУ = 0
			И Выборка.ФактическаяСтоимостьНУ <> 0 Тогда
		
			ФактическаяСтоимостьБУ = Выборка.ФактическаяСтоимостьНУ;
		КонецЕсли;
		
		Если Выборка.ПредварительнаяСтоимостьБУ > ФактическаяСтоимостьБУ
			И Выборка.ОтражатьВБУ Тогда
			
			Если ФактическаяСтоимостьБУ = 0 Тогда
			...
			Иначе	
				ТекстОшибки = СтрШаблон(ТекстОшибки_БУ, 
										...
										Формат(ФактическаяСтоимостьБУ, "ЧДЦ=2; ЧН=0"),
										Выборка.ВалютаРегламентированногоУчета,
										Выборка.Ссылка);
			КонецЕсли; 
...
Процедура СформироватьТаблицуВтСписокДокументов(НачалоПериода, КонецПериода, СписокОрганизаций, МенеджерВременныхТаблиц)
	...
	|
	|	И ТекущиеДокументы.АналитикаКапитализацииРасходов В (
	|			ВЫБРАТЬ
