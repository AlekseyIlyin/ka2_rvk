...
Процедура ЗаполнитьСлужебныеРеквизитыПоНоменклатуре(Знач ПервичныйДокумент, ТоварыРасхождения) Экспорт
	...
	Если ТипЗнч(ПервичныйДокумент) = Тип("ДокументСсылка.ПередачаТоваровХранителю") Тогда
		...
		ПроверитьЗаполнитьСклад = ОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияСкладаВСтрокеТЧ(
			ПервичныйДокумент,
			СкладГруппа);
	...
	Попытка
		...
		Если ЗаписатьЗаказ Тогда
			Если ПересчитываемыеСтроки.Количество() > 0 Тогда
				...
				ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ПересчитываемыеСтроки, СтруктураДействий, Неопределено);
			КонецЕсли;
