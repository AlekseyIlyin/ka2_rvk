...
Процедура ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета)
	...
	//++ НЕ УТ
	РасшифровкаКодовОшибок.Вставить(15, НСтр("ru='Для документов выпуска обнаружены отрицательные движения по возвратным отходам при производстве давальческой продукции'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(16, НСтр("ru='Неправильно заполнена аналитика учета выпуска возвратных отходов (кор. аналитика учета номенклатуры, кор. аналитика финансового учета).'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
	...
	//++ НЕ УТ
	РасшифровкаКодовОшибок.Вставить(23, НСтр("ru='В незавершенном производстве в качестве партии производства используется ссылка на служебный элемент справочника ""Партии производства""'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
	...
	//++ НЕ УТ
	РасшифровкаКодовОшибок.Вставить(26, НСтр("ru='В движениях по распределению материалов в отчете переработчика заполнен реквизит ""Группа продукции"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(27, НСтр("ru='В движениях по выпуску продукции отличается заполнение реквизитов ""Аналитика финансового учета"" и ""Группа продукции"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
	...
	//++ НЕ УТ
	РасшифровкаКодовОшибок.Вставить(51, НСтр("ru='Не правильный раздел учета в движениях списания комиссионного товара.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
	...
	//++ НЕ УТ
	РасшифровкаКодовОшибок.Вставить(53, НСтр("ru='Неправильная хозяйственная операция в движениях выпуска побочных изделий на расходы.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
	...
	//++ НЕ УТ
	РасшифровкаКодовОшибок.Вставить(58, НСтр("ru = 'Аналитика финансового учета в приходном движении не соответствует партии производства.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
	...
	РасшифровкаКодовОшибок.Вставить(84, НСтр("ru='Заполнены различные коды строк в аналитике учета партии приходного документа при одинаковых аналитиках в строках документа'", ОбщегоНазначения.КодОсновногоЯзыка()));	
	РасшифровкаКодовОшибок.Вставить(85, НСтр("ru='Не заполнены реквизиты расчета партий и расчета себестоимости в движениях по регистру ""Движения Прочие активы-пассивы""'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
//++ Устарело_Переработка24
	...
	РасшифровкаКодовОшибок = Новый Соответствие;
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru='Упр. стоимость в движениях по регистру ""Себестоимость товаров"" не приведена к валюте упр. учета'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
		...
		Запрос,
		РасшифровкаКодовОшибок,
		НСтр("ru='приведение стоимостей в отчете переработчика к валюте упр. учета'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
//++ Устарело_Переработка24
	...
	РасшифровкаКодовОшибок = Новый Соответствие;
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru='Требуется сформировать движения по регистру ""Выпуск продукции"" для отчетов переработчиков'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(2, НСтр("ru='Требуется удалить движения по регистру ""Выпуск продукции"" для поступлений от переработчиков'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
		...
		Запрос,
		РасшифровкаКодовОшибок,
		НСтр("ru='некорректные движения по регистру ""Выпуск продукции""'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
//-- Устарело_Переработка24
	...
	// Сформируем описание возможных кодов ошибок в движениях.
	РасшифровкаКодовОшибок = Новый Соответствие;
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru='В незавершенном производстве в качестве партии производства используется ссылка на документ партии, а не на справочник ""Партии производства""'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
		ПараметрыРасчета,
		Запрос,
		РасшифровкаКодовОшибок,
		НСтр("ru='переход на новую партию производства'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
		...
		Запрос,
		РасшифровкаКодовОшибок,
		НСтр("ru='некорректные движения ""Трудозатраты НЗП""'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
//-- НЕ УТ
	...
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru = 'Неправильное движение в документе ""Производство без заказа"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасшифровкаКодовОшибок.Вставить(2, НСтр("ru = 'Не указывается ИФЗ в документе ""Производство без заказа"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасшифровкаКодовОшибок.Вставить(3, НСтр("ru = 'Неправильное движение в документе ""Этап производства"" при списании на другие партии: для забалансовых типов запасов раздел учета должен оставаться забалансовым.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(4, НСтр("ru = 'Неправильное движение в документе ""Производство без заказа"" при списании на другие партии: для забалансовых типов запасов раздел учета должен оставаться забалансовым.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыРасчета,
		Запрос,
		РасшифровкаКодовОшибок,
		НСтр("ru = 'исправление движений производственных документов'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	//++ НЕ УТ
	РасшифровкаКодовОшибок.Вставить(3, НСтр("ru = 'Неправильное движение возврата на другой склад в документе ""Возврат сырья давальцу"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
