...
Функция ПолучитьОбщиеДанныеДляЗаполнения(КонтекстВыполнения)
	...
	Если НЕ КонтекстВыполнения.ЭтоГодовойОтчет Тогда
		...
		Параметры.Вставить("НачалоПериода", ДобавитьМесяц(КонтекстВыполнения.НачалоПериодаОтчета, -12));
		Параметры.Вставить("КонецПериода", ДобавитьМесяц(КонтекстВыполнения.КонецПериодаОтчета, -12));
		Параметры.Вставить("ВариантНастроек", "");
		...
		Параметры.Вставить("НачалоПериода", ДобавитьМесяц(КонтекстВыполнения.НачалоПериодаОтчета, -24));
		Параметры.Вставить("КонецПериода", ДобавитьМесяц(КонтекстВыполнения.КонецПериодаОтчета, -24));
		Параметры.Вставить("ВариантНастроек", "");
...
Процедура ЗаполнитьПоказательПоАлгоритмамФормаОтчета2019Кв1(КонтекстВыполнения, Данные, Алгоритмы, ПараметрыПоказателя)
	...
	Для Каждого Алгоритм Из Алгоритмы Цикл
		...
		Иначе
			...
			ПараметрыПоказателя.КонецПериода = ДобавитьМесяц(КонтекстВыполнения.КонецПериодаОтчета, 12*КодГода);
		КонецЕсли;
...
Процедура ДобавитьРегулярныеГрафыФормыФормаОтчета2019Кв1(КонтекстВыполнения)
	...
	Иначе 
		ПериодПрошлыйГод = ДобавитьМесяц(КонтекстВыполнения.КонецПериодаОтчета,-1*12);
		ПериодПредПрошлыйГод = ДобавитьМесяц(КонтекстВыполнения.КонецПериодаОтчета,-2*12);
	КонецЕсли;
...
Процедура ЗаполнитьКоличествоНМАССальдо04_05(КонтекстВыполнения, ПараметрыПоказателя)
	...
	ГрафыКЗаполнению.Вставить("04", ДобавитьМесяц(КонтекстВыполнения.КонецПериодаОтчета, 12));
	ГрафыКЗаполнению.Вставить("05", ДобавитьМесяц(КонтекстВыполнения.КонецПериодаОтчета, 24));
...
Процедура ЗаполнитьПоказателиСПервоначальнойСтоимостьюНМА(КонтекстВыполнения, ПараметрыПоказателя)
	...
	ДатаКонцаПериодаОтчета = ДобавитьМесяц(КонтекстВыполнения.КонецПериодаОтчета, КорректировкаПериода);
...
Процедура ЗаполнитьПоказателиОДДС_ФормаОтчета2019Кв1(КонтекстВыполнения, ПараметрыПоказателя, Алгоритм, ЗначениеПоказателя)
	...
	ОбщиеПараметрыРасшифровки.Вставить("КонецПериода", ДобавитьМесяц(КонтекстВыполнения.КонецПериодаОтчета, 12*КодГода));
...
Функция ЗаполнитьПараметризируемыеНаименования(Наименование, КонтекстВыполнения, ПараметрыПоказателя = Неопределено)
	...
	ЗаголовокКонецСравнительногоПериода = СтрШаблон(ЗаголовокШаблон, Формат(ДобавитьМесяц(КонтекстВыполнения.КонецПериодаОтчета,-1*12), "ДФ = 'дд ММММ гггг'"));
...
Функция ЗаполнениеФормОтчетности(ПараметрыОтчета, РедакцияФормы = Неопределено)
	...
	Если РедакцияФормы = "ФормаОтчета2025Кв1" Тогда
		ЗаполнениеФормОтчетности.Вставить("Баланс", Истина);
		ЗаполнениеФормОтчетности.Вставить("ОФР", Истина);
		ЗаполнениеФормОтчетности.Вставить("ОИК", Истина);
		ЗаполнениеФормОтчетности.Вставить("ОДДС", Истина);
		ЗаполнениеФормОтчетности.Вставить("ОЦИПС", Истина);
		ЗаполнениеФормОтчетности.Вставить("Пояснения3", Истина);
		ЗаполнениеФормОтчетности.Вставить("Пояснения4", Истина);
		ЗаполнениеФормОтчетности.Вставить("Пояснения5", Истина);
		ЗаполнениеФормОтчетности.Вставить("Пояснения6", Истина);
		ЗаполнениеФормОтчетности.Вставить("Пояснения7", Истина);
		ЗаполнениеФормОтчетности.Вставить("Пояснения8", Истина);
		ЗаполнениеФормОтчетности.Вставить("Пояснения9", Истина);
		ЗаполнениеФормОтчетности.Вставить("Пояснения10", Истина);
		ЗаполнениеФормОтчетности.Вставить("Пояснения11", Истина);
	Иначе
