	...
	Если ПравилаЗаполнения <> Неопределено Тогда
		...
		Для Каждого СтрокаОтчетности Из СтрокиОтчетности Цикл
			...
			Если ПравилаСтроки <> Неопределено Тогда
				...
				ОпределитьНомерДопСтроки(СтрокаОтчетности, ПараметрыПоказателя, СчетчикДопСтрок);
	...
	Для Каждого Алгоритм Из Алгоритмы Цикл
		...
		Если ПоказательЗаполняетсяСтандартно(ПараметрыПоказателя.ДопАлгоритмСтроки) Тогда
			...
			РасшифровкаЗаголовок = СтрШаблон(РасшифровкаЗаголовок, 
				ЗаполнитьПараметризируемыеНаименования(СтрокаСчет.СтрокаНаименованиеРасшифровки, КонтекстОтчета, ПараметрыПоказателя),
				ГрафаНаименование);
				
			НастройкиРасшифровки = НастройкиРасшифровки(
				КонтекстВыполнения.Организация,
				ПараметрыПоказателя,
				АлгоритмСтрока.ТипПоказателя,
				Счета,
				КорСчета,
				ОтборПоАналитике);
...
Процедура ЗаполнитьДанныеРасчетаСтоимостиЧистыхАктивов(КонтекстВыполнения, ТаблицаСведенийСчета75_01, ЗначенияВычетаИзРасчетаАктивов1530)
	...
	Если КонтекстВыполнения.КонтекстОтчета.Свойство("РегулярныеГрафы") Тогда
		...
		Для Каждого Графа ИЗ КонтекстВыполнения.КонтекстОтчета.РегулярныеГрафы Цикл
...
Процедура ЗаполнитьФормуПояснения5(КонтекстВыполнения, ЗаполнятьПредыдущиеПериоды)
	...
	Для Каждого Строка Из Строки.Строки Цикл
		...
		ОпределитьНомерДопСтроки(СтрокаДанных, ПараметрыПоказателя, СчетчикДопСтрок);
...
Процедура ЗаполнитьСтрокуПояснений5(КонтекстВыполнения, ДанныеЗаполнения, ДанныеСтроки, ПараметрыПоказателя)
	...
	Для Каждого СтрокаСчета Из ДанныеСтроки Цикл
		...
		СтрокаЗаголовок = ЗаполнитьПараметризируемыеНаименования(СтрокаСчета.НаименованиеРасшифровки, КонтекстВыполнения.КонтекстОтчета, ПараметрыПоказателя);
...
Процедура ДобавитьРегулярныеГрафыФормы(КонтекстВыполнения)
	...
	КонецТекПериода.Вставить("КлючДанных", КонтекстОтчета.КонецПериодаОтчета);
...
Функция ЗаполнитьПараметризируемыеНаименования(Наименование, КонтекстОтчета, ПараметрыПоказателя = Неопределено)
...
Процедура ОпределитьНомерДопСтроки(Строка, ПараметрыПоказателя, СчетчикДопСтрок)
