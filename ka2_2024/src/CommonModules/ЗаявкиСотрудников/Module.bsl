...
Функция ТребуетсяПечатнаяФормаСПодписью(Ссылки) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Таблица.Ссылка.ВариантФормированияФайлаОтвета КАК ВариантФормированияФайлаОтвета
	|ИЗ
	|	БизнесПроцесс.ЗаявкаСотрудникаСправка2НДФЛ.СправкиНДФЛ КАК Таблица
	|ГДЕ
	|	Таблица.СправкаНДФЛ = &Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	Таблица.Ссылка.ВариантФормированияФайлаОтвета
	|ИЗ
	|	Документ.ЗаявкаСправка2НДФЛКабинетСотрудника.ДокументыПоЗаявке КАК Таблица
	|ГДЕ
	|	Таблица.ДокументСсылка = &Ссылка";
...
Процедура ЗарегистрироватьАннулированиеЗаявки(Заявка, Исполнитель)
	...
	Если РезультатВыполнения <> Перечисления.РезультатыВыполненияЗаявокКабинетСотрудника.Аннулирована Тогда
		...
		Иначе
			...
			ИмяТаблицы = ОбщегоНазначения.ИмяТаблицыПоСсылке(Заявка);
			
			Запрос = Новый Запрос;
			Запрос.УстановитьПараметр("Предмет", ЗаявкаЗадание);
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	ЗадачаИсполнителя.Ссылка КАК Ссылка
			|ИЗ
