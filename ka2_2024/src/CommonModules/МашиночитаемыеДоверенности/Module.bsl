		...
		Для Каждого СтрокаТаблицы Из РезультатПолученияПолномочий.Полномочия Цикл
			...
			Если ТипЗнч(Доверенность) = Тип("СправочникСсылка.МЧД003") Тогда
				ПроверяемоеПолномочие =
					Справочники.КлассификаторПолномочийМЧД003.НайтиПолномочиеПоКоду(СтрокаТаблицы.Код);
				ТаблицаНастроек = 
					Справочники.КлассификаторПолномочийМЧД003.ТаблицаНастроекОтбора(ПроверяемоеПолномочие);
			Иначе
				ПроверяемоеПолномочие =
			...
			Если ЗначениеЗаполнено(РеквизитыПолномочия.Скрипт) Тогда
				...
				Попытка
					Ответ = Новый Структура("Успех, ТекстОшибки", Ложь, "");
					ПараметрыПроверки = ПараметрыДляВыполненияСкриптаПроверкиПолномочий(Доверенность,
						ЭлектронныйДокумент, Ответ);
...
Функция ПараметрыДляВыполненияСкриптаПроверкиПолномочий(Доверенность, ЭлектронныйДокумент, Ответ)
	...
	СодержаниеДанныхОтправителя = ДанныеЭлектронногоДокумента.ДанныеОтправителя.Содержание;
