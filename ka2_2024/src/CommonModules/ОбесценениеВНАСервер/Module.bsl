...
Процедура ИнициализироватьВтНачисленияВосстановления(Запрос, ТекстыЗапроса)
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ втБалансоваяСтоимость
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	ДанныеПоЕГДС = Результат[11].Выгрузить(); // ТаблицаЗначений - 
...
Процедура ТекстЗапросаПрочиеДоходы(Запрос, ТекстыЗапроса)
	...
	|	""Прочие доходы, кроме восстановления обесценения"" КАК ИД
	|ИЗ
	...
	|	NULL,
	|	NULL,
	|	""Восстановление обесценения по документу ОбесценениеВНА при смене ЕГДС"" КАК ИД
	|ИЗ
	|	ВтОстаткиДляВосстановления КАК втОстаткиОбесценения
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтНовыеЕГДС КАК ВтНовыеЕГДС
	|		ПО (ВтНовыеЕГДС.ВнеоборотныйАктив = втОстаткиОбесценения.ВнеоборотныйАктив)
	...
	|	втОстаткиОбесценения.Подразделение,
	|	втОстаткиОбесценения.НаправлениеДеятельности,
	|	ВтОтражениеДоходовРасходов.СтатьяДоходов,
	|	ВтОтражениеДоходовРасходов.АналитикаДоходов,
	|	втОстаткиОбесценения.ОбесценениеРегл,
	|	втОстаткиОбесценения.ОбесценениеУпр,
	|	втОстаткиОбесценения.ОбесценениеУпр,
	|
	...
	|	NULL,
	|	NULL,
	|	""Восстановление обесценения при перемещении ВНА с ЕГДС в организации отправителе в другую организацию"" КАК ИД
	|ИЗ
	|	ВтОстаткиДляРеклассификации КАК втОстаткиОбесценения   
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтНовыеЕГДС КАК ВтНовыеЕГДС
	...
	|		ПО (ВтОтражениеДоходовРасходов.ВнеоборотныйАктив = втОстаткиОбесценения.ВнеоборотныйАктив)
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтНоваяАналитикаВНА КАК НоваяАналитика
	|		ПО (НоваяАналитика.ВнеоборотныйАктив = втОстаткиОбесценения.ВнеоборотныйАктив
	|			И НоваяАналитика.Организация <> втОстаткиОбесценения.Организация
	|			И втОстаткиОбесценения.ЕГДС <> ЗНАЧЕНИЕ(Справочник.ЕдиницыГенерирующиеДенежныеСредства.ПустаяСсылка))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	NULL,
	|	NULL,
	|	""Восстановление обесценения при изменении суммы обесценения документом ОбесценениеВНА"" КАК ИД
	|ИЗ
	|	втВосстановления КАК втВосстановления
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтОтражениеДоходовРасходов КАК ВтОтражениеДоходовРасходов
