...
Процедура ЗаполнитьЗначенияПоказателейРасчетаЗарплаты(МенеджерВременныхТаблиц, ТаблицаПоказателей) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СреднийЗаработок = 0;
		Если ПолучитьФункциональнуюОпцию("ИндексироватьСреднийЗаработокЧастично")
			И (Выборка.Показатель = ПоказателиСреднегоЗаработка["СреднийЗаработокИндексируемый"] Или Выборка.Показатель = ПоказателиСреднегоЗаработка["СреднийЗаработокНеиндексируемый"]) Тогда
			ЧастичнаяИндексация = ДанныеЧастичнойИндексации(Выборка.Сотрудник, Выборка.ДатаНачала, ДополнительныеПараметры);
			Если Выборка.Показатель = ПоказателиСреднегоЗаработка["СреднийЗаработокИндексируемый"] Тогда 
				СреднийЗаработок = ЧастичнаяИндексация.ИндексируемыйЗаработок;
			Иначе 
				СреднийЗаработок = ЧастичнаяИндексация.НеиндексируемыйЗаработок;
			КонецЕсли;
		Иначе 
...
Процедура ЗаполнитьЗначенияПоказателейРасчетаЗарплатыПоТаблицеЗначений(ДополнительныеПоказатели, ТаблицаПоказателей) Экспорт
	...
	Для Каждого СтрокаПоказателя Из ДополнительныеПоказатели Цикл
		...
		СреднийЗаработок = 0;
		Если ПолучитьФункциональнуюОпцию("ИндексироватьСреднийЗаработокЧастично")
			И (СтрокаПоказателя.Показатель = ПоказателиСреднегоЗаработка["СреднийЗаработокИндексируемый"] Или СтрокаПоказателя.Показатель = ПоказателиСреднегоЗаработка["СреднийЗаработокНеиндексируемый"]) Тогда
			ЧастичнаяИндексация = ДанныеЧастичнойИндексации(СтрокаПоказателя.Сотрудник, СтрокаПоказателя.ДатаНачала, ДополнительныеПараметры);
			Если СтрокаПоказателя.Показатель = ПоказателиСреднегоЗаработка["СреднийЗаработокИндексируемый"] Тогда 
				СреднийЗаработок = ЧастичнаяИндексация.ИндексируемыйЗаработок;
			Иначе 
				СреднийЗаработок = ЧастичнаяИндексация.НеиндексируемыйЗаработок;
			КонецЕсли;
...
Функция ДанныеЧастичнойИндексации(Сотрудник, ДатаНачалаСобытия, ДополнительныеПараметры = Неопределено) Экспорт
	...
	СреднийЗаработок = УчетСреднегоЗаработкаКлиентСервер.СреднийЗаработокОбщий(ДанныеДляРасчета.ДанныеОНачислениях, ДанныеДляРасчета.ДанныеОВремени, ДополнительныеПараметры);
	ЧастичнаяИндексация = УчетСреднегоЗаработкаКлиентСервер.ДанныеЧастичнойИндексацииСреднегоЗаработка(
		ДанныеДляРасчета.ДанныеОНачислениях, ДанныеДляРасчета.ДанныеОВремени, ДополнительныеПараметры, СреднийЗаработок);			
...
Процедура ЗаполнитьДанныеЧастичнойИндексацииОтпусков(Объект, ЧастичнаяИндексация, ИмяРеквизитаСреднийЗаработок) Экспорт 
	...
	Объект[ИмяРеквизитаИндексируемый] = ЧастичнаяИндексация.ИндексируемыйЗаработок;
	Объект[ИмяРеквизитаНеиндексируемый] = ЧастичнаяИндексация.НеиндексируемыйЗаработок;
...
Процедура СоздатьВТПолныеСуммыПремий(МенеджерВременныхТаблиц, ИсключаемыйРегистратор)
	...
	Запрос.Текст = 
		...
		|	ИЗ
		|		ВТПериодыРасчетаСреднегоСотрудников КАК ПериодыРасчетаСреднегоСотрудников
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДанныеРегистра_ДанныеОНачисленияхДляРасчетаСреднегоЗаработкаОбщий КАК ДанныеРегистра
