...
Функция ОбновитьАктуальныеЦеныТорговойПлощадки(УчетнаяЗапись, ЦенаПродажи, ТаблицаЦен) Экспорт
	...
	Попытка
		Запрос = Новый Запрос;
			...
			|		ВЫБОР
			|			КОГДА ЦеныНоменклатурыСрезПоследних.ВидЦены = &ВидЦен
			...
			|		И ЦеныНоменклатурыСрезПоследних.Номенклатура ЕСТЬ НЕ NULL) КАК АктуальныеЦены
			|
			|СГРУППИРОВАТЬ ПО
		...
		Если  ИспользуетсяЦенообразование25 Тогда   
			...
			ТекстИсточника = "РегистрСведений.ЦеныНоменклатуры25.СрезПоследних(
			|		,
			|		ВидЦены.СпособЗаданияЦены = ЗНАЧЕНИЕ(Перечисление.СпособыЗаданияЦен.ЗагружаетсяИзЯндексМаркет)
			|		ИЛИ ВидЦены = &ВидЦен) КАК ЦеныНоменклатурыСрезПоследних			
			|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатуры КАК ХарактеристикиНоменклатуры
			|			ПО (ХарактеристикиНоменклатуры.Владелец = ВЫБОР
			|			КОГДА ЦеныНоменклатурыСрезПоследних.Номенклатура.ИспользованиеХарактеристик = ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.ОбщиеДляВидаНоменклатуры)
			|				ТОГДА ЦеныНоменклатурыСрезПоследних.Номенклатура.ВидНоменклатуры
			...
			|					ПО (ЦеныНоменклатурыСрезПоследних.ВидЦены = ВнутренняяТаблицаЦен.ВидЦены)"; 
			
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстИсточника", ТекстИсточника);
			Если ИспользоватьУпаковкиНоменклатуры Тогда 
				УсловиеПоУпаковке ="(ЦеныНоменклатурыСрезПоследних.УпаковкаЦО = СтатусыПубликацииТоваровЯндексМаркет.Упаковка 
				|				   ИЛИ ЦеныНоменклатурыСрезПоследних.УпаковкаЦО = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка))"; 		
			Иначе
		...
		Иначе
			...
			ТекстИсточника = "РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&ТекущаяДата, ВидЦены = &ЦенаПродажи) КАК ЦеныНоменклатурыСрезПоследних
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыПубликацииТоваровЯндексМаркет КАК СтатусыПубликацииТоваровЯндексМаркет
			|		ПО (СтатусыПубликацииТоваровЯндексМаркет.УчетнаяЗапись = &УчетнаяЗапись)
			|			И ЦеныНоменклатурыСрезПоследних.Номенклатура = СтатусыПубликацииТоваровЯндексМаркет.Номенклатура
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстИсточника", ТекстИсточника);
			Если ИспользоватьХарактеристикиНоменклатуры Тогда 
				УсловиеПоХарактеристике ="ЦеныНоменклатурыСрезПоследних.Характеристика = СтатусыПубликацииТоваровЯндексМаркет.Характеристика"; 		
			Иначе
...
Функция ПолучитьДанныеУчетнойСистемыДляСравнения(УчетнаяЗапись, НоменклатураПартнера) Экспорт  
	...
	Если ВыборкаДанных.Следующий() Тогда
		...
		ШтрихКоды = ПолучитьШтрихКоды(ВыборкаДанных.Номенклатура, ВыборкаДанных.Характеристика);
