...
Функция ПолучитьСведенияОПоказателяхОтчета(ИДОтчета, ИДРедакцииОтчета, ПараметрыОтчета = Неопределено) Экспорт
	...
	ИначеЕсли ИДОтчета = "РегламентированныйОтчетИмущество" Тогда
		...
		ИначеЕсли ИДРедакцииОтчета = "ФормаОтчета2019Кв1" 
			...
			РегламентированнаяОтчетностьУП.ПолучитьСведенияОПоказателяхОтчетаИмуществоФормаОтчета2019_2022(ПоказателиОтчета, ИДРедакцииОтчета, ПараметрыОтчета);
...
//
// Пример:
//   Если Организация.ОтделениеИностраннойОрганизации Тогда
//		 Возврат Истина;
//	 Иначе
...
//
// Пример:
//   Если Организация.Метаданные().Реквизиты.Найти("ОтделениеИностраннойОрганизации") <> Неопределено Тогда
//		 Возврат Организация.ОтделениеИностраннойОрганизации;
//	 Иначе
//		 Возврат Ложь;
//	 КонецЕсли;
//
...
//                 |ИЗ
//                 |	РегистрСведений.ПараметрыНачисленияЗемельногоНалога.СрезПоследних(&ДатаСреза, Организация = &Организация) КАК РегистрацияЗемельныхУчастковСрезПоследних";
...
//                 |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПараметрыНачисленияЗемельногоНалога.СрезПоследних(&ДатаСреза, Организация = &Организация) КАК РегистрацияЗемельныхУчастковСрезПоследних
...
Процедура ДополнитьСтруктуруМногострочныхЧастей(Параметры, СтруктураМногострочныхЧастей) Экспорт
	...
	Если НастраиваемаяБухгалтерскаяОтчетность.ИспользуетсяНастраиваемаяОтчетность(Параметры) Тогда
		НастраиваемаяБухгалтерскаяОтчетность.ДополнитьСтруктураМогострочныхЧастей(Параметры, СтруктураМногострочныхЧастей);
	КонецЕсли;
