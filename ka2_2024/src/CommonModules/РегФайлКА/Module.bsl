...
Функция НаправленияУчетнойЗаписи(ФайлDOM) Экспорт

	Получатели  = ФайлDOM.ПолучитьЭлементыПоИмени("Получатель");
	...
	Для Каждого Получатель из Получатели Цикл
		...
		Тип = КонтекстЭДОСервер.ПолучитьЗначениеУзлаXML(Получатель,"ТипПолучателя");
			
		Если Метаданные.Перечисления.ТипыКонтролирующихОрганов.ЗначенияПеречисления.Найти(Тип) <> Неопределено Тогда
			Направления.Добавить(Перечисления.ТипыКонтролирующихОрганов[Тип]);
		КонецЕсли;
