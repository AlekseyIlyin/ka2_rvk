...
Функция ДанныеДляКадровогоПеревода(Сотрудники, ВремяРегистрацииСотрудников, ИсключаемыйРегистратор, ШаблонДляЗаполнения, ПолноеИмяОбъектаМетаданных="Документ.КадровыйПеревод") Экспорт
	...
	Если СтруктураДанных.ТабличныеЧасти.Свойство("ЕжегодныеОтпуска") Тогда
		...
		Для Каждого СтрокаСотрудника Из ДанныеСотрудников Цикл
			...
			ДанныеДокумента.ДатаСобытия = СтрокаСотрудника.Период;
			ДанныеДокумента.Вставить("ПериодПредыдущихЗначений", СтрокаСотрудника.РабочееМестоПериодРегистрации);
			...
			Если ДополнятьДанныеПозицииРабочимМестом Тогда
				...
				Объект.Вставить("Организация", СтрокаСотрудника.Организация);
				Объект.Вставить("РабочееМесто", СтрокаСотрудника.РабочееМестоОхраныТруда);
