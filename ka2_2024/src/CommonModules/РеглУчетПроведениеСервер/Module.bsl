	...
	Попытка
		...
		Для каждого СтрокаОшибки Из ТаблицаОшибокОтражения Цикл
			...
			Если ТипЗнч(СтрокаОшибки.ВидСчета) <> Тип("ПеречислениеСсылка.ВидыСчетовРеглУчета") Тогда
				ТекстСообщения = НСтр("ru = 'При записи счетов требующих настроек, ошибка: некорректно задан вид счета ""%1"".'", ОбщегоНазначения.КодОсновногоЯзыка());
				ТекстСообщения = СтрШаблон(ТекстСообщения, СтрокаОшибки.ВидСчета);
				ЗаписьЖурналаРегистрации(НСтр("ru = 'Отражение в регламентированном учете'", ОбщегоНазначения.КодОсновногоЯзыка()), 
			...
			Если РеглУчетВыборкиСерверПовтИсп.НастраиваемыеВидыСчетов().Найти(СтрокаОшибки.ВидСчета) = Неопределено Тогда
