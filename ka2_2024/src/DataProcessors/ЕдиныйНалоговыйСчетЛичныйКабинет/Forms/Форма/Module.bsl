...
Процедура ПодготовитьФормуНаСервере(КонтролироватьНаличиеДанных = Ложь)
	...
	ПоказатьСтраницуСРекламойСогласия = Не ИспользоватьСервисДанныхЕНС
		И Не ЕстьСогласиеНаПолучениеДанныхЕНС 
		И НастройкаПоказыватьСтраницуРекламыСогласия = Неопределено
		И Не ОтложитьПоказРекламыАвторизацииПоСогласию;
...
Процедура СообщитьОНекорректныхДанныхЧерезБИП()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.СообщенияВСлужбуТехническойПоддержки") Тогда
		...
		ДанныеСообщения.Сообщение            = СообщениеПользователяОбОшибкеВДанных(СведенияОбОрганизации, ЕстьПолноеВложениеЖурналаЗагрузки,
...
Процедура СообщитьОНекорректныхДанныхЧерезБТС()
	...
	ПараметрыОбращения.Текст    = СообщениеПользователяОбОшибкеВДанных(СведенияОбОрганизации, ЕстьПолноеВложениеЖурналаЗагрузки,
