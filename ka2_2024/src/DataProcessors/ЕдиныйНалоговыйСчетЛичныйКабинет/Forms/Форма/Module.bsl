...
&НаКлиенте
Процедура ДекорацияОшибкаАвторизацииПоСогласиюОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	Если НавигационнаяСсылкаФорматированнойСтроки = "СогласиеНаРаскрытиеНалоговойТайны" Тогда
		СоздатьСогласиеНаРаскрытиеНалоговойТайны();
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
...
&НаКлиенте
Процедура КартинкаЗапланированыРегламентныеРаботыЗакрытьБаннерНажатие(Элемент)
	
	ЗапланированаНедоступностьСервисаБаннерПогашен = Истина;
	Элементы.БаннерЗапланированыРегламентныеРаботы.Видимость = Ложь;
...
Процедура ПродолжитьСКЭП(Команда)
	...
	Если Не АвторизацииСЕНСПоКЭП И Не АвторизацииСЕНСПоСогласию Тогда
		СохранитьСпособАвторизацииСКЭПНаСервере(Объект.Организация);
	КонецЕсли;
...
Процедура ПрекратитьОбновлениеДанных()
	...
	Если ИспользуетсяАвторизацияСервиса И Не АвторизацииСЕНСПоСогласию Тогда
		РегистрыСведений.СостоянияПодключенияЕНС.СохранитьСпособАвторизации(Объект.Организация,
			Перечисления.СпособыАвторизацииКСервисуЕНС.ПоСогласию);
	КонецЕсли;
...
Процедура ПодготовитьФормуНаСервере(КонтролироватьНаличиеДанных = Ложь)
	...
	СпособАвторизацииСЕНС = РегистрыСведений.СостоянияПодключенияЕНС.ПолучитьСпособАвторизацииСЕНС(Объект.Организация);
	АвторизацииСЕНСПоКЭП = СпособАвторизацииСЕНС = Перечисления.СпособыАвторизацииКСервисуЕНС.ПоКЭП;
	АвторизацииСЕНСПоСогласию = СпособАвторизацииСЕНС = Перечисления.СпособыАвторизацииКСервисуЕНС.ПоСогласию;
	...
	Если Не ЕстьСогласиеНаПолучениеДанныхЕНС И АвторизацииСЕНСПоКЭП И НастройкаПоказыватьСтраницуРекламыСогласия = Неопределено
		И Не ОтложитьПоказРекламыАвторизацииПоСогласию Тогда
		ПоказатьСтраницуСРекламойСогласия = Истина;
	Иначе
		ПоказатьСтраницуСРекламойСогласия = Ложь;
	КонецЕсли;
...
Функция ПослеАвторизацииНаСервисеEnsIntegration(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Иначе
			...
			Иначе
				Если АвторизацииСЕНСПоСогласию Тогда
					ЕстьДоступКЭДО = Ложь;
					ПоказатьБаннерОшибкаАвторизацииПоСогласию = Истина;
					УправлениеФормой();
				Иначе
...
&НаСервереБезКонтекста
Процедура СохранитьСпособАвторизацииСКЭПНаСервере(Организация)
	РегистрыСведений.СостоянияПодключенияЕНС.СохранитьСпособАвторизации(Организация, Перечисления.СпособыАвторизацииКСервисуЕНС.ПоКЭП);
КонецПроцедуры
...
Процедура СообщитьОбОшибкеЧерезБИП()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.СообщенияВСлужбуТехническойПоддержки") Тогда
		...
		ВложенияПолнаяЗапись  = ВложениеЖурналЗагрузкиЕНС(Объект.Организация, Объект.Период, Истина);
		ВложенияКраткаяЗапись = ВложениеЖурналЗагрузкиЕНС(Объект.Организация, Объект.Период, Ложь);
		ЕстьПолноеВложениеЖурналаЗагрузки = ЗначениеЗаполнено(ВложенияПолнаяЗапись);
		ЕстьКраткоеВложениеЖурналаЗагрузки = ЗначениеЗаполнено(ВложенияКраткаяЗапись);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Вложения, ВложенияПолнаяЗапись);
		...
		ДанныеСообщения.Сообщение            = СообщениеПользователяОбОшибке(Истина, ЕстьПолноеВложениеЖурналаЗагрузки, Истина, ЕстьКраткоеВложениеЖурналаЗагрузки);
...
Процедура СообщитьОбОшибкеЧерезБТС()
	...
	Вложения = Новый Массив;
	ВложенияПолнаяЗапись  = ВложениеЖурналЗагрузкиЕНСДляБТС(Объект.Организация, Объект.Период, Истина);
	...
	ПараметрыОбращения.Текст    = СообщениеПользователяОбОшибке(ЕстьЗаписиЖурналаРегистраций, ЕстьПолноеВложениеЖурналаЗагрузки, Ложь, ЕстьКраткоеВложениеЖурналаЗагрузки);
...
Процедура СообщитьОНекорректныхДанныхЧерезБИП()
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.СообщенияВСлужбуТехническойПоддержки") Тогда
		...
		ДанныеСообщения.Сообщение            = СообщениеПользователяОбОшибкеВДанных(СведенияОбОрганизации, ЕстьПолноеВложениеЖурналаЗагрузки, Истина, ЕстьКраткоеВложениеЖурналаЗагрузки, ОписаниеПрограммы);
		ДанныеСообщения.ИНН                  = СведенияОбОрганизации.ИНН;
		ДанныеСообщения.ИспользоватьСтандартныйШаблон = Ложь;
...
Процедура СообщитьОНекорректныхДанныхЧерезБТС()
	...
	ПараметрыОбращения.Текст    = СообщениеПользователяОбОшибкеВДанных(СведенияОбОрганизации, ЕстьПолноеВложениеЖурналаЗагрузки, Ложь, ЕстьКраткоеВложениеЖурналаЗагрузки, ОписаниеПрограммы);
	ПараметрыОбращения.Вложения = Вложения;
	ПараметрыОбращения.ИспользоватьСтандартныйШаблон = Ложь;
	ИнформационныйЦентрКлиент.ОткрытьФормуНовогоОбращенияВСлужбуПоддержки(ПараметрыОбращения);
...
&НаКлиенте
Функция СообщениеПользователяОбОшибке(ЕстьЗаписиЖурналаРегистраций, ЕстьВложениеЖурналаЗагрузки, ЕстьТехническаяИнформация, ЕстьКраткийЛогЗагрузки)
	...
	Сообщение.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		...
		|#ОшибкаЗагрузки
		|Организация: ""%1"" ИНН: %2 КПП: %3 ОГРН: %4 Код НО: %5
		|Конфигурация: %6, версия %7 (версия подсистемы %8)'"),
		Объект.Организация,
		...
		Если ЕстьЗаписиЖурналаРегистраций Или ЕстьВложениеЖурналаЗагрузки Или ЕстьТехническаяИнформация Или ЕстьКраткийЛогЗагрузки Тогда
			Сообщение.Добавить(Символы.ПС);
			Сообщение.Добавить(НСтр("ru = 'Приложены файлы:'"));
		КонецЕсли;
		Если ЕстьВложениеЖурналаЗагрузки Тогда
			Сообщение.Добавить(Символы.ПС);
			Сообщение.Добавить(НСтр("ru = ' - записи из журнала загрузки единого налогового счета'"));
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция СообщениеПользователяОбОшибкеВДанных(СведенияОбОрганизации, ЕстьВложениеЖурналаЗагрузки, ЕстьТехническаяИнформация, ЕстьКраткийЛогЗагрузки, ОписаниеПрограммы)
	...
	Сообщение.Добавить(СтрШаблон(НСтр(
			...
			|#НекорректныеДанные
			|Данные на %4
			|Организация: ""%1"" ИНН: %2 КПП: %5 ОГРН: %6 Код НО: %7 Телефон: %3 
			|Конфигурация: %8, версия %9 (версия подсистемы %10)'"),
			...
			ОписаниеПрограммы.Версия,
			ОписаниеПрограммы.ВерсияПодсистемы));
	Если ЕстьВложениеЖурналаЗагрузки Или ЕстьТехническаяИнформация Или ЕстьКраткийЛогЗагрузки Тогда
		Сообщение.Добавить(Символы.ПС);
		Сообщение.Добавить(НСтр("ru = 'Приложены файлы:'"));
	КонецЕсли;
	Если ЕстьВложениеЖурналаЗагрузки Тогда
...
Процедура ПроверитьДоступностьСервиса()
	...
	ЕдиныйНалоговыйСчетИнтеграция.ПроверкаСервиса();
...
Процедура ОформитьДоступностьСервиса(СервисПерегружен = Ложь)
	...
	Если ЗначениеЗаполнено(НастройкиДоступностиЛичногоКабинетаЕНС) Тогда
		...
		Если Не ЗначениеЗаполнено(ПричинаБлокировки) Тогда
			...
			Элементы.ТекстРегламентныеРаботы.Заголовок         = ЗаголовокТехническиеРаботыФакт;
	...
	Если ЗначениеЗаполнено(НастройкиПлановойДоступностиЛичногоКабинетаЕНС) Тогда
		...
		Если Не ЗначениеЗаполнено(ПричинаБлокировки) Тогда
			...
			Если ДатаНачала > УниверсальнаяТекущаяДата // дата в будущем
				...
				Элементы.ТекстЗапланированыРегламентныеРаботы.Заголовок         = ЗаголовокТехническиеРаботыПлан;
