...
Процедура УстановитьТекстЗапросаСписок()
	...
	Если ОтборХозяйственныеОперации.НайтиПоЗначению(Перечисления.ХозяйственныеОперации.СписаниеОС) <> Неопределено Тогда
		...
		|			ТОГДА ТИПЗНАЧЕНИЯ(ВЫРАЗИТЬ(РеестрДокументовПереопределяемый.Ссылка КАК Документ.СписаниеОС2_4).ДокументОснование) = ТИП(Документ.ИзменениеУсловийДоговораАренды)";
	КонецЕсли;
	...
	Если ОтборХозяйственныеОперации.НайтиПоЗначению(Перечисления.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента) <> Неопределено Тогда
		...
		|			И ТИПЗНАЧЕНИЯ(РеестрДокументовПереопределяемый.Ссылка) В (ТИП(Документ.ПоступлениеБезналичныхДенежныхСредств), ТИП(Документ.ПриходныйКассовыйОрдер))
		|			ТОГДА ВЫРАЗИТЬ(РеестрДокументовПереопределяемый.Договор КАК Справочник.ДоговорыКонтрагентов).ТипДоговора В (&ДоговорыДоходнойАренды)";
	КонецЕсли;
