...
Процедура ДобавитьТекстЗапросаДвиженийАктивовПассивов(Запрос)
	...
	Если ПолучитьФункциональнуюОпцию("ФормироватьФинансовыйРезультат") Тогда
		...
		|	ТИПЗНАЧЕНИЯ(Т.Регистратор) <> ТИП(Документ.ВводОстатков)
		|	И ТИПЗНАЧЕНИЯ(Т.Регистратор) <> ТИП(Документ.ВводОстатковВнеоборотныхАктивов2_4)
		|	И ТИПЗНАЧЕНИЯ(Т.Регистратор) <> ТИП(Документ.ВводОстатковУзловКомпонентовАмортизации)
		|	И ТИПЗНАЧЕНИЯ(Т.Регистратор) <> ТИП(Документ.ВводОстатковПрочиеРасходы)
		|	И ТИПЗНАЧЕНИЯ(Т.Регистратор) <> ТИП(Документ.ВводОстатковПрочихАктивовПассивов)
		|	И ТИПЗНАЧЕНИЯ(Т.Регистратор) <> ТИП(Документ.ВводОстатковСПодотчетниками)
		...
		|			ИЛИ ТИПЗНАЧЕНИЯ(ДокументСторно.СторнируемыйДокумент) = ТИП(Документ.ВводОстатковУзловКомпонентовАмортизации)
