...
Процедура ОбработкаОповещенияЗаписиВладельца(ИмяСобытия, Параметр, Источник)
	...
	ОбновитьДанные = 
		ЗначениеЗаполнено(ВладелецЭЦП)
		И (ВладелецЭЦП = Параметр ИЛИ ВладелецЭЦП = Источник);
...
Процедура УправлениеФормой(ПриОткрытии = Ложь) Экспорт
	...
	Если ЭтоТекущаяСтраница(ЭтотОбъект, Элементы.НеЗаполненаОрганизация)
		...
		ИЛИ ЭтоТекущаяСтраница(ЭтотОбъект, Элементы.ВопросОНаличииЭЦП) И НЕ ЭтоОткрытиеСВыбраннойЭЦП Тогда
		Возврат;
	КонецЕсли;
...
Процедура ИнициализацияДляЭПВМоделиСервиса() Экспорт
	...
	Если НЕ ЗаявлениеСозданоКопированием Тогда
		...
		ИначеЕсли ЭтоРуководитель(ЭтотОбъект) И НЕ ИспользоватьСуществующий(ЭтотОбъект) ИЛИ НЕ ЗначениеЗаполнено(Организация) Тогда
			// АЦ УЦ
...
Процедура ПолучитьДанныеОрганизации_ИзСертификата(Форма, ВладелецСтруктура) Экспорт
	...
	// Краткое наименование
	ВладелецСтруктура.Свойство("O", Форма.КраткоеНаименование_ИзСертификата);
	
	// ОРГН
	Если НЕ Форма.ЭтоИностраннаяОрганизация Тогда
		ВладелецСтруктура.Свойство(?(Форма.ЭтоЮридическоеЛицо, "OGRN", "OGRNIP"), Форма.ОГРН_ИзСертификата);
	КонецЕсли;
...
Функция УдалосьОпределитьОрганизациюПоСертификату()
	...
	Если ЗначениеЗаполнено(ИНН_ИзСертификата) Тогда
