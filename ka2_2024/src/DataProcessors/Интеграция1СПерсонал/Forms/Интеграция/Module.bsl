...
Процедура УстановитьЗаголовкиСАдресом()
	...
	АдресТехподдержки =  АдресТехподдержки();
	ШаблонЗаголовка = НСтр("ru = 'Подключение займет некоторое время, после его завершения вы сможете продолжить.
		|Если при подключении возникли проблемы - свяжитесь с нами:'");
	ТекстЗаголовка = СтрШаблон("%1 %2", ШаблонЗаголовка, АдресТехподдержки);
	...
	ШаблонЗаголовка = НСтр("ru = 'Этот идентификатор нужно указывать при обращении в отдел технической поддержки по адресу'");
	ТекстЗаголовка = СтрШаблон("%1 %2", ШаблонЗаголовка, АдресТехподдержки);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ИдентификаторПриложения", "Подсказка", ТекстЗаголовка);
...
Функция АдресТехподдержки()
	...
	Возврат СтрШаблон("%1@%2", "support", ПостфиксАдреса());
