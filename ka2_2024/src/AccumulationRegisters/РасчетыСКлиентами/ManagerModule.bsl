...
Функция ЕстьИзмененияИтоговНабора(Набор, ДвиженияДоИзменения) Экспорт
	...
	Если ЕстьИзменения Тогда
		...
		Если ТипЗнч(ПроблемныйРегистратор) = Тип("ДокументСсылка.РегистраторРасчетов") Тогда
			СтрокиТекста.Добавить(ТекстРекомендацииДляРегистратораРасчетов());
		Иначе
...
// Возвращаемое значение:
//  Строка - Текст рекомендации для регистратора расчетов в случае не удачного обновления регистра
//
Функция ТекстРекомендацииДляРегистратораРасчетов() Экспорт
	
	Возврат НСтр("ru = 'Для аналитики учета по партнерам и объекта расчетов документа необходимо перезаполнить регистры взаиморасчетов обработкой ""Заполнение регистров взаиморасчетов"".'");
	
КонецФункции

// Возвращаемое значение:
//  Строка - Текст на случай непридвиденной ситуации при обновлении движений документа
//
Функция ТекстНепредвиденнаяСитуацияПриОбновленииДокумента() Экспорт
	
	Возврат НСтр("ru = 'При обновлении движений документа произошла непредвиденная ситуация. Обновление документа отменено.'");
	...
	Пока ОбновляемыеДанные.Следующий() Цикл
		...
		Исключение
			...
			Если ЗначениеЗаполнено(ПроблемныйРегистратор) Тогда
				...
				ОбновлениеИнформационнойБазы.ЗарегистрироватьПроблемуСДанными(
					ПроблемныйРегистратор,
					ТекстНепредвиденнаяСитуацияПриОбновленииДокумента()
