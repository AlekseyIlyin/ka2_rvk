...
Функция ТаблицаДвижений(Регистратор)
	...
	ЗаполнитьСтоимость(ТаблицаДвиженийУчетСебестоимости, Регистратор);
	...
	Если ЗначениеЗаполнено(ТаблицаДвиженийУчетСебестоимости) Тогда
		...
		Если ФормироватьСписаниеПоПартиям Тогда
			ТипыКорректировки = Новый Массив();
			ТипыКорректировки.Добавить(Тип("ДокументСсылка.КорректировкаРеализации"));
			ТипыКорректировки.Добавить(Тип("ДокументСсылка.КорректировкаПриобретения"));
			Отказ = Ложь;
			...
			КонецЕсли; 
			Если ТипыКорректировки.Найти(ТипЗнч(Регистратор)) = Неопределено Тогда
			...
			Иначе
				ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ОстаткиПоПартиямСписание, ТаблицаДвижений);	
			КонецЕсли;
...
Процедура ЗаполнитьСтоимость(ТаблицаДвиженийУчетСебестоимости, Регистратор)
	//TODO: Реализация ЗаполнитьСтоимость
	
	ТипРегистратора = ТипЗнч(Регистратор);
	
	ТекстЗапроса = "";
	Если ТипРегистратора = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		ТекстЗапроса = "ВЫБРАТЬ
		|	ТаблицаДвижений.АналитикаУчетаНоменклатуры,
		...
		|	усОстаткиПоПартиямОбороты.Партия,
		|	усОстаткиПоПартиямОбороты.КоличествоРасход КАК Количество,
		|	усОстаткиПоПартиямОбороты.СтоимостьРасход КАК Стоимость
		|ИЗ
		|	РегистрНакопления.усОстаткиПоПартиям.Обороты(,, Регистратор, АналитикаУчетаНоменклатуры В
		|		(ВЫБРАТЬ
		|			ВТ_ТаблицаДвижений.АналитикаУчетаНоменклатуры
		|		ИЗ
		|			ВТ_ТаблицаДвижений КАК ВТ_ТаблицаДвижений)) КАК усОстаткиПоПартиямОбороты
		|ГДЕ
		|	усОстаткиПоПартиямОбороты.Регистратор = &ОснованиеРегистратора";
	ИначеЕсли ТипРегистратора = Тип("ДокументСсылка.КорректировкаПриобретения") Тогда
		ТекстЗапроса = "ВЫБРАТЬ
		|	ТаблицаДвижений.АналитикаУчетаНоменклатуры,
		|	ТаблицаДвижений.Партия,
		|	ТаблицаДвижений.Количество,
		...
		|	усОстаткиПоПартиямОбороты.АналитикаУчетаНоменклатуры,
		|	усОстаткиПоПартиямОбороты.Партия,
		|	усОстаткиПоПартиямОбороты.КоличествоПриход КАК Количество,
		|	усОстаткиПоПартиямОбороты.СтоимостьПриход КАК Стоимость
		|ИЗ
		|	РегистрНакопления.усОстаткиПоПартиям.Обороты(,, Регистратор, АналитикаУчетаНоменклатуры В
		|		(ВЫБРАТЬ
		|			ВТ_ТаблицаДвижений.АналитикаУчетаНоменклатуры
	...
	Запрос.УстановитьПараметр("ТаблицаДвижений", ТаблицаДвиженийУчетСебестоимости);
	Запрос.УстановитьПараметр("ОснованиеРегистратора", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Регистратор, "ДокументОснование"));
	Выборка = Запрос.Выполнить().Выбрать();
	Для Каждого Строка Из ТаблицаДвиженийУчетСебестоимости Цикл
		Если Не ЗначениеЗаполнено(Строка.Стоимость) И Выборка.НайтиСледующий(Строка.АналитикаУчетаНоменклатуры, "АналитикаУчетаНоменклатуры") Тогда
			Строка.Партия = Выборка.Партия;
			Строка.Стоимость = Выборка.Стоимость / Выборка.Количество * Строка.Количество;
		КонецЕсли;
	КонецЦикла;
