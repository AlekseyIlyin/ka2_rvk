...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекОбъект.ВОКонтекстФормыДополнить(СтррКонтекст, "_ВидыЗадач", Истина);
...
&НаСервере
Функция ПолучитьПредставление(ИдТипаЗадания)
	
	Результат = ТипЗадачиСписок.НайтиПоЗначению(ИдТипаЗадания);
...
Процедура ПрочестьОбъектИзХранилища()
	...
	Для Каждого стзЭлемент Из ТЗн Цикл
		стзЭлемент.ТипЗадачиПредставление = ПолучитьПредставление(стзЭлемент.ТипЗадачи);	
	КонецЦикла;	
