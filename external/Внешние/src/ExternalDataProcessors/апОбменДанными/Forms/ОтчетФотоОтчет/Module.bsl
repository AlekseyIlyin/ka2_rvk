...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтррКонтекст.Вставить("Конфигурация");
	...
	Если ТекОбъект.ПоколениеКонфигурацииСервер(">=УТ_11.4.8", СтррКонтекст) Тогда 
		ТипОтчетаСтрокой = ВариантыОтчетовКлиентСервер.ТипОтчетаСтрокой(Параметры.ФотоОтчет, Истина);
	Иначе
	...
	Если Не ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НастройкиОтчета, "ВыводитьСуммуВыделенныхЯчеек", Истина) Тогда
		...
		Элементы.ОтчетТабличныйДокумент.УстановитьДействие("ОтчетТабличныйДокументПриАктивизацииОбласти", ""); // gi_240302 было "ПриАктивизацииОбласти"
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если МодульК().ПоколениеКонфигурации(">=УТ_11.4.8") Тогда  
		ИмяСобытияИзменениеВарианта = ВариантыОтчетовКлиентСервер.ПолноеИмяПодсистемы() + ".ИзменениеВарианта";
	Иначе
		//@skip-check unknown-method-property
		ИмяСобытияИзменениеВарианта = ВариантыОтчетовКлиентСервер.ИмяСобытияИзменениеВарианта();
	КонецЕсли;
	Если ИмяСобытия = ИмяСобытияИзменениеВарианта ИЛИ ИмяСобытия = "Запись_НаборКонстант" Тогда	
...
Процедура ОтчетТабличныйДокументОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") Тогда
		...
		ИначеЕсли Расшифровка.Свойство("Описание") Тогда
			...
			Оповещение = Новый ОписаниеОповещения("ВыполнитьПослеПредупреждение", ЭтотОбъект,);
			ПоказатьПредупреждение(Оповещение, Расшифровка.Описание);
	...
	ИначеЕсли Расшифровка = "" Тогда
		...
		Оповещение = Новый ОписаниеОповещения("ВыполнитьПослеПредупреждение", ЭтотОбъект,);		
		ПоказатьПредупреждение(Оповещение, "Файл фотографии не найден");
...
&НаКлиенте
Процедура ОтчетТабличныйДокументПриАктивизацииОбласти(Элемент) Экспорт
	// ???
КонецПроцедуры
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСоставОтчета
...
// ОбработчикиСобытийЭлементовТаблицыФормыСоставОтчета
#КонецОбласти
...
//		Попытка 
//			Если МодульК().ПоколениеКонфигурации(">=УТ_11.4.8") Тогда
//				Модуль = Вычислить("РаботаСФайламиКлиент");
//				Модуль.ОткрытьФайл(ДанныеФайла, Ложь);
//			Иначе
//				Модуль = Вычислить("ПрисоединенныеФайлыКлиент");
//				Модуль.ОткрытьФайл(ДанныеФайла, Ложь);
//			КонецЕсли;
//		Исключение
...
&НаСервере
Функция ПолучитьИдентификаторыПараметровОтчета(Настройки, Отчет)
...
Процедура СформироватьОтчетСервер(КомпоновщикНастроек,ПараметрыФОрмирования, ДокументРезультат)
	...
	КСтр = Новый КвалификаторыСтроки(36);
	Массив = Новый Массив;
	Массив.Добавить(Тип("Строка"));
	ОписаниеТиповС = Новый ОписаниеТипов(Массив, , КСтр);	
	...
	Если СтрНайти(ПараметрыФормирования.Действие, "ТабличныйДокумент") <> 0 Тогда
		...
		//@skip-check structure-consructor-too-many-keys
		СтррДанныеФото = Новый Структура("ВыводитьФотографии, КартинкаФотоНеНайдено, СозданаВТТ, ДатаСоздания, Фото, ФотоПредставление, ДокументМерчендайзинга, ДокументМерчендайзингаПредставление, Расширение, Описание, Широта, Широта2, Долгота,Долгота2, ХранимыйФайл, ВладелецФайла", 
					...
					Для каждого СтрокаГруппировкаКатегорияФото Из СтрокаГруппировка2.Строки Цикл
						...
						Пока Индекс <= (ФотоВСтроке - 1) Цикл	
							...
							Для Сч = 1 По КоличествоКолонок Цикл
								...
								ЗаполнитьЗначенияСвойств(СтррДанныеФото, Строка);
								//@skip-check wrong-string-literal-content
								СтррДанныеФото.ДатаСоздания = Формат(Строка["ДатаСоздания"],"ДФ=""дд.ММ.гггг""");
								ЗаполнитьОснову(ОбластьОснова, СтррДанныеФото);
								Если Первый Тогда
...
&НаСервере
Функция ПолучитьВыражениеУсловияДляЗапроса(ВидСравнения, ПравыйОперанд)
...
//
//	// gi_240301 - ошибка в конфигурациях УТ 11.4.14, КА 2.5.16.68:
//	//Возврат ПрисоединенныеФайлы.ПолучитьДанныеФайла(ФайлКартинки, ИдентификаторФормы);
//
//	//{{ gi_240301 - исправлено для совместимости с УТ 11.4.14 и выше 
//	ТекОбъект = РеквизитФормыВЗначение("Отчет");
//	УстановитьБезопасныйРежим(Истина);
//	Если ТекОбъект.ПоколениеКонфигурацииСервер(">=УТ_11.4.8", СтррКонтекст) Тогда
//		Модуль = Вычислить("РаботаСФайлами");
//		Возврат Модуль.ДанныеФайла(ФайлКартинки, ИдентификаторФормы);
//	Иначе
//		Модуль = Вычислить("ПрисоединенныеФайлы");
//		Возврат Модуль.ПолучитьДанныеФайла(ФайлКартинки, ИдентификаторФормы);
//	КонецЕсли;
//	//}}
//	
//КонецФункции
...
&НаКлиентеНаСервереБезКонтекста
Функция НайтиНастройкуВарианта(ТекОбъект, ИдентификаторЭлемента)
	ПоискНастройкиВарианта = ТекОбъект.БыстрыйПоискНастроекВарианта.Получить(ИдентификаторЭлемента);
	Если ПоискНастройкиВарианта = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;
	КорневойУзелКД = ТекОбъект.ФотоОтчет.КомпоновщикНастроек.Настройки.ПолучитьОбъектПоИдентификатору(ПоискНастройкиВарианта.ИдентификаторУзлаКД);
...
Процедура БыстрыеНастройкиЗаполнить(Знач ПараметрыКлиента)
	...
	Исключение
		...
		ВерсияОбработки = ЭтотОбъект.СтррКонтекст.Версия;
		Текст = "Версия обработки: " + ВерсияОбработки + ", конфигурация """ + Метаданные.Имя + " " + Метаданные.Версия + """:" + Символы.ПС + 
			"Происходит ошибка при попытке обновить ""быстрые настройки"" отчета ""Фото отчет"". Обратитьесь в техническую поддержку ""Агент Плюс"".";
		//@skip-check use-non-recommended-method
		Сообщить(Текст);
		//@skip-check use-non-recommended-method
...
Процедура ВыбратьПериод(Форма, КнопкаВыбораИмя) Экспорт
	...
	Контекст.Вставить("ИдентификаторЭлемента", ИдентификаторЭлемента);
	Обработчик = Новый ОписаниеОповещения("ВыбратьПериодЗавершение", ЭтотОбъект, Контекст);      //ОтчетыКлиент.ВыбратьПериодЗавершение()
