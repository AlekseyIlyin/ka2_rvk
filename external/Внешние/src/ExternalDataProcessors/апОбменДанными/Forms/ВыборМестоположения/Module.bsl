...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Местоположение") Тогда
		...
		КонецЕсли; 
	Иначе
		ВызватьИсключение("Форма ""ВыборМестоположения"": ожидается свойство Параметры.Местоположение типа ""Структура""!");
	КонецЕсли; 
...
Процедура КомандаАвтоопределение(Команда)
	...
	Если СтррРезультат.Свойство("error") Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Не удалось определить местоположение по причине:'")  + Символы.ПС + СтррРезультат.error);
	Иначе
