...
#Область ОбработчикиСобытийЭлементовТаблицыФормыТЗСтатусы
...
// ОбработчикиСобытийЭлементовТаблицыФормыТЗСтатусы
#КонецОбласти
...
Функция СтрРазделить_(Знач Строка, Знач Разделитель = ",", Знач ПропускатьПустыеСтроки = Неопределено,
...
Процедура ПрочитатьСтатусНастроекСервер()
	...
	ТекОбъект = РеквизитФормыВЗначение("Объект");	
	// sk_220623 MOD-1212 ТекОбъект.КонтекстФормыИнициализировать(СтррКонтекст, Параметры);
	
	СтррКонтекст.СтатусНастройкиОбмена = ТекОбъект.ГотовностьНастроекОбмена();
	СтррКонтекст.СтатусыСправочников = РеквизитФормыВЗначение("Объект").ГотовностьСправочников("НастройкиАгентов,МобильныеУстройства");
...
Процедура ШагСценарияЗагрузкаМУ()
	...
	Если Этап = 1 Тогда
		...
		Если СтррСтатусАгенты.Готово И СтррСтатусМУ.Готово Тогда // справочники агентов и МУ в порядке
			СтатусСтрокиОбновить(ШагСценария, 1, НСтр("ru = 'МУ и настройки агентов готовы.'"));
	...
	ИначеЕсли Этап = 3 Тогда // проверка результата загрузки МУ из ЛК - в т.ч. по оповещению "АПМобильныеУстройства_БыстраяНастройка"
		ПрочитатьСтатусНастроекСервер();		
		СтррСтатусМУ 	 = СтррКонтекст.СтатусыСправочников.МобильныеУстройства;
		Если Не СтррСтатусМУ.Готово Тогда
...
Процедура ШагСценарияНастройкиАгентов()
	...
	Если Этап = 1 Тогда
		ПрочитатьСтатусНастроекСервер();
		СтррСтатусАгенты = СтррКонтекст.СтатусыСправочников.НастройкиАгентов;
		Если СтррСтатусАгенты.Готово Тогда // настройки агентов корректны
