...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекОбъект.ВОКонтекстФормыДополнить(СтррКонтекст, "_ДополнительныеОтчетыДляМУ", Истина);
	ТекОбъект.ВОПриСозданииФормыСписка(ЭтотОбъект);
	
	СтвВидыКоманд = ТекОбъект.ВидыКомандДляМУ();
	Для Каждого Элемент Из СтвВидыКоманд Цикл
		ВидыКомандДляМУ.Добавить(Строка(Элемент.Ключ), Элемент.Значение);
	КонецЦикла;
	//ВидыКомандДляМУ = ТекОбъект.ВидыКомандДляМУ();
...
&НаКлиенте
Процедура ТПЭлементыПередНачаломИзменения(Элемент, Отказ)
	Отказ = Истина;
КонецПроцедуры
