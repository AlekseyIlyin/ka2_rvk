...
Процедура УказатьОбщееВремяПосещения(Команда)
	...
	Иначе
		...
		СтррПараметры = Новый Структура("Ключ,ВремяНачала,ВремяОкончания", СтрИндекс, ВремяНачала, ВремяОкончания);
...
//  ДобавлятьЧислоКРезультату   - Булево - добавлять числовое значение к результату в начале строки.                                       
//
// Возвращаемое значение:
...
Функция ГрафикПолучитьПредставление()
	...
	Для Каждого СтрокаТ Из ГрафикПосещений Цикл
		...
		Если РабочихДней = 5 Тогда
			ПредставлениеНедели = ?(Выходных = 2, НСтр("ru='все дни'"), НСтр("ru='по будням'") + ?(Выходных = 0, "",
				", " + СтрВыходные));
		Иначе
		...
		Если ПредставлениеНедели <> "" Тогда
			СтрРезультат = СтрРезультат + ?(СтрРезультат = "", "", "; ") + Строка(ТекНеделя + 1) + ": "
				+ ПредставлениеНедели;
		КонецЕсли;
...
Функция ГрафикПолучитьПункты()
	...
	СтррРезультат = Новый Структура("ПериодГрафика,УчитыватьВремяПосещения,Пункты", ПериодГрафика,
		УчитыватьВремяПосещения);
...
Процедура ВыборВремениЗавершение(СтррРезультат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(СтррРезультат) = Тип("Структура") И СтррРезультат.КодВозвратаДиалога = КодВозвратаДиалога.ОК Тогда
		...
		СтрВремя = ПолучитьСтрокуВремениИзДаты(СтррРезультат.ВремяНачала) + Символы.ПС + ПолучитьСтрокуВремениИзДаты(
			СтррРезультат.ВремяОкончания);
