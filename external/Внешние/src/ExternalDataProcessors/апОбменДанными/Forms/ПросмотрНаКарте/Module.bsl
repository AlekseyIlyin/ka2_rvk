...
Процедура ОбновитьЗаголовокФормы()
	...
	//@skip-check form-self-reference
	ЭтаФорма.Заголовок = НСтр("ru = 'Карта'") 
...
Функция ЧислоВСтроку(Число) 
	...
	Возврат Формат(Число, "ЧРД=.; ЧРГ=' '; ЧГ=0");
...
Процедура КартаОбновитьТекущийМакет()
	...
	ФлПоказатьМаркер = СтррМаркер.Широта <> 0 И тзМаркеры.Количество() <> 0;
	СтррЦентр = ?(ФлПоказатьМаркер, СтррМаркер, СтррКонтекст.КартаЦентр); // Если маркер еще не использовался, показываем центр карты
	СтрПараметры = КартаСформироватьПараметрРежимРедактора("Report", СтррЦентр); // Параметры скрипта
	...
	Иначе
		...
		#Если Не ВебКлиент Тогда
			...
			Документ = Элементы.ПолеБраузера.Document; // ExternalObject	
...
Процедура КартаДобавитьМаркеры(УстановитьТаймер = Ложь)
	...
	Попытка 
		...
		СтрТочекШирота  = ""; // для соединения маркеров полилинией аккумулируем координаты в переменных стрТочекШирота и стрТочекДолгота
		СтрТочекДолгота = "";
		...
		Для Каждого Маркер Из тзМаркеры Цикл
			...
			СтрТочекШирота	= СтрТочекШирота  + ЧислоВСтроку(Маркер.Широта)  + ";";
			СтрТочекДолгота = СтрТочекДолгота + ЧислоВСтроку(Маркер.Долгота) + ";";
			
		КонецЦикла;
		
		Если тзМаркеры.Количество() > 1 Тогда // соединяем маркеры прямой линией
			
			ПоследнийМаркер = тзМаркеры[тзМаркеры.Количество()-1];
			СтрПолигон = ЧислоВСтроку(ПервыйМаркер.Широта) + ";" + ЧислоВСтроку(ПервыйМаркер.Долгота) + ";" 
							+ ЧислоВСтроку(ПоследнийМаркер.Широта) + ";" + ЧислоВСтроку(ПоследнийМаркер.Долгота);
			
			СтрТочекШирота  = Лев(СтрТочекШирота, СтрДлина(СтрТочекШирота)-1);
			СтрТочекДолгота = Лев(СтрТочекДолгота, СтрДлина(СтрТочекДолгота)-1);
			
			СтрРасстояние = МодульКарты.РасстояниеМеждуТочкамиNMEAКлиентПредставление(
								ПервыйМаркер.Широта, ПервыйМаркер.Долгота, ПоследнийМаркер.Широта, ПоследнийМаркер.Долгота);
			
			ЦветЛинии = "#20bcff"; 
			ТекстИнфо = "Расстояние: " + СтрРасстояние;
			ОкноБраузера.setPolyline(СтрТочекШирота, СтрТочекДолгота, "", ЦветЛинии, 5, "solid",  85/100, СтрПолигон, "", "", ТекстИнфо, "");
			//ОкноБраузера.SetBounds(стрПолигон);
...
// Это используется??? Или копи-паст из другого модуля?
// Параметры:
