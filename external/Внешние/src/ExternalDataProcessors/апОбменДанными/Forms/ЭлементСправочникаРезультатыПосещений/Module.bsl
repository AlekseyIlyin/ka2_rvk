...
&НаКлиенте
Процедура ШаблонАнкеты_ПредставлениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)

	СтандартнаяОбработка = Ложь;
	
	СтррПараметры  = Новый Структура("ID,ДляВыбора", ШаблонАнкеты, Истина);
	Оповещение = Новый ОписаниеОповещения("ШаблонАнкеты_ПредставлениеВыборЗавершение", ЭтотОбъект, Неопределено);
	
	//@skip-check form-self-reference
	МодульМТ.ОткрытьФормуОбработки("СправочникШаблоныАнкет", СтррПараметры, ЭтаФорма.КлючУникальности, Оповещение);
	
КонецПроцедуры

&НаКлиенте
Процедура ШаблонАнкеты_ПредставлениеОткрытие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	МодульМТ.ВООткрытьФорму("_ШаблоныАнкет", ШаблонАнкеты);
...
Процедура СохранитьОбъектВХранилище()
	...
	СтрРеквизитыШапки = "Статус,Наименование,Комментарий,ШаблонАнкеты,ШаблонАнкеты_Представление";
...
&НаКлиенте
Процедура ШаблонАнкеты_ПредставлениеВыборЗавершение(СтррРезультат, ДополнительныеПараметры) Экспорт
	
	Если СтррРезультат = Неопределено Тогда
		Возврат;
	Иначе
		ШаблонАнкеты = СтррРезультат.ID;
		ШаблонАнкеты_Представление = СтррРезультат.Наименование;
		//@skip-check form-self-reference
		ЭтаФорма.Модифицированность = Истина;
	КонецЕсли; 
