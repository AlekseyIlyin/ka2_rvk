...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Добавление колонки "АКФ_СпрТТ" в реквизит Состав и добавление элемента (колонка "СПРАВОЧНИК ТТ" в ТЗ) на форму
...
Процедура ПриОткрытии(Отказ)
	...
	Если ДатаНачала = '00010101' Тогда
		ДатаПриИзменении(Элементы.ДатаНачала);
	КонецЕсли; 
	...
	ЦиклическийПриИзменении(Элементы.Циклический);
...
&НаКлиенте
Процедура ЦиклическийПриИзменении(Элемент)
	
	УстановитьМодифицированостьФормы(Истина);
	
	Элементы.ДатаОкончания.Доступность = Циклический;
	
	Если Элементы.СоставПланПосещений.Видимость Тогда
		Элементы.ПериодВыводаПлана.Видимость = Циклический;
	Конецесли;
	
	КорректировкаДатаОкончания();
...
Процедура ДатаПриИзменении(Элемент)
	...
	ИначеЕсли ДатаОкончания <> '00010101' И НачалоДня(КонецНедели(ДатаОкончания)) <> ДатаОкончания Тогда
		ДатаОкончания = КонецНедели(ДатаОкончания);
		ОчиститьСообщения();
		Текст = НСтр("ru = 'Дата окончания должна быть уставновлена на конец недели.'");
		//@skip-check object-deprecated
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст, Неопределено, "ДатаОкончания");
	...
	Если СтрокаТ <> Неопределено Тогда
		...
		Если Имя = "СоставГрафик" Или Имя = "СоставПланПосещений" Тогда
			...
			Иначе // редактирование строки плана посещения партнера
				...
				Если СтррКонтекст.Конфигурация = "УТ_АКФ" Тогда
					СтррПараметры.Вставить("График", ПППолучитьПланПоПартнеру(СтрокаТ["ИдАдресаДоставки"]));
				Иначе
...
#Область СлужебныеПроцедурыИФункции_ОбработчикиСобытийТДПланПосещений
...
// СлужебныеПроцедурыИФункции_ОбработчикиСобытийТДПланПосещений
#КонецОбласти
...
Процедура ПрочестьОбъектИзХранилища(КлючID) 
	...
	Если СтррОбъект = Неопределено Тогда // создание нового элемента списка, нужно назначить автоматическое имя списка
		...
		Циклический 	= Истина;
	Иначе
		Если НЕ СтррОбъект.Свойство("Циклический") Тогда
			Циклический = Истина;
		КонецЕсли;			
...
Процедура СохранитьОбъектВХранилище()
	...
	// Сохраняемые реквизиты объекта
	//@skip-check structure-consructor-too-many-keys
	СтррОбъект = Новый Структура("Наименование,
								|	Детализация,
								|	ПериодГрафикаПлана,
								|	ДатаНачала,
								|	ДатаОкончания,
...
Процедура АКФ_ОбновитьДанныеСтрокиПоТорговойТочке(СтрокаТ)
	...
	//@skip-check structure-consructor-too-many-keys
	CтррРеквизитыТТ = Новый Структура("СпрТТПартнер, СпрТТАдрес, СпрТТАдресГуидАП, СпрТТЗонаДоставки");
	
	АКФ_ПолучитьДанныеТорговойТочки(СтрокаТ.АКФ_СпрТТ, CтррРеквизитыТТ);
	
	СтрокаТ.Партнер              		   = CтррРеквизитыТТ.СпрТТПартнер;
	СтрокаТ.Адрес						   = CтррРеквизитыТТ.СпрТТАдрес;
...
Процедура ОбновитьДетализациюСТТ(МодифицированностьФормы = Неопределено)
	...
	Элементы.Циклический.Видимость = ВидимостьПланаПосещений;
	...
	Элементы.ПериодВыводаПлана.Видимость = ?(ВидимостьПланаПосещений И Циклический, Истина, Ложь);
...
#Область ВыводСтрокТЧ
	...
	Если СтррКонтекст.Конфигурация = "УТ_АКФ" Тогда
		ПараметрыВывода.Вставить("стррПоиск", Новый Структура("ИдАдресаДоставки"));
	Иначе
...
Функция ТДППВывестиСтроку(Макет, СтрокаТ, ПараметрыВывода)
	...
	Если СтррКонтекст.Конфигурация = "УТ_АКФ" Тогда
		СтррПоиск["ИдАдресаДоставки"] = СтрокаТ["ИдАдресаДоставки"];
	Иначе
...
Функция ТДППЦветаДляМакета()
	...
	СтррРезультат.Вставить("ЦветПервогоДня", Новый Цвет(204, 255, 204));
	//@skip-check new-color
	СтррРезультат.Вставить("ЦветПоследнегоДня", Новый Цвет(255, 200, 200));
	//@skip-check new-color
	СтррРезультат.Вставить("ЦветФона0", Новый Цвет(-1,-1,-1));
	//@skip-check new-color
	СтррРезультат.Вставить("ЦветФона1", Новый Цвет(240,240,240));
	//@skip-check structure-consructor-too-many-keys
...
&НаКлиенте
Процедура ТДПППерекраситьКолонку(АдресКолонки, ИтогКолонки = Неопределено)
...
Функция ПППолучитьДатуНачалаТекущегоПериода(ДатаНачала, ПериодГрафикаПлана)
	...
	Если НЕ Циклический Тогда
		Возврат ДатаНачала;
	КонецЕсли;
	
	ПромДатаНачала = ДатаНачала;
	//@skip-check use-non-recommended-method
...
Процедура ОбновитьТаблицуАгентовНаСервере()
	...
	Для Каждого Элемент Из ТекОбъект.НастройкиАгентов Цикл
		Если Элемент.СсылкаСписокТТ = ID И ЗначениеЗаполнено(ID) Тогда
			...
			НоваяСтрока.ФИО = Элемент.Пользователь.ФизическоеЛицо;
...
//  ДобавлятьЧислоКРезультату   - Булево - добавлять числовое значение в начале строки.
//
// Возвращаемое значение:
...
&НаКлиенте
Процедура КорректировкаДатаОкончания()

	Если НЕ Циклический Тогда
		ДатаОкончания = ДатаНачала + ((ПериодГрафикаПлана - 1) * 86400);		
	КонецЕсли;
